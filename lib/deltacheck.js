...
DeltaCheck.prototype.executeAutoFixPicklists = async function() {
    ...
    for (var sObjectType in this.autoFixPicklistsInfo.deployPicklistValues) {
        ...
        for (var i = 0; i < customObject.fields.length; i++) {
            if (this.autoFixPicklistsInfo.deployPicklistValues[sObjectType].hasOwnProperty(customObject.fields[i].fullName)) {

                var dataPackPicklistValues = this.autoFixPicklistsInfo.deployPicklistValues[sObjectType][customObject.fields[i].fullName];

                if (customObject.fields[i].type === 'MultiselectPicklist') {
                    var component = customObject.fields[i].fullName;

                    if (component.includes('__c')) {
                        component = component.substring(0, component.indexOf('__c'));
                    }

                    var retrievedMetadata = await this.vlocity.utilityservice.retrieveGlobalValueSet(component);
                    ...
                    if (retrievedMetadata) {
                        ...
                        for (var y = 0; y < dataPackPicklistValues.length; y++) {
                            ...
                            for (var dataPackPickListValue of dataPackPicklistValues[y]) {
                                ...
                                if (!valueFound) {
                                    retrievedMetadata.customValue.push({fullName: dataPackPickListValue, default: false, label: dataPackPickListValue});
                                }
                            }
                        }

                        globalValueSet.push(retrievedMetadata);
                    }
                } else {
                    for (var y = 0; y < dataPackPicklistValues.length; y++) {
                        var valueFound = false;

                        for (var z = 0; z < customObject.fields[i].valueSet.valueSetDefinition.value.length; z++) {
                            if (customObject.fields[i].valueSet.valueSetDefinition.value[z].fullName === dataPackPicklistValues[y]) {
                                customObject.fields[i].valueSet.valueSetDefinition.value[z].isActive = true;
                                valueFound = true;
                                break;
                            }
                        }
                        ...
                        if (!valueFound) {
                            customObject.fields[i].valueSet.valueSetDefinition.value.push({ default: false, isActive: true, label: dataPackPicklistValues[y], fullName: dataPackPicklistValues[y]});
                        }
                    }
                }
            }
