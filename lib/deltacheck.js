...
DeltaCheck.prototype.runDeltaCheck = async function(jobInfo, currentContextData) {
    ...
    this.actionName = jobInfo.executeAction ? jobInfo.executeAction : jobInfo.jobAction;

    if (this.actionName === 'CheckStaleObjects') {
        this.jobInfo.checkStaleObjects = true;
    } else if (this.actionName === 'DeltaMetadataCheck') {
        this.jobInfo.deltaMetadataCheck = true;
    }
    ...
    await this['execute' + this.actionName]();
};

DeltaCheck.prototype.executeDeltaMetadataCheck = async function() {

    for (var sObject in this.deltaMetadataCheckInfo) {
        var fieldName = this.deltaMetadataCheckInfo[sObject].fieldName;
        var query = 'SELECT ' + fieldName + ' FROM ' + sObject;

        var result = await this.vlocity.queryservice.query(query);   
            
        if (!result) {
            for (var i = 0; i < this.deltaMetadataCheckInfo[sObject].vlocityDataPackKey.length; i++) {
                this.jobInfo.deltaCheckResults[this.deltaMetadataCheckInfo[sObject].vlocityDataPackKey[i]] = { status: 'Error' };   
            }

            delete this.deltaMetadataCheckInfo[sObject];
        }
    }

    for (var sObject in this.deltaMetadataCheckInfo) {
        for (var i = 0; i < this.deltaMetadataCheckInfo[sObject].vlocityDataPackKey.length; i++) {
            this.jobInfo.deltaCheckResults[this.deltaMetadataCheckInfo[sObject].vlocityDataPackKey[i]] = { status: 'Ready' };   
        }
    }
};

/**
 * The method will check lookup references and determine the state of DataPack lookup references.
 */
        ...
        for (var sObjectType in this.querySObjectsInfo) {
            for (var whereClauseHash in this.querySObjectsInfo[sObjectType]) {
                ...
                if (!queryResult[sObjectType] || !queryResult[sObjectType][whereClauseHash]) {
                    for (var vlocityDataPackKey in vlocityDataPackKeys) {
                        this.jobInfo.deltaCheckResults[vlocityDataPackKey] = { status: 'Error' };   
                    }
 ...
 */
DeltaCheck.prototype.executeRunDeltaCheck = async function() {
    if (!this.vlocity.utilityservice.isEmptyObject(this.querySObjectsInfo)) {
        var queryResultsMap = {};
...
DeltaCheck.prototype.buildWhereClauseFromMap = async function(sObjectType, sObject) {
    ...
    return this.buildWhereClauseHash(Object.keys(sObject), sObject, fieldsDefinitionsMap, sObjectType);
};
...
DeltaCheck.prototype.processDataPack = async function(dataPackData, vlocityDataPackKey) {
    ...
    if (this.jobInfo.deltaMetadataCheck) {
        var deltaMetadata = this.vlocity.datapacksutils.getDeltaMetadataCheck(sObjectType);

        if (deltaMetadata) {
            var objectType = dataPackData[deltaMetadata.SObjectType];

            if (!this.deltaMetadataCheckInfo[objectType]) {
                var fieldName = dataPackData[deltaMetadata.FieldApiName];
                this.deltaMetadataCheckInfo[objectType] = { fieldName : fieldName, vlocityDataPackKey : []};
            }

            this.deltaMetadataCheckInfo[objectType].vlocityDataPackKey.push(vlocityDataPackKey);
            return;
        }
    }
...
DeltaCheck.prototype.buildQueryMap = async function(sObjects, queryChildren) {
    ...
    for (sObjectType in sObjects) {
        for (var i = 0; i < sObjects[sObjectType].length; i++) {
            ...
            whereClause = await this.buildWhereClauseFromMap(sObjectType, sObject);
