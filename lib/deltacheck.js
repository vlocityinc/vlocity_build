...
DeltaCheck.prototype.runDeltaCheck = async function(jobInfo, currentContextData) {
    ...
    await self.compareQueryResultWithDataPacks(queryResult, this.deltaCheckJobInfo.contextDataToCompareAgainst);
    await self.compareDataPacksWithQueryResult(this.deltaCheckJobInfo.contextDataToCompareAgainst, queryResult);
    return self.deltaCheckResult;
};

DeltaCheck.prototype.compareQueryResultWithDataPacks = async function(queryResult, dataPacks) {
    for (var sObjectType in queryResult) {
        for (var whereClauseHashKey in queryResult[sObjectType]) {
            if (!(dataPacks[sObjectType][whereClauseHashKey])) {
                ...
                if (!this.deltaCheckResult.new[vlocityDataPackKey]) {
                    this.deltaCheckResult.new[vlocityDataPackKey] = [];
                }

                this.deltaCheckResult.new[vlocityDataPackKey].push({[whereClauseHashKey] : dataPacks[sObjectType][whereClauseHashKey]});
            }
        }
    }
};

DeltaCheck.prototype.compareDataPacksWithQueryResult = async function(dataPacks, queryResult) {
    for (var sObjectType in dataPacks) {
        for (var whereClauseHashKey in dataPacks[sObjectType]) {
            if (!(queryResult[sObjectType][whereClauseHashKey])) {
                var vlocityDataPackKey = this.deltaCheckJobInfo.whereClauseHashToVlocityDataPackKey[whereClauseHashKey];

                if (!this.deltaCheckResult.deleted[vlocityDataPackKey]) {
                    this.deltaCheckResult.deleted[vlocityDataPackKey] = [];
                }

                this.deltaCheckResult.deleted[vlocityDataPackKey].push({[whereClauseHashKey] : queryResult[sObjectType][whereClauseHashKey]});
            } else {
                var firstSObject = dataPacks[sObjectType][whereClauseHashKey];
                var secondSObject = queryResult[sObjectType][whereClauseHashKey];
                await this.compareDataPackWithSObject(firstSObject, secondSObject, whereClauseHashKey);
            }
        }
    }
};

DeltaCheck.prototype.compareDataPackWithSObject = async function(dataPackData, sObject, whereClauseHashKey) {
    for (var fieldName in dataPackData) {
        ...
        if (sObject.hasOwnProperty(fieldName)) {
            ...
            if (dataPackValue !== sObject[fieldName]) {
                ...
                if (!this.deltaCheckResult.updated[vlocityDataPackKey]) {
                    this.deltaCheckResult.updated[vlocityDataPackKey] = [];
                }

                this.deltaCheckResult.updated[vlocityDataPackKey].push({[whereClauseHashKey] : { 
                    fieldName : [fieldName],
