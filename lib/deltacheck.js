    ...
    if (queryResultMap) {
        for (var sObjectType in queryResultMap) {
            for (var whereClauseHash in queryResultMap[sObjectType]) {
                ...
                this.deltaCheckJobInfo.matchingKeyValueByType[sObjectType][matchingKeyFieldValue] = sObject.Id;
                this.deltaCheckJobInfo.matchingKeyValueByType[sObjectType][sObject.Id] = matchingKeyFieldValue;

                if (this.deltaCheckJobInfo.childToParentLookupField[sObjectType]) {
                    var parentSObject = this.deltaCheckJobInfo.childToParentLookupField[sObjectType].parentSObject;
                    var parentRecordSObjectType = this.deltaCheckJobInfo.childToParentLookupField[sObjectType].sObjectType;
                    var parentRecordMatchingKeyValue = this.getMatchingKeyFieldValue(parentRecordSObjectType, parentSObject);
                    ...
                    if (!this.deltaCheckJobInfo.whereClauseHashToVlocityDataPackKey[parentRecordSObjectType]) {
                        this.deltaCheckJobInfo.whereClauseHashToVlocityDataPackKey[parentRecordSObjectType] = {};
                    }

                    this.deltaCheckJobInfo.whereClauseHashToVlocityDataPackKey[parentRecordSObjectType][whereClauseHash] = vlocityDataPackKey;
                }
            }
        }
    }
    ...
    if (sObject) {
        ...
        if (queryForChildren) {
            ...
            for (var childSObjectType in queryForChildren) {
                ...
                var queryRecord = {vlocityDataPackKey : vlocityDataPackKey};
                ...
                this.deltaCheckJobInfo.childToParentLookupField[childSObjectType] = {parentSObject : sObject, sObjectType : sObjectType};
