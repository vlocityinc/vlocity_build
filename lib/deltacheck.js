...
const DeltaCheck = module.exports = function (vlocity) {
...
DeltaCheck.prototype.compareQueryResultWithDataPacks = async function (jobInfo, queryResult, dataPacks) {
  for (let sObjectType in queryResult) {
    for (let whereClauseHashKey in queryResult[sObjectType]) {
      if (!(dataPacks[sObjectType][whereClauseHashKey])) {
        let vlocityDataPackKey = this.deltaCheckJobInfo.whereClauseHashToVlocityDataPackKey[whereClauseHashKey]
  ...
  for (let sObjectType in dataPacks) {
    for (let whereClauseHashKey in dataPacks[sObjectType]) {
      if (!queryResult[sObjectType] || !(queryResult[sObjectType][whereClauseHashKey])) {
        ...
        let firstSObject = dataPacks[sObjectType][whereClauseHashKey]
        let secondSObject = queryResult[sObjectType][whereClauseHashKey]
...
DeltaCheck.prototype.buildQueryMap = async function (sObjects, replaceMatchingKeys, compare) {
  if (sObjects && !this.vlocity.utilityservice.isEmptyObject(sObjects)) {
    ...
    for (let sObjectType in sObjects) {
...
DeltaCheck.prototype.executeQueries = async function (queriesMap) {
  queriesMap = await this.buildQueries(queriesMap)
  const queriedRecordsMap = {}
  const queryPromises = []

  for (let sObjectType in queriesMap) {
    for (let query in queriesMap[sObjectType]) {
...
DeltaCheck.prototype.buildUniqueKey = async function (sObjectType, vlocityDataPackKey, sObject, matchingKeyFieldValue) {
  ...
  if (matchingKeyField && matchingKeyField.includes(',')) {
    ...
    if (!this.vlocity.utilityservice.isEmptyObject(sObjectRecord)) {
      if (!this.deltaCheckJobInfo.generatedMatchingKeyValueToId[sObjectType]) {
