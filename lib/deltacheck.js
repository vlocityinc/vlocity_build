...
DeltaCheck.prototype.executeAutoFixPicklists = async function() {
    ...
    for (var sObjectType in this.autoFixPicklistsInfo.deployPicklistValues) {
        ...
        var globalValueSet = [];
        var updateCustomObject = false;
        var updateGlobalValueSet = false;
        ...
        for (var i = 0; i < customObject.fields.length; i++) {
            ...
            if (this.autoFixPicklistsInfo.deployPicklistValues[sObjectType].hasOwnProperty(fieldName)) {
                ...
                if (fieldDefinition.valueSet
                    ...
                    if (!fieldDefinition.fullName.includes('%vlocity_namespace%__')) { // Can not update Global Value Set picklists metadata in a managed environment 
                        updateGlobalValueSet = await this.updateGlobalValueSetDefinition(fieldDefinition.valueSet.valueSetName, dataPackPicklistValues, globalValueSet)
                    }
                ...
                for (var y = 0; y < dataPackPicklistValues.length; y++) {
                    var dataPackValue = dataPackPicklistValues[y];
                    var valueNotFound = true;

                    for (var z = 0; z < fieldValues.length; z++) {
                        if (fieldValues[z].fullName === dataPackValue) {
                            if (fieldValues[z].isActive === "true") {
                                continue;
                            } else if (fieldValues[z].isActive === "false") {
                                updateCustomObject = true;
                                fieldValues[z].isActive = true;
                            }
                    ...
                    if (valueNotFound) {
                        updateCustomObject = true;
                        fieldValues.push({fullName: dataPackPicklistValues[y], label: dataPackPicklistValues[y], default: false, isActive: true});
                    }
                }
            }
        }
        ...
        if (updateGlobalValueSet) {
            await this.vlocity.utilityservice.updateGlobalValueSet(globalValueSet);
        }
    }
};

DeltaCheck.prototype.updateGlobalValueSetDefinition = async function(valueSetName, dataPackPicklistValues, globalValueSet) {
    var updateGlobalValueSet = false;
    var retrievedPicklistGlobalValueSet = await this.vlocity.utilityservice.retrieveGlobalValueSet(valueSetName);

    if (retrievedPicklistGlobalValueSet) {
        if (!(retrievedPicklistGlobalValueSet.customValue instanceof Array)) {
            retrievedPicklistGlobalValueSet.customValue = [retrievedPicklistGlobalValueSet.customValue];
        }

        for (var y = 0; y < dataPackPicklistValues.length; y++) {
            var valueNotFound = true;

            for (var i = 0; i < retrievedPicklistGlobalValueSet.customValue.length; i++) {
                var orgValue = retrievedPicklistGlobalValueSet.customValue[i];

                if (orgValue.fullName === dataPackPicklistValues[y]) {
                    if (orgValue.isActive === 'true') {
                        continue;
                    } else if (orgValue.isActive === 'false'){
                        orgValue.isActive = true;
                        updateGlobalValueSet = true;
                    }
            ...
            if (valueNotFound) {
                updateGlobalValueSet = true;
                retrievedPicklistGlobalValueSet.customValue.push({default: false, fullName: dataPackPicklistValues[y], label: dataPackPicklistValues[y]});
            }
        }

        globalValueSet.push(retrievedPicklistGlobalValueSet);
    }

    return updateGlobalValueSet;
};
...
DeltaCheck.prototype.processDataPack = async function(dataPackData, vlocityDataPackKey) {
    ...
    if (this.jobInfo.AutoFixPicklists) {
        ...
        for (var fieldName in restrictedPicklistValuesMap) {
            var unmanagedOrgFieldWithNamespace = fieldName;

            if (fieldName.includes('__c') && !fieldName.includes('%vlocity_namespace%__')) {
                unmanagedOrgFieldWithNamespace = '%vlocity_namespace%__' + unmanagedOrgFieldWithNamespace;
            }

            if (!dataPackData[fieldName] && !dataPackData[unmanagedOrgFieldWithNamespace]) {
                continue;
            }

            var dataPackFieldValue = dataPackData[fieldName] ? dataPackData[fieldName] : dataPackData[unmanagedOrgFieldWithNamespace];
            var orgFieldDefinitions = restrictedPicklistValuesMap[fieldName] ? restrictedPicklistValuesMap[fieldName] : restrictedPicklistValuesMap[unmanagedOrgFieldWithNamespace];
            var orgFieldValues = [];
            
            for (var fieldDefinition of orgFieldDefinitions) {
                orgFieldValues.push(fieldDefinition.value);
            }

            if (dataPackFieldValue.includes(';')) {
                dataPackFieldValue = dataPackFieldValue.split(';');
            } else {
                dataPackFieldValue = [dataPackFieldValue];
            }

            var picklistValuesNotFound = [];

            for (var i = 0; i < dataPackFieldValue.length; i++) {
                if (!orgFieldValues.includes(dataPackFieldValue[i])) {
                    picklistValuesNotFound.push(dataPackFieldValue[i]);
                }
            }

            if (!this.vlocity.utilityservice.isEmptyObject(picklistValuesNotFound)) {
                ...
                this.autoFixPicklistsInfo.deployPicklistValues[sObjectType][fieldName] = picklistValuesNotFound;   
            }
        }
    ...
    if (this.jobInfo.CheckStaleObjects) {
        ...
        if (vlocityDataPackType === 'SObject') {
            ...
            for (var i = 0; i < matchingKeys.length; i++) {
                if (!dataPackData.hasOwnProperty(matchingKeys[i])) {
