                ...
                if (!(stat == 'New' && sourceObject[field] == "")) {
                    ...
                    if (typeof targetObject[field] == 'string' 
                        && typeof sourceObject[field] == 'string') {
                            var diff = "diff --git a/a/a.html a/a/a.html\n" + gitDiff(targetObject[field], sourceObject[field]);

                            fs.outputFileSync('seediffs.html', diff2html.getPrettyHtml(diff));
        
                            fieldDiffs.push({
                                gitDiff: diff,
                                htmlDiff: diff2html.getPrettyHtml(diff),
                                jsonDiff:  diff2html.getJsonFromDiff(diff),
