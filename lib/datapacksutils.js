        ...
        if (jobInfo.diffType[dataPackKey] && jobInfo.diffType[dataPackKey] != 'Unchanged') {
            if (sourceDataPack.SFDXData) {
                ...
                for (var uniqueKey in sourceDiffKeys) {
                    ...
                    var obj = { editable: sourceDiffKeys[uniqueKey]};

                    if (sourceDiffKeys[uniqueKey] && targetDiffKeys[uniqueKey]) {
                        foundTarget.add(targetDiffKeys[uniqueKey].VlocityRecordSourceKey);

                        obj.diffType = jobInfo.diffType[dataPackKey] ? jobInfo.diffType[dataPackKey] : 'New';
                ...
                for (var obj of allTargetSObjects) {
                    if (!foundTarget.has(obj.VlocityRecordSourceKey)) {
                        if (this.isDeletedDuringDeploy(sourceDataPack.VlcoityDataPackType, obj.VlocityRecordSObjectType)) {
        ...
        && stringify(sourceObject) != stringify(targetObject)) {
            ...
            if (sourceObjParsed != null 
                ...
                if (Array.isArray(sourceObjParsed) && Array.isArray(targetObjParsed)) {
                    var stringifiedSource = stringify(sourceObjParsed, {space: 4}); 
                    var stringifiedTarget = stringify(targetObjParsed, {space: 4}); 
...
DataPacksUtils.prototype.getDiffsForSfdx = function(jobInfo, diffType, sourceSfdxObject, targetSfdxObject) {
    ...
    if (typeof data == 'string') {   
        type = fileType(new Buffer(data));
    }

    //var validExtensions = [ '.xml', '.html', '.js' ];

    //if (validExtensions.includes(path.extname(sourceSfdxObject.VlocityDataPackKey))) {
...
DataPacksUtils.prototype.getGitDiffs = function(target, source, type) {
    if (type != null) {
        if (target && typeof target == 'string' 
            ...
            let diffString = gitDiff(target, source, { fakediff: false, flags: '--patience' });
