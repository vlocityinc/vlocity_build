...
DataPacksUtils.prototype.getAdditionalSFMetadataDependencies = async function(jobInfo){
...
DataPacksUtils.prototype.retrieveSalesforce = async function(jobInfo, listOnly) {
    ...
    if (jobInfo.specificManifestKeys) {
        ...
        if(jobInfo.maxDepth !== 0){
            let additionalSFDependencies = await this.getAdditionalSFMetadataDependencies(jobInfo);
