...
DataPacksUtils.prototype.getAllParentChildKeys = async function(dataPacks, jobInfo) {
    ...
    for (var dataPack of dataPacks) {
        ...
        for (let datapack of dataPacks) {
            if (datapack.VlocityDataPackParents.indexOf(dataPack.VlocityDataPackKey) !== -1
                && datapack.VlocityDataPackKey !== dataPack.VlocityDataPackKey
                && newChildReferences.indexOf(datapack.VlocityDataPackKey) === -1) {
                newChildReferences.push(datapack.VlocityDataPackKey);
            }
        }

        newParents.sort();
        newChildReferences.sort();
        dataPack.VlocityDataPackParents = newParents;
        dataPack.VlocityDataPackChildren = newChildReferences;
    }
}
