...
DataPacksUtils.prototype.getFieldLabel = function(sObject, fieldApiName) {
    var self = this;
...
DataPacksUtils.prototype.getFieldDiffsIndividual = function(allSourceDataPacks, allTargetDataPacks, jobInfo, dataPackKey) {
    try {
        ...
        if (sourceDataPack.hasOwnProperty('SFDXData')) {
            ...
            if (dataPackStatus == 'Changed') {
                ...
                for (var uniqueKey in sourceDiffKeys) {
                    ...
                    obj.fieldDiffs = this.getFieldDiffsForSObjects(sourceDataPack.VlocityDataPackType, sourceDiffKeys[uniqueKey], targetDiffKeys[uniqueKey] || {});
                ...
                for (var obj of allTargetSObjects) {
                    ...
                    if (!foundTarget.has(obj.VlocityRecordSourceKey)) {
                        ...
                        deletedObj.fieldDiffs = this.getFieldDiffsForSObjects(sourceDataPack.VlocityDataPackType, {}, obj, true);
            ...
            this.getTitleFieldsForDataPack(sourceDataPack, targetDataPack, allFieldDiffsByRecordSourceKey[0]);
            ...
            var titleObjects = this.getTitleObjectsForDataPack(sourceDataPack, targetDataPack);
...
DataPacksUtils.prototype.getFieldDiffsForSObjects = function(dataPackType, sourceObject, targetObject, isDeleted) {
    var self = this;
    var fieldDiffs = [];

    var unhashableFields = this.getUnhashableFields(dataPackType, sourceObject.VlocityRecordSObjectType) || [];

    this.handleDataPackEvent('hashSObjectData', dataPackType, { sobject: sourceObject });
    this.handleDataPackEvent('hashSObjectData', dataPackType, { sobject: targetObject });
    ...
    for (var i = 0; i < allFields.length; i++) {
        ...
        && stringify(sourceObject) != stringify(targetObject)) {
            ...
            let fieldLabel = this.getFieldLabel(sObjectType, field);
            ...
            if ((sourceObjParsed != null && typeof sourceObjParsed == 'object') 
                ...
                if (Array.isArray(sourceObjParsed) || Array.isArray(targetObjParsed)) {
                    ...
                    self.setNestedFieldDiffs(sourceObjParsed || {}, targetObjParsed || {}, field, fieldDiffs, recordSourceKey,  sObjectType);
                }
...
DataPacksUtils.prototype.setNestedFieldDiffs = function(sourceObject, targetObject, currentPath, fieldDiffs,  vlocityRecordSourceKey, sObjectType) {
    var self = this;
    ...
    for (var field in sourceObject) {
        ...
        let fieldLabel = self.getFieldLabel(sObjectType, fieldPath);
        ...
        if (sourceObject[field] != targetObject[field]
            ...
            && !Array.isArray(sourceObject[field])
            && fieldType !== 'object') {
                self.setNestedFieldDiffs(sourceObject[field], targetObject[field] || {}, `${currentPath}.${field}`, fieldDiffs, vlocityRecordSourceKey, sObjectType);
...
DataPacksUtils.prototype.handleDataPackEvent = function(eventName, dataPackType, input) {
    if (!this[dataPackType] && fs.existsSync(path.join(__dirname, 'datapacktypes', dataPackType.toLowerCase() + '.js'))) {
        let requiredDataPack = require(path.join(__dirname, 'datapacktypes', dataPackType.toLowerCase() + '.js'));
        this[dataPackType] = new requiredDataPack(this.vlocity);
    }
    ...
    if (profiles.length > 0 || permissionSets.length > 0) {
        ...
        for (var profile of profiles) {
            try {
                ...
                for (var profileNode of profileNodes) {
                    currentProfile.Profile[profileNode.type] = this.removeInvalidProfileElements(adminProfile[0][profileNode.type], currentProfile.Profile[profileNode.type], profileNode.key, profileNode.type);
                }
                ...
                currentProfile.Profile.tabVisibilities = this.filterInvalidTabs(currentProfile.Profile.tabVisibilities, profileInOrg[0], allCustomObjMetadata);
        ...
        for (var permissionSet of permissionSets) {
            ...
            for (var permissionSetNode of profileNodes) {
                currentPermissionSet.PermissionSet[permissionSetNode.type] = this.removeInvalidProfileElements(adminProfile[0][permissionSetNode.type], currentPermissionSet.PermissionSet[permissionSetNode.type], permissionSetNode.key, permissionSetNode.type);
...
DataPacksUtils.prototype.addPackageDirToSfdxProjectJson = function(packageDir) {
    ...
    if(!fs.existsSync(sfdxProjectJsonPath)){
        ...
        sfdxProjectJson = JSON.parse(fs.readFileSync(sfdxProjectJsonPath, {encoding: 'utf-8'}));
        if(!sfdxProjectJson.packageDirectories.find((entry) => entry.path === packageDir)){
            sfdxProjectJson.packageDirectories.push({
                "path": packageDir,
                "default": false
            });
        }
...
DataPacksUtils.prototype.deploySalesforce = async function(jobInfo) {
    ...
    if (deploymentOptions.sourcepath) {
        ...
        try {
            ...
            jobInfo.currentStatus = Object.assign(jobInfo.currentStatus, this.formatSalesforceDeployResponseFromCLI(response));
...
DataPacksUtils.prototype.formatSalesforceDeployResponse = function (response) {
    VlocityUtils.report("Source deploy response is ",response);
...
DataPacksUtils.prototype.formatSalesforceDeployResponseFromCLI = function (response) {
    VlocityUtils.report("Source deploy response is ",response);
    let currentStatusObj = {};
    response = response?.details?.componentSuccesses;
    if (!response || !Array.isArray(response)) return currentStatusObj;
    for ( component of response) {
        var metadataKey = component.fileName;
        currentStatusObj[metadataKey] ='Success';
    }
    return currentStatusObj;
}   
