...
DataPacksUtils.prototype.checkAllowedKey = function(jobInfo, dataPackKey, dataPackTypeFolder) {
    ...
    if (dataPackTypeFolder == 'objects') {
        ...
        } else if (!jobInfo.allAllowedTypes.Salesforce.CustomObject) {
            return false;
        }
    } else {
     ...
     //  VlocityUtils.report('Dir', dir);
        for (let file of this.getFiles(path.join(sfdxProject.sfdxPath, sfdxFolder, dir))) {
            ...
            if (file[0] != '.') {
                ...
                var dataPackTypeFolder = dataPackType;

                dataPackType = dataPackType.replace(new RegExp(path.sep, 'g'), '/');

                if (dataPackType.indexOf('/') != -1) {
                    dataPackTypeFolder = dataPackType.split('/')[0];
                }

                try {
                    var dataPackKey = `${dataPackType}/${file}`;

                    if (folderOnlyMetadata.includes(dataPackTypeFolder)) {
                        var splitKey = dataPackType.split('/');
                        dataPackKey = `${splitKey[0]}/${splitKey[1]}`;
                    }

                    if (checkAllowedTypes && !this.checkAllowedKey(jobInfo, dataPackKey, dataPackTypeFolder)) {
                        continue;
                    }
