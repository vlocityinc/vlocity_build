...
DataPacksUtils.prototype.deploySalesforce = async function(jobInfo) {
    ...
    if (deploymentOptions.sourcepath) {
        let leadingPath = path.join('main', 'default'),
            tempSfdxFolder = sfdxFolder.replace(/DX\//g, 'DX/tempDeploy/');
        ...
        try {
            ...
            await this.prepareProfilesForDeploy(deploymentOptions.sourcepath.replace(/DX\//g, 'DX/tempDeploy/'));

            if (fs.existsSync(tempSfdxFolder)) {
                deploymentOptions = Object.assign({}, {
                    sourcepath: deploymentOptions.sourcepath.replace(/DX\//g, 'DX/tempDeploy/'),
                    targetusername: deploymentOptions.targetusername
                });
            ...
            if (fs.existsSync(tempSfdxFolder)) {
                ...
                await fs.removeSync(tempSfdxFolder.split('salesforce_sfdx')[0]);
                deploymentOptions.sourcepath = deploymentOptions.sourcepath.replace('DX/tempDeploy/', 'DX/');
            }
