        ...
        Object.keys(obj[newDataField][0]).forEach(key => {
            ...
            if (Object.keys(jobInfo.platformObjectMappings[dataPackType]).includes(key)) {
                ...
                obj[newDataField][0][key].forEach(subObj => {
                    ...
                    Object.keys(subObj).forEach(fieldName => {
                        if (subMappings.fieldNames[fieldName]) {
                            if(subObj[fieldName][parentElementKeyName]){
                                subObj[fieldName][subMappings.fieldNames[parentElementKeyName]] = JSON.parse((JSON.stringify(subObj[fieldName][parentElementKeyName]).split(key).join(childObjDataField)).split(dataField).join(newDataField));
                                delete subObj[fieldName][parentElementKeyName];
                            }
                            if (subObj[fieldName].VlocityMatchingRecordSourceKey) {
                                subObj[fieldName].VlocityMatchingRecordSourceKey = subObj[fieldName].VlocityMatchingRecordSourceKey.split(key).join(childObjDataField).split(dataField).join(newDataField);
                    ...
                    level2Obj.VlocityRecordSourceKey = level2Obj.VlocityRecordSourceKey.split(key).join(childObjDataField).split(dataField).join(newDataField);
