...
DataPacksUtils.prototype.getFieldDiffs = async function(allSourceDataPacks, allTargetDataPacks, jobInfo) {
    ...
    var allKeys = Object.keys(allTargetDataPacks);
    ...
    for (var dataPackKey of allKeys) {
        ...
        if (jobInfo.diffType[dataPackKey] && jobInfo.diffType[dataPackKey] != 'Unchanged') {
            ...
            for (var obj of allFieldDiffsByRecordSourceKey) {
                ...
                var labelFields = self.getRecordLabel(null, obj.editable.VlocityRecordSObjectType);

                var label = self.objectLabels[obj.editable.VlocityRecordSObjectType];
            ...
            if (typeof sourceObject[field] == 'object') {
                ...
                if (!(stat == 'New' && sourceObject[field] == "")) {
                    ...
                    if (typeof targetObject[field] == 'string' 
                        && typeof sourceObject[field] == 'string') {
                            var diff = "diff --git a/a/a.html a/a/a.html\n@@ -0,0 +0,0 @@\n" + gitDiff(targetObject[field], sourceObject[field], { forceFake: true });

                            fs.outputFileSync('seediffs.html', diff2html.getPrettyHtml(diff, {showFiles:false}));
        
                            fieldDiffs.push({
                                gitDiff: diff,
                                htmlDiffLines: diff2html.getPrettyHtml(diff, { outputFormat: "line-by-line" }),
                                htmlDiffSideBySide: diff2html.getPrettyHtml(diff, { outputFormat: "side-by-side" }),
...
DataPacksUtils.prototype.getNestedDiffFieldType = function(sObjectType, fieldPath) {
    var diffFields = this.getExpandedDefinition(null, sObjectType, "DiffFields") || {};
...
DataPacksUtils.prototype.setNestedFieldDiffs = async function(sourceObject, targetObject, currentPath, fieldDiffs,  vlocityRecordSourceKey, sObjectType) {
    ...
    for (var field in sourceObject) {
        ...
        var fieldType = this.getNestedDiffFieldType(sObjectType, fieldPath);
        ...
        && stringify(sourceObject[field]) != stringify(targetObject[field])) {
            fieldDiffs.push({
                status: 'Changed',
                value: typeof sourceObject[field] == 'object' ? stringify(sourceObject[field]) : sourceObject[field],
                old:  typeof targetObject[field] == 'object' ? stringify(targetObject[field]) : targetObject[field],
                field: fieldPath,
