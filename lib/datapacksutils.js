...
var DataPacksUtils = module.exports = function(vlocity) {
    ...
    const ignoreFilePath = path.resolve(process.cwd(), '.vlocityignore');
...
DataPacksUtils.prototype.onEvent = async function(event, dataPack, jobInfo) {
    ...
    if (eventHandler) {
        await this.vlocity.datapacksutils.runJavaScript(jobInfo.projectPath, eventHandler, dataPack, jobInfo);
    }
}
        ...
        if (jobInfo.diffType[dataPackKey]) {
            if (sourceDataPack.SFDXData) {
                ...
                if (dataPackStatus == 'Changed') {
                    ...
                    if (dataPackKey == 'Product2/f50aa30e-8feb-b195-cde8-e810d4010d2f') {
                       // debugger;
                    }
...
DataPacksUtils.prototype.getTitleFieldsForDataPack = async function(sourceDataPack, targetDataPack, diffObject) {
    ...
    for (var i = titleFields.length-1; i >= 0; i--) {
        ...
        if (fieldDiff.fieldType == 'side-by-side' || fieldDiff.fieldType == 'line-by-line') {
            gitDiffForTitle = this.getGitDiffs(sourceValue, targetValue || sourceValue, { ext: 'json' }, true);
                ...
                if (Array.isArray(sourceObjParsed) || Array.isArray(targetObjParsed)) {
                    ...
                    if (stringifiedSource != stringifiedTarget) {
                        fieldDiffs.push({
                            ...
                            gitDiff: this.getGitDiffs(stringifiedTarget, stringifiedSource, { ext: 'json' }),
                            status: 'Changed',
...
DataPacksUtils.prototype.getDiffsForSfdx = function(jobInfo, diffType, sourceSfdxObject, targetSfdxObject) {
    ...
    var data = targetSfdxObject ? targetSfdxObject.SFDXData : sourceSfdxObject.SFDXData;
    var type;

    if (typeof data == 'string') {   
        type = fileType(Buffer.from(data));
    }

    var validExtensions = [ '.cls', '.js' ];
    var ext = path.extname(sourceSfdxObject.VlocityDataPackKey);

    if (validExtensions.includes(ext)) {
        type = { ext: ext.substring(1) };
    }

    if (type) {
        VlocityUtils.report('Diffing...', sourceSfdxObject.VlocityDataPackKey);
        thisDiff = this.getGitDiffs(targetSfdxObject.SFDXData, sourceSfdxObject.SFDXData, type);
    ...
    jobInfo.sObjectsInfo[sourceSfdxObject.VlocityDataPackKey] = sfdxDiff;
};
    ...
    for (var field in sourceObject) {
        ...
        if (sourceObject[field] != targetObject[field]
            ...
            && fieldType !== 'object') {
                ...
                if (sourceObjectValue != targetObjectValue) {
                    fieldDiffs.push({
                        //fieldImportance: this.getFieldImportance(dataPackType, sObjectType, field),
                        fieldType: 'json' ,
                        gitDiff: this.getGitDiffs(targetObjectValue, sourceObjectValue, { ext: 'json' }),
                        status: 'Changed',
                        value: sourceObject[field],
                        old: targetObject[field],
                        field: field,
            ...
            && sourceObjectValue != targetObjectValue) {
                ...
                var data = sourceObjectValue ? sourceObjectValue : targetObjectValue;
                var type;
                ...
                // fieldType === object is used to specify jsons that are diffed as a single fielDiff
                if (fieldType === 'object') {
                    type = 'json';
                }
                
                fieldDiffs.push({
                    gitDiff: type ? this.getGitDiffs(targetObjectValue, sourceObjectValue, type) : null,
                    status: 'Changed',
                    value: sourceObjectValue,
...
DataPacksUtils.prototype.getNoDiffs = function(data, type) {
    ...
    return `diff --git File.${type.ext} File.${type.ext}\n@@ -1,0 +1,0 @@${formattedData}`;
}
        ...
        while (batchStartIndex < dataLength) {
            ...
            batch.execute(data.splice(0, thisBatchSize))
                .on("error", function (batchInfo) {
                    VlocityUtils.error('Error, batchInfo:', batchInfo);
                })
                .on("queue", function (batchInfo) {
                    VlocityUtils.log(operation + ' batch queued')
                ...
                .on("response", function (rets) {
                    for (var i = 0; i < rets.length; i++) {
                        if (rets[i].success) {
                            //VlocityUtils.log("#" + (i + 1) + " " + operation + "ed successfully, id = " + rets[i].id);
                        } else {
                            VlocityUtils.log("# " + (i + 1) + " error occurred, message = " + rets[i].errors.join(', '));
                        }
                    }
                    batchCnt++;
                    if (batchCnt == numBatches) {
                        VlocityUtils.log(operation + ' job finished')
                        job.close();
                        resolve();
                    }
                });
