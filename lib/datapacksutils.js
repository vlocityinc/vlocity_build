...
DataPacksUtils.prototype.getFieldLabel = async function(sObject, fieldApiName) {
    ...
    if (!self.fieldLabels[sObject]) {
        ...
        return await self.vlocity.jsForceConnection.sobject(sObject.replace('%vlocity_namespace%', self.vlocity.namespace)).describe(function(err, meta) {
            ...
            if (meta) {
                ...
                return self.generateFieldLabel(sObject, fieldApiName); 
            } else {
                return null; 
            }
        });
    } else {
        return await self.generateFieldLabel(sObject, fieldApiName);
    }
}
...
DataPacksUtils.prototype.logActionWithContext = async function(componentType, componentName, actionName, context) {
    var self = this;
    var stringifiedContext = {};
    for (const key in context) {
        if (typeof context[key] === 'object' && context[key] !== null) {
            stringifiedContext[key] = JSON.stringify(context[key]);
        } else {
            stringifiedContext[key] = context[key];
        }
    }
    var payload = {
        sClassName: this.vlocity.namespace + '.BusinessProcessDisplayController.BusinessProcessDisplayControllerOpen',
        sMethodName: 'logActionWithContext',
        input: JSON.stringify({
    ...
    VlocityUtils.verbose('Logging Action With Context', componentType, componentName, actionName, payload);
    try {
        return await this.vlocity.datapacks.makeApexPostRequest('/' + this.vlocity.namespace + '/v1/invoke/', payload);
    } catch (err) {
        VlocityUtils.error('Error logging action with context', err.message ? err.message : '');
    }
}
...
DataPacksUtils.prototype.getCustomObjectInfo = async function(objectInput) {
    ...
    while (tries < 3) {
        try {
            ...
            let read = (await this.vlocity.jsForceConnection.metadata.read('CustomObject', [ customObject.fullName ]))[0];
...
DataPacksUtils.prototype.deploySalesforce = async function(jobInfo) {
    ...
    if (deploymentOptions.sourcepath) {
        ...
        VlocityUtils.log('Calling sf project deploy start');
