...
DataPacksUtils.prototype.getFieldDiffs = function(sourceDataPack, targetDataPack, jobInfo) {
    var self = this;
    var allFieldDiffsByRecordSourceKey = [];

    var dataField = self.getDataField(sourceDataPack);

    var allSourceSObjects = self.getAllIndividualSObjectsInDataPack(sourceDataPack.VlocityDataPackData[dataField][0]);

    var allTargetSObjects = self.getAllIndividualSObjectsInDataPack(targetDataPack.VlocityDataPackData[dataField][0]);

    // Target are most important 
    var targetDiffKeys = self.getAllByUniqueKey(sourceDataPack.VlocityDataPackType, Object.values(allTargetSObjects));

    // For Target need to keep track of Unique
    var sourceDiffKeys = self.getAllByUniqueKey(sourceDataPack.VlocityDataPackType,Object.values(allSourceSObjects), targetDiffKeys);

    var allAddedSourceKeys = new Set();

    for (var uniqueKey in sourceDiffKeys) {
        if (sourceDiffKeys[uniqueKey] && targetDiffKeys[uniqueKey]) {
            
            self.setFieldDiffsForSObjects(sourceDiffKeys[uniqueKey], targetDiffKeys[uniqueKey]);

            allAddedSourceKeys.add(sourceDiffKeys[uniqueKey].VlocityRecordSourceKey);
            allAddedSourceKeys.add(targetDiffKeys[uniqueKey].VlocityRecordSourceKey);
        }
    }

    allSourceSObjects.forEach(function(obj) {
        if (!allAddedSourceKeys.has(obj.VlocityRecordSourceKey)) {
            obj.diffStatus = 'New';
    ...
    allTargetSObjects.forEach(function(obj) {
        if (!allAddedSourceKeys.has(obj.VlocityRecordSourceKey)) {
            ...
            if (self.isDeletedDuringDeploy(sourceDataPack.VlcoityDataPackType, obj.VlocityRecordSObjectType)) {
                obj.diffStatus = 'Deleted';

                allFieldDiffsByRecordSourceKey.push(obj);
            }
        }

        if (obj.diffStatus == 'Changed') {
            allFieldDiffsByRecordSourceKey.push(obj);
        }
    });

    jobInfo.diffFields[targetDataPack.VlocityDataPackKey] = allFieldDiffsByRecordSourceKey;
};

var ignoredFields = new Set([ 'VlocityRecordSourceKey', 'CurrencyIsoCode' ]);

DataPacksUtils.prototype.setFieldDiffsForSObjects = function(sourceObject, targetObject) {
    var self = this;
    var fieldDiffs = [];
    ...
    for (var field in sourceObject) {
        ...
        && sourceObject[field] != targetObject[field]) {
            try {
                var sourceObjParsed = JSON.parse(sourceObject[field]);
                var targetObjParsed = JSON.parse(targetObject[field]);

                if (typeof sourceObjParsed == 'object' && typeof targetObjParsed == 'object') {
                    self.setNestedFieldDiffs(sourceObjParsed, targetObjParsed, field, fieldDiffs, targetObject.VlocityRecordSourceKey);
                    continue;
                }
               
            } catch (e) {
                // Should be ignorable
                //VlocityUtils.verbose(e);
            }
            ...
            if (typeof sourceObject[field] == 'object') {
                ...
                var targetMatchingKey = sourceObject[field].VlocityMatchingRecordSourceKey ? sourceObject[field].VlocityMatchingRecordSourceKey : sourceObject[field].VlocityLookupRecordSourceKey;
                 
                if (sourceMatchingKey != targetMatchingKey) {
                    fieldDiffs.push({
                        new: sourceMatchingKey,
                        old: targetMatchingKey,
                        field: field,
                        label: field,
                        VlocityRecordSourceKey: targetObject.VlocityRecordSourceKey
                    });
                }
            } else {
                fieldDiffs.push({
                    new: sourceObject[field],
    ...
    if (fieldDiffs.length > 0) {
        targetObject.diffStatus = 'Changed';
        targetObject.fieldDiffs = fieldDiffs;
    }
}

DataPacksUtils.prototype.setNestedFieldDiffs = function(sourceObject, targetObject, currentPath, fieldDiffs,  targetVlocityRecordSourceKey) {
    var self = this;

    for (var field in sourceObject) {
        if (typeof sourceObject[field] == 'object' 
        && typeof targetObject[field] == 'object' 
        && typeof targetObject[field] != null 
        && !Array.isArray(sourceObject[field])) {
            self.setNestedFieldDiffs(sourceObject[field], targetObject[field], `${currentPath}.${field}`, fieldDiffs, targetVlocityRecordSourceKey);
        } else if (sourceObject[field] != targetObject[field] && stringify(sourceObject[field]) != stringify(targetObject[field])) {
            fieldDiffs.push({
                new: sourceObject[field],
                old: targetObject[field],
                field: `${currentPath}.${field}`,
                label: `${currentPath}.${field}`,
                VlocityRecordSourceKey: targetVlocityRecordSourceKey
            });
        }
    }
}
...
DataPacksUtils.prototype.getAllByUniqueKey = function(dataPackType, sObjects, validKeys) {
    ...
    sObjects.forEach(function(sObject) {
        ...
        // If startswith AutoKey -> generate based on SObject
        ...
        if (diffKeys) {
            ...
            while (diffKeys[i]) {
                ...
                diffKeys[i].forEach(function(field) {
                    if (sObject[field] ==  null || sObject[field] == "") {
                        diffKey = null;
                        return;
                    }

                    diffKey += sObject[field] + '|';
                });
