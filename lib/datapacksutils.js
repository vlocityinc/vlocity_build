...
DataPacksUtils.prototype.getAllParentChildKeys = async function(dataPacks, jobInfo) {
    ...
    for (var dataPack of dataPacks) {
        ...
        newChildReferences[dataPack.VlocityDataPackKey] = newParents;
    }

    for (var dataPack of dataPacks) {
        dataPack.VlocityDataPackChildren = [];
        for (var key of Object.keys(newChildReferences)) {
            if (newChildReferences[key].indexOf(dataPack.VlocityDataPackKey) !== -1
                && key !== dataPack.VlocityDataPackKey
                && dataPack.VlocityDataPackChildren.indexOf(key) === -1) {
                dataPack.VlocityDataPackChildren.push(key);
            }
        }
        dataPack.VlocityDataPackChildren.sort();
    }
}
