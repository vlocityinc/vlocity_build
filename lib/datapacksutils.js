    ...
    if (currentArray) {
        for (var item of currentArray) {
            if (adminMap[item[keyToCompare][0]] &&
                (keyToCompare !== 'tab' || (keyToCompare === 'tab' && item.visibility[0] !== 'Hidden'))
            ) {
...
DataPacksUtils.prototype.deploySalesforce = async function(jobInfo) {
    ...
    if (deploymentOptions.sourcepath) {
        let leadingPath = path.join('main', 'default'),
            tempSfdxFolder = path.join(jobInfo.tempFolder, 'tempDeploy', 'salesforce_sfdx');
            profilesFolder = path.join(sfdxFolder, leadingPath, 'profiles'),
            tempProfilesFolder = path.join(tempSfdxFolder, leadingPath, 'profiles'),
            permissionSetsFolder = path.join(sfdxFolder, leadingPath, 'permissionsets'),
            tempPermissionSetsFolder =  path.join(tempSfdxFolder, leadingPath, 'permissionsets')
        ...
        try {
            ...
            let sfdxFolderRegex = new RegExp(sfdxFolder, "g");
            await this.prepareProfilesForDeploy(deploymentOptions.sourcepath.replace(sfdxFolderRegex, tempSfdxFolder));

            if (fs.existsSync(tempSfdxFolder)) {
                deploymentOptions = Object.assign({}, {
                    sourcepath: deploymentOptions.sourcepath.replace(sfdxFolderRegex, tempSfdxFolder),
                    targetusername: deploymentOptions.targetusername
                });
            ...
            if (fs.existsSync(tempSfdxFolder)) {
                ...
                let tempSfdxFolderRegex = new RegExp(tempSfdxFolder, "g");
                deploymentOptions.sourcepath = deploymentOptions.sourcepath.replace(tempSfdxFolderRegex, sfdxFolder);
            }
