...
DataPacksUtils.prototype.getListFileNameNoUnderscore = function(dataPackType, SObjectType) {
  
    var listFileNameKeys = this.getListFileName(dataPackType, SObjectType);

    return listFileNameKeys[0].substr(1);
}
...
DataPacksUtils.prototype.getDiffParentField = function(dataPackType, SObjectType) {
    return this.getExpandedDefinition(dataPackType, SObjectType, "DiffParentField");
}

DataPacksUtils.prototype.getDiffDisplayParentKey = function(dataPackType, sObject) {

    var parentField = this.getExpandedDefinition(dataPackType, sObject.VlocityRecordSObjectType, "DiffDisplayParentField");

    if (parentField && sObject[parentField]) {
        if (sObject[parentField].VlocityMatchingRecordSourceKey) {
            return sObject[parentField].VlocityMatchingRecordSourceKey;
        } else if (sObject[parentField].VlocityLookupRecordSourceKey) {
            return sObject[parentField].VlocityLookupRecordSourceKey;
        }
    }
...
DataPacksUtils.prototype.getObjectLabel = async function(sObject) {
    var self = this;
    ...
    await this.getFieldLabel(sObject, 'Name');

    return this.objectLabels[sObject];
}
    ...
    try {
        ...
        if (sourceDataPack.hasOwnProperty('SFDXData')) {
            ...
            for (var uniqueKey in sourceDiffKeys) {
                ...
                obj.VlocityDiffParentKey = diffParentKey;
                obj.VlocityDiffParentField = diffParentField;
                obj.VlocityDiffDisplayParentKey = this.getDiffDisplayParentKey(dataPackType, obj.editable);
                obj.VlocityFolderDisplayName = this.getListFileNameNoUnderscore(dataPackType, obj.editable.VlocityRecordSObjectType); 

                var fieldDiffResult = await this.getFieldDiffsForSObjects(dataPackKey, sourceDataPack.VlocityDataPackType, sourceDiffKeys[uniqueKey], targetDiffKeys[uniqueKey] || {});

                obj.fieldDiffs = fieldDiffResult.fieldDiffs;

                obj.diffType = "Unchanged";

                if (!targetDiffKeys[uniqueKey]) {
                    obj.diffType = "New";
                } else {
                    for (var fieldDiffSingle of fieldDiffResult.fieldDiffs) {
                        if (fieldDiffSingle.value != fieldDiffSingle.old) {
                ...
                for (var separatedChild of fieldDiffResult.childObjects) {
                    allFieldDiffsByRecordSourceKey.push(separatedChild);
                }
            }
            ...
            for (var obj of allTargetSObjects) {
                ...
                var editableObj = this.deserializeAll(JSON.parse(JSON.stringify(obj)));
                ...
                if (!foundTarget.has(obj.VlocityRecordSourceKey)) {
                    let deletedObj = {
                        ...
                        VlocityDiffDisplayParentKey: this.getDiffDisplayParentKey(dataPackType, editableObj),
                        VlocityFolderDisplayName: this.getListFileNameNoUnderscore(dataPackType, editableObj.VlocityRecordSObjectType)
                    };

                    var fieldDiffResult = await this.getFieldDiffsForSObjects(dataPackKey, sourceDataPack.VlocityDataPackType, {}, obj, true);

                    deletedObj.fieldDiffs = fieldDiffResult.fieldDiffs;
            ...
            for (var obj of allFieldDiffsByRecordSourceKey) {
                ...
                if (!obj.VlocitySObjectRecordLabel && obj.editable) {
                    var labelFields = this.getRecordLabel(null, obj.editable.VlocityRecordSObjectType);

                    var label = '';
                    //await this.getObjectLabel(obj.editable.VlocityRecordSObjectType);
            ...
            for (var change of allFieldDiffsByRecordSourceKey) {
                ...
                if (change.fieldDiffs) {
                    change.fieldDiffs = change.fieldDiffs.sort((a, b) => {
                        return this.fieldDiffsSortBy(a, b);
                    });
                } 
            }
        }
...
DataPacksUtils.prototype.getTitleFieldsForDataPack = async function(sourceDataPack, targetDataPack, diffObject) {
    ...
    for (var i = titleFields.length-1; i >= 0; i--) {
        ...
        diffObject.fieldDiffs.push({
            ...
            diffKey: `${diffObject.editable.VlocityRecordSourceKey}/${field}`,
            fieldType: fieldType,
    ...
    if (sourceDataPack.VlocityDataPackParents || (targetDataPack && targetDataPack.VlocityDataPackParents)) {
        ...
        diffObject.fieldDiffs.push({
           ...
           //gitDiff: gitDiffForParents[0].diffString,
            diffKey: `${diffObject.editable.VlocityRecordSourceKey}/${field}`,
            status: (targetParentsJSON == null || sourceParentsJSON == targetParentsJSON) ? 'Unchanged' : 'Changed',
    ...
    if (sourceTitleObjects) {
        ...
        for (var i = 0; i < sourceTitleObjects.length; i++) {
            ...
            var titleObject = {
                ...
                VlocityRecordSourceKey: titleObjectForType.VlocityRecordSourceKey,
                VlocityRecordEditField: titleObjectForType.VlocityRecordEditField,
                VlocityDiffParentKey: titleObjectForType.VlocityDiffParentKey,
                //VlocityDiffDisplayParentKey: titleObjectForType.VlocityDiffDisplayParentKey,
                //VlocityFolderDisplayName: titleObjectForType.VlocityFolderDisplayName
            };

            if (titleObjectForType.fieldType == 'DisplayOnly') {
                titleObject.editable.TitleObjectDisplay = titleObjectForType.value;
            } else {
                titleObject.editable.DisplayCodeText = titleObjectForType.value;
            }
            ...
            for (var gitHunk of gitDiffForTitle) {
                ...
                titleObject.fieldDiffs.push({
                    ...
                    value: typeof sourceValue == 'object' ? stringify(sourceValue, { space: 4 }) : sourceValue,
                    old: (targetValue && typeof targetValue == 'object') ? stringify(targetValue, { space: 4 }) : targetValue,
                    field: (titleObjectForType.fieldType == 'DisplayOnly' ? 'TitleObjectDisplay' : 'DisplayCodeText'),
                    diffKey: this.guid(),
...
DataPacksUtils.prototype.getFieldDiffsForSObjects = async function(dataPackKey, dataPackType, sourceObject, targetObject, isDeleted) {
    var self = this;
    var fieldDiffs = [];
    var childObjects = [];
    ...
    for (var i = 0; i < allFields.length; i++) {
        ...
        && ignoreFieldDiffsForType.indexOf(field) == -1) {
            ...
            if ((sourceObjParsed != null && typeof sourceObjParsed == 'object') 
                ...
                if (Array.isArray(sourceObjParsed) || Array.isArray(targetObjParsed)) {
                    ...
                    fieldDiffs.push({
                        ...
                        diffKey: `${sourceObject.VlocityRecordSourceKey}/${field}`,
                        status: isDeleted ? 'Deleted' : stringifiedSource != stringifiedTarget ? 'Changed' : 'Unchanged',
                        value: stringifiedSource,
                        old: stringifiedTarget,
                        field: field,
                    ...
                    await self.setNestedFieldDiffs(dataPackType, sourceObjParsed || {}, targetObjParsed || {}, field, fieldDiffs, recordSourceKey, sObjectType);
                }
            ...
            if ((sourceObject[field] != null && typeof sourceObject[field] == 'object') 
                ...
                var gitDiffArray =  this.getGitDiffs(targetObjectValue, sourceObjectValue, expansionType);

                if (!gitDiffArray[0].diffString) {   
                    fieldDiffs.push({
                        fieldImportance: this.getFieldImportance(dataPackType, sObjectType, field),
                        fieldType: this.getFieldDisplayType(dataPackType, sObjectType, field),
                        diffKey: this.guid(),
                    ...
                    var separatedChild = {
                        editable: {
                            VlocityRecordSourceKey: recordSourceKey,
                            VlocityRecordSObjectType: field,
                            VlocityRecordEditField: field,
                            DisplayCodeText: sourceObjectValue ? sourceObjectValue : targetObjectValue
                        },
                        diffType: gitDiffArray[0].hasDiffs ? 'New' : 'Unchanged',
                        isDisplayOnly: true,
                        VlocityDataPackKey: dataPackKey,
                        VlocityDataPackType: dataPackType, 
                        VlocityRecordSourceKey: `${recordSourceKey}/${field}`,
                        VlocityRecordEditField: field,
                        VlocitySObjectRecordLabel: fieldLabel,
                        VlocityDiffParentKey: recordSourceKey,
                        VlocityDiffParentField: field,
                        VlocityDiffDisplayParentKey: recordSourceKey,
                        fieldDiffs: []
                    };

                    var sortFieldForDisplay = 1;
                    
                    for (var diffHunk of gitDiffArray) {

                        separatedChild.fieldDiffs.push({
                            fieldImportance: sortFieldForDisplay++,
                            fieldType: expansionType,
                            ...
                            label: field,
                            VlocityRecordSourceKey: `${recordSourceKey}/${field}`,
                            VlocityDataPackKey: recordSourceKey,
                            VlocityDataPackType: dataPackType,
                            VlocityRecordEditField: field
                        });
                    }

                    childObjects.push(separatedChild);
                }
            }
        }
    }

    return { fieldDiffs: fieldDiffs, childObjects: childObjects };
};
...
DataPacksUtils.prototype.getDiffsForSfdx = function(jobInfo, diffType, sourceSfdxObject, targetSfdxObject) {
    ...
    if (type == 'utf8') {
        ...
        } else if (sourceSfdxObject.SFDXData.length > 1500000) {
    ...
    for (var diffHunk of thisDiff) {
        fieldDiffs.push({
            ...
            field: "SFDXData",
            diffKey: `${sourceSfdxObject.VlocityDataPackKey}/SFDXData`,
            gitDiff: diffHunk.diffString,
            label: "Data",
            status: diffHunk.hasDiffs ? 'Changed' : 'Unchanged',
...
DataPacksUtils.prototype.fieldDiffsSortBy = function(obj1, obj2) {
    ...
    if (obj1.fieldImportance && !obj2.fieldImportance) {
        return -1;
    } else if (obj2.fieldImportance && !obj1.fieldImportance) {
        return 1;
    } else if (obj1.fieldImportance && obj2.fieldImportance && obj1.fieldImportance != obj2.fieldImportance) {
        // Field Important is alphabeical sorted - High, Low, Unimportant
        return obj1.fieldImportance < obj2.fieldImportance ? -1 : 1;
...
DataPacksUtils.prototype.setNestedFieldDiffs = async function(dataPackType, sourceObject, targetObject, currentPath, fieldDiffs,  vlocityRecordSourceKey, sObjectType) {
    var self = this;
    ...
    for (var field in sourceObject) {
        ...
        if ((sourceObjectValue == '' || sourceObjectValue == null || sourceObjectValue == '{\n}' || sourceObjectValue == '[\n]' ) 
            && (targetObjectValue == '' || targetObjectValue == null || targetObjectValue == '{\n}' || targetObjectValue == '[\n]')) {
            continue;
        }
        ...
        && !Array.isArray(sourceObject[field])
        && fieldType !== 'object') {
            await self.setNestedFieldDiffs(dataPackType, sourceObject[field], targetObject[field] || {}, `${currentPath}.${field}`, fieldDiffs, vlocityRecordSourceKey, sObjectType);
        } else if (Array.isArray(sourceObject[field]) || Array.isArray(targetObject[field])) {
            fieldDiffs.push({
                fieldImportance: this.getFieldImportance(dataPackType, sObjectType, field),
                fieldType: 'json',
                diffKey: `${vlocityRecordSourceKey}/${fieldPath}`,
                status: sourceObjectValue == targetObjectValue ? 'Unchanged' : targetObjectValue == null ? 'New' : 'Changed',
                value: sourceObjectValue,
                old: targetObjectValue,
                field: field,
...
DataPacksUtils.prototype.retrieveIndividualMetadata = async function(inputMap) {
    ...
    try {
        ...
        if (metaDesc.xmlName == 'Layout') {
            ...
            standardValueSets.forEach(item => deploymentOptions.allSFDXFileNames.add(item));
        } else {
        ...
        for (var key of jobInfo.specificManifestKeys) {
            ...
            if ((typeIndex == -1 && this.metadataTypesByFolder[key]) || this.metadataTypesByFolder[key.substring(0, typeIndex)]) {
                ...
                if (typeForKey == 'profiles') {
                   await this.readProfileAndWriteAsXML(nameForKey, path.join(sfdxFullPath, key));
                    ...
                    if (this.hasFolderMetadata.includes(typeForKey) && nameForKey.indexOf('/') != -1) {
                        ...
                        if (!jobInfo.specificManifestKeys.includes(folderMetadataFileName)) {
                            jobInfo.specificManifestKeys.push(folderMetadataFileName);
                        }
                        ...
                        deploymentOptions.metadata += `${xmlFolderName}Folder:${folderMetadataFileName}`;
                    }
...
DataPacksUtils.prototype.prepareProfilesForDeploy = async function(sourcepath) {
    ...
    /* TODO Need to account for these items being part of the current deployment */
    var existingTypesInProject = {};
    ...
    if (profiles.length > 0 || permissionSets.length > 0) {
        ...
        var profileNodes = [ 
            ...
            /* TODO Need to better handle Tab Visibilities for Objects that don't exist in Org since they are returned as Hidden */
        ...
        for (var profile of profiles) {
            ...
            for (var profileNode of profileNodes) {
                currentProfile.Profile[profileNode.type] = this.removeInvalidProfileElements(adminProfile[profileNode.type], currentProfile.Profile[profileNode.type], profileNode.key);
            }
            ...
            delete currentProfile.Profile.tabVisibilities;
        ...
        for (var permissionSet of permissionSets) {
            ...
            for (var permissionSetNode of profileNodes) {
                currentPermissionSet.PermissionSet[permissionSetNode.type] = this.removeInvalidProfileElements(adminProfile[permissionSetNode.type], currentPermissionSet.PermissionSet[permissionSetNode.type], permissionSetNode.key);
            }

            delete currentPermissionSet.PermissionSet['$'];
            delete currentPermissionSet.PermissionSet.tabVisibilities;
...
DataPacksUtils.prototype.removeInvalidProfileElements = function(adminArray, currentArray, keyToCompare) {
    var processedArray = [];
    var adminMap = {};
    ...
    if (currentArray) {
        for (var item of currentArray) {
            if (adminMap[item[keyToCompare][0]]) {
                ...
                VlocityUtils.log('Removing', item[keyToCompare][0])
            }
        }
    }
...
DataPacksUtils.prototype.filterInvalidLayouts = function(currentArray, layoutNames) {
    ...
    for (var item of currentArray) {
        ...
        if (layoutNames.includes(decodeURIComponent(layoutName))) {
            ...
            VlocityUtils.log('Removing', item.layout[0]);
        }
    }
...
DataPacksUtils.prototype.deploySalesforce = async function(jobInfo) {
    ...
    if (deploymentOptions.sourcepath) {
        ...
        await this.prepareProfilesForDeploy(deploymentOptions.sourcepath);
            ...
            if (Array.isArray(deploymentErrors)) {
                for (var deployedFile of deploymentErrors) {
                    ...
                    if (deployedFile.filePath) {
                        ...
                        metadataKey = `${deployedFile.type}/${deployedFile.fullName}`;
                    }
            ...
            for (var keyFromFile of allDeployedFiles) {
                ...
                if (fs.existsSync(filePath)) {
                    ...
                    if (deploymentErrors.message == 'Unknown SFDX Error') {
                        ...
                        jobInfo.currentStatus[key] = 'Error';
                        jobInfo.currentErrors[key] = resultsByMetadataKeyAndFilePath[filePath].error;
                        ...
                        jobInfo.currentStatus[key] = 'Error';
                        jobInfo.currentErrors[key] = resultsByMetadataKeyAndFilePath[baseFilePath].error;
                    } else {
