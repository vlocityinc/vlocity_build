    ...
    for (var i = 0; i < allFields.length; i++) {
        ...
        && sourceObject[field] != targetObject[field]) {
            ...
            if ((sourceObject[field] != null && typeof sourceObject[field] == 'object') 
                ...
                var stat = targetObjectValue != null ? 'Changed' : 'New';

                if (!(stat == 'New' && sourceObjectValue == "")) {

                    let diff; 
                    
                    if (sourceObjectValue && typeof sourceObjectValue == 'string' && targetObjectValue && typeof targetObjectValue == 'string') {
                        diff = `diff --git ${field} ${field}\n@@ -0,0 +0,0 @@\n + ${gitDiff(targetObjectValue, sourceObjectValue, { forceFake: true })}`;
                    }
    ...
    for (var field in sourceObject) {
        ...
        if (sourceObject[field] != targetObject[field]
            ...
            && sourceObjectValue != targetObjectValue) {
                ...
                if (typeof targetObjectValue == 'string' && typeof sourceObjectValue == 'string') {
                    diff = "diff --git a/a/a.html a/a/a.html\n@@ -1,1 +0,0 @@\n" + gitDiff(targetObjectValue, sourceObjectValue, { forceFake: true });
                }
