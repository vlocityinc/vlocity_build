...
DataPacksUtils.prototype.getAsManifestInfo = function(dataPack) {
    return DataPacksUtils.getAsManifestInfo(filePath);
}
...
DataPacksUtils.prototype.updateDataPackWithNewMappings = function (dataPack, jobInfo) {
    var dataField = this.getDataField(dataPack);
    var defaultNS = '%vlocity_namespace%__';

    // If DataPack has a dataField that has been impacted due to new Foundation package
    if (Object.keys(jobInfo.platformObjectMappings.Mappings).includes(dataField.replace(defaultNS, ''))) {
        let obj = {};
        let mappings = jobInfo.platformObjectMappings.Mappings[dataField.replace(defaultNS, '')];
        let newDataField = defaultNS + mappings.objectName;
        ...
        Object.keys(obj[newDataField][0]).forEach(key => {
            if (Object.keys(mappings.fieldNames).includes(key.replace(defaultNS, '')) && mappings.fieldNames[key.replace(defaultNS, '')] !== key.replace(defaultNS, '')) {
                obj[newDataField][0][defaultNS + mappings.fieldNames[key.replace(defaultNS, '')]] = obj[newDataField][0][key];
                delete obj[newDataField][0][key];
            }
            if (Object.keys(jobInfo.platformObjectMappings.Mappings).includes(key.replace(defaultNS, ''))) {
                let subMappings = jobInfo.platformObjectMappings.Mappings[key.replace(defaultNS, '')];
                let childObjDataField = defaultNS + subMappings.objectName;
                ...
                obj[newDataField][0][key].forEach(subObj => {
                    ...
                    Object.keys(subObj).forEach(fieldName => {
                        if (subMappings.fieldNames[fieldName.replace(defaultNS, '')]) {
                            level2Obj[defaultNS + subMappings.fieldNames[fieldName.replace(defaultNS, '')]] = subObj[fieldName];
                            if (fieldName !== subMappings.fieldNames[fieldName.replace(defaultNS, '')]) {
                                delete level2Obj[fieldName];
                            }
                        }
                    });
                    level2Obj.VlocityRecordSObjectType = childObjDataField;
                    level2Obj.VlocityRecordSourceKey = level2Obj.VlocityRecordSourceKey.replace(subObj.VlocityRecordSObjectType, childObjDataField);
        ...
        obj[newDataField][0].VlocityRecordSourceKey = obj[newDataField][0].VlocityRecordSourceKey.replace(dataField, newDataField);
