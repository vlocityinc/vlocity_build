...
DataPacksUtils.prototype.getAdditionalSFMetadataDependencies = async function(jobInfo, sfFolderPath){
    ...
    if (salesforceTypes.indexOf('classes') !== 0) {
        var allApexClassData = await this.vlocity.utilityservice.fetchAllRecords('SELECT Id,Name FROM ApexClass');
        var selectedApexIDNameMappings = {};
        allApexClassData.forEach(item => {
            let className = 'classes/' + item.Name + '.cls';
            if (jobInfo.specificManifestKeys.indexOf(className) !== -1) {
                selectedApexIDNameMappings['classes/' + item.Name + '.cls'] = item.Id;
            }
        });
    }
    ...
    for (let key in selectedApexIDNameMappings) {
        try {
            let query = `Select MetadataComponentName, MetadataComponentType, RefMetadataComponentName, RefMetadataComponentType from MetadataComponentDependency WHERE RefMetadataComponentId = '${selectedApexIDNameMappings[key]}'`;
            VlocityUtils.report('Query', query);
            let foundMetadataComponents = await this.vlocity.jsForceConnection.tooling.query(query);
            VlocityUtils.report('Found Records', foundMetadataComponents.records.length);
            dependenciesArr.push(foundMetadataComponents);
    ...
    fs.outputFileSync(path.join(sfFolderPath, 'sfReferences.yaml'), yaml.safeDump(metadataComponentMappings), "UTF-8");
...
DataPacksUtils.prototype.parallelRetrieve = async function(input) {
    ...
    try {
        ...
        if (jobInfo.maxDepth !== 0) {
            ...
            jobInfo.specificManifestKeys = jobInfo.specificManifestKeys.concat(retrievedLWCDependencies.retrievedLWCDependencyKeys);
        }
...
DataPacksUtils.prototype.retrieveLwcDependencies = async function (retrievedMetadata, input, jobInfo) {
    ...
    htmlFileContent.forEach(item => {
        htmlContentArr = item.split('<');
        htmlContentArr.forEach(content => {
            if (content.indexOf('c-') === 0) {
                let lwcKey = (content.split('>')[0]).split('-')[1];
    ...
    return {
        retrievedLWCDependencyKeys: retrievedLWCDependencyKeys,
...
DataPacksUtils.prototype.getSfdxData = async function(jobInfo) {
    ...
    let sfdxReferences = {};
    if (fs.existsSync(path.join(sfdxProject.sfdxPath, 'sfReferences.yaml'))) {
        sfdxReferences = yaml.safeLoad(fs.readFileSync(path.join(sfdxProject.sfdxPath, 'sfReferences.yaml'), 'utf8'));
    }
