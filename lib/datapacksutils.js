...
DataPacksUtils.prototype.getTitleFieldsForDataPack = async function(sourceDataPack, targetDataPack, diffObject) {
    ...
    var sourceTitleFields = await this.handleDataPackEvent('createTitleFields', sourceDataPack.VlocityDataPackType, { dataPackData: sourceDataPack });
    var targetTitleFields;
    
    if (targetDataPack) {
        targetTitleFields = await this.handleDataPackEvent('createTitleFields', targetDataPack.VlocityDataPackType, { dataPackData: targetDataPack });
    }

    if (sourceTitleFields) {
        // Put values into primary SObject to simplify later adding fieldDiff
        for (var titleFieldForType of sourceTitleFields) {
            sourcePrimarySObject[titleFieldForType.field] = titleFieldForType.value;
        }

        titleFields = titleFields.concat(sourceTitleFields);
    }

    if (targetTitleFields && targetPrimarySObject) {
        for (var titleFieldForType of targetTitleFields) {
            targetPrimarySObject[titleFieldForType.field] = titleFieldForType.value;
        }
    }
    ...
    for (var i = titleFields.length-1; i >= 0; i--) {
        ...
        var gitDiffForTitle = null;
        var sourceValue = sourcePrimarySObject[field];
        var targetValue = targetPrimarySObject ? targetPrimarySObject[fieldDiff.field] : null;

        if (fieldDiff.fieldType == 'side-by-side' || fieldDiff.fieldType == 'line-by-line') {
            gitDiffForTitle = this.getGitDiffs(targetValue || sourceValue, sourceValue, 'json', true);

            // Force not sidebyside if no target
            if (sourceValue == targetValue || targetValue == null) {
                fieldType = 'line-by-line';
            }
        } 
        ...
        && stringify(sourceObject) != stringify(targetObject)) {
                ...
                if (Array.isArray(sourceObjParsed) || Array.isArray(targetObjParsed)) {
                    ...
                    if (stringifiedSource != stringifiedTarget) {
                        fieldDiffs.push({
                            ...
                            gitDiff: this.getGitDiffs(stringifiedTarget, stringifiedSource, 'json'),
                            status: 'Changed',
            ...
            if ((sourceObject[field] != null && typeof sourceObject[field] == 'object') 
                ...
                fieldDiffs.push({
                    fieldImportance: this.getFieldImportance(dataPackType, sObjectType, field),
                    fieldType: this.getFieldDisplayType(dataPackType, sObjectType, field),
                    gitDiff: this.getGitDiffs(targetObjectValue, sourceObjectValue, expansionType),
                    status: fieldStatus,
...
DataPacksUtils.prototype.getDiffsForSfdx = function(jobInfo, diffType, sourceSfdxObject, targetSfdxObject) {
    ...
    if (type == 'utf8') {
        ...
        if (!blacklistedTypes.includes(type)) {
            ...
            thisDiff = this.getNoDiffs('(Hidden)', 'txt');
        }
    }
        ...
        fieldDiffs: [
            {
                ...
                gitDiff: thisDiff,
                label: "Data",
                old: targetSfdxObject && (sourceSfdxObject.SFDXData != targetSfdxObject.SFDXData) ? targetSfdxObject.SFDXData : null,
    ...
    for (var field in sourceObject) {
        ...
        if (sourceObject[field] != targetObject[field]
            ...
            && fieldType !== 'object') {
                ...
                if (sourceObjectValue != targetObjectValue) {
                    fieldDiffs.push({
                        //fieldImportance: this.getFieldImportance(dataPackType, sObjectType, field),
                        fieldType: 'json' ,
                        gitDiff: this.getGitDiffs(targetObjectValue, sourceObjectValue, fieldType ? fieldType : 'json'),
                        status: 'Changed',
                        value: sourceObject[field],
                        old: targetObject[field],
                        field: field,
            ...
            && sourceObjectValue != targetObjectValue) {
                ...
                fieldDiffs.push({
                    gitDiff: this.getGitDiffs(targetObjectValue, sourceObjectValue, 'json'),
                    status: 'Changed',
                    value: sourceObjectValue,
...
DataPacksUtils.prototype.saveCurrentJobInfo = function(jobInfo, force) {
    if (force || !this.lastWriteTimeJobInfo || (Date.now() - this.lastWriteTimeJobInfo > 60000)) {
        ...
        fs.writeFileSync(CURRENT_INFO_FILE, JSON.stringify(jobInfo, null, 4), 'utf8');
...
DataPacksUtils.prototype.getNoDiffs = function(data, type) {
...
DataPacksUtils.prototype.getGitDiffs = function(target, source, type, forceFake) {
    if (type != null && type != 'base64') {
        if (target && typeof target == 'string' 
            ...
            let diffString = gitDiff(target, source, { flags: '--unified=10', forceFake: (forceFake == true) });

            if (diffString) {
                let hasHeader = diffString[0] == '@' && diffString[1] == '@';
                return `diff --git File.${type} File.${type}\n${hasHeader ? '' : '@@ -1,0 +1,0 @@\n'}${diffString}`;
            } else {
                return this.getNoDiffs(target, type);
            }
            ...
            return this.getNoDiffs(source, type);
        }
    }
...
DataPacksUtils.prototype.createBulkJob = async function (objectName, operation, data) {
        ...
        while (batchStartIndex < dataLength) {
            ...
            batch.execute(data.splice(0, thisBatchSize))
                ...
                .on("queue", function (batchInfo) {
                    batch.poll(1000 /* interval(ms) */, 40000 /* timeout(ms) */);
                })
                .on("response", function (rets) {
                    for (var i = 0; i < rets.length; i++) {
                        if (!rets[i].success) {
                            VlocityUtils.error('Error', 'Batch Insert Failed', `#${i+1}`, rets[i].errors.join(', '));
                        }
                    }
                    batchCnt++;
            ...
            self.vlocity.jsForceConnection.query(query)
                ...
                .on("end", async function () {
                    if (records.length > 0 && salesforceIdDeleteBatch[salesforceId] != 'true') {
                        ...
                        await self.createBulkJob(objectName, 'delete', records);
                    }
                    resolve();
                })
...
var unsupportedMetadataTypes = [ 'AccessControlPolicy', 'Bot', 'BotVersion', 'FlowDefinition', 'Form', 'FormSection', 'InstalledPackage', 'IntegrationHubSettings', 'IntegrationHubSettingsType', 'LightningComponentBundle', 'Package', 'PermissionSetGroup', 'Audience', 'ManagedTopics', 'KeywordList', 'ModerationRule', 'UserCriteria', 'CleanDataService' ]
...
DataPacksUtils.prototype.retrieveSalesforce = async function(jobInfo) {
    ...
    if (jobInfo.specificManifestKeys) {
        ...
        for (var key of jobInfo.specificManifestKeys) {
            ...
            deploymentOptions.sourcepath += path.join(sfdxFolder, 'main', 'default', key);
        }
        ...
        for (var metaDesc of allMetadata.metadataObjects) {
            ...
            if (unsupportedMetadataTypes.includes(metaDesc.xmlName) 
...
DataPacksUtils.prototype.getSFDXProject = function(jobInfo) {
    ...
    if (!fs.existsSync(path.join(process.cwd(), 'sfdx-project.json')) || jobInfo.deleteSfdxProject || jobInfo.resetSfdxProjectLocation) { 

        if (!jobInfo.sfdxExpandFolder) {
            jobInfo.sfdxExpandFolder = this.vlocity.tempFolder;
        }
        
        var sfdxTempFolder = path.relative(process.cwd(), `${jobInfo.sfdxExpandFolder}/DX/salesforce_sfdx`);
        ...
        fs.outputFileSync(path.join(process.cwd(), 'sfdx-project.json'), JSON.stringify(salesforceProject), { encoding: 'utf8' });
    ...
    if (deploymentOptions.sourcepath) {
        ...
        if (response) {   
            for (var includedFile of response) {
                ...
                if (includedFile.filePath) {
                    var folder = 'default/';
                    metadataKey = includedFile.filePath.substring(dir.lastIndexOf(folder)+folder.length);
                } else {
