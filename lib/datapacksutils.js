...
DataPacksUtils.prototype.updateDataPackWithNewMappings = function (dataPack, jobInfo) {
    ...
    let mappings = jobInfo.platformObjectMappings[dataPackType][dataField.replace(defaultNS, '')];
    ...
    Object.keys(obj[newDataField][0]).forEach(key => {
        ...
        if (Object.keys(jobInfo.platformObjectMappings[dataPackType]).includes(key.replace(defaultNS, ''))) {
            let subMappings = jobInfo.platformObjectMappings[dataPackType][key.replace(defaultNS, '')];
