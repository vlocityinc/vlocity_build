    ...
    let promise = await new Promise((resolve, reject) => {
        ...
        if (!self.fieldLabels[sObject]) {
            ...
            self.vlocity.jsForceConnection.sobject(sObject.replace('%vlocity_namespace%', self.vlocity.namespace)).describe(function(err, meta) {
                ...
                resolve(self.fieldLabels[sObject][fieldApiName]);
            });
        } else {
            resolve(self.fieldLabels[sObject][fieldApiName]);
        }       
...
DataPacksUtils.prototype.getFieldDiffs = async function(allSourceDataPacks, allTargetDataPacks, jobInfo, callback) {
    var self = this;
    ...
    for (var i = 0; i < allKeys.length; i++) {
        ...
        if (jobInfo.diffStatus[dataPackKey] != 'Unchanged') {
            ...
            for (var uniqueKey in sourceDiffKeys) {
                if (sourceDiffKeys[uniqueKey] && targetDiffKeys[uniqueKey]) {
                    ...
                    if (fieldDiffs.length > 0) {
                        allFieldValues[sourceDiffKeys[uniqueKey].VlocityRecordSourceKey] = fieldDiffs;
                        allFieldValues[targetDiffKeys[uniqueKey].VlocityRecordSourceKey] = fieldDiffs;
                    }
                }
            }

            for (var sourceObj of allSourceSObjects) {

                var obj = { editable: sourceObj  };

                if (!allFieldValues[sourceObj.VlocityRecordSourceKey]) {
                    obj.diffStatus = 'New';
                    obj.fieldDiffs = await self.getFieldDiffsForSObjects(sourceDataPack.VlocityDataPackType, sourceObj, {});
                } else {
                    obj.diffStatus = 'Changed';
                    obj.fieldDiffs = allFieldValues[sourceObj.VlocityRecordSourceKey];
                }
            ...
            allTargetSObjects.forEach(function(obj) {
                if (!allFieldValues[obj.VlocityRecordSourceKey]) {
                    if (self.isDeletedDuringDeploy(sourceDataPack.VlcoityDataPackType, obj.VlocityRecordSObjectType)) {
                        allFieldDiffsByRecordSourceKey.push({ editable: sourceObj, diffStatus: 'Deleted' });
                    }
                }
            });
            ...
            for (var obj of allFieldDiffsByRecordSourceKey) {
                ...
                var label = obj.VlocityRecordSObjectType;
                ...
                obj.VlocitySObjectRecordLabel = label;
            }
