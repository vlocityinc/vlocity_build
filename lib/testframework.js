...
TestFramework.prototype.result = async function(input, resultWithDetails) {
    ...
    VlocityUtils.milestone('Running Test Procedures', `0 of ${input.testKeys.length}`);
    ...
    do {
            ...
            && finalResult.Status === 'InProgress')
        {
            if (finalResult.Total) {
                VlocityUtils.milestone('Running Test Procedures', `${finalResult.success + finalResult.failures} of ${finalResult.Total}`);
            }
