...
TestFramework.prototype.start = async function(input) {
    ...
    let result =  await this.vlocity.jsForceConnection.apex.post('/' + this.vlocity.namespace + '/v1/testframework/start', request, function(err, res) {
        if (err) { return VlocityUtils.error('Start Error', err); }

        VlocityUtils.verbose('Start Success', res);
    });
...
TestFramework.prototype.startAll = async function() {
    ...
    let result = await this.vlocity.jsForceConnection.apex.post('/' + this.vlocity.namespace + '/v1/testframework/startAll', {}, function(err, res) {
        if (err) { return VlocityUtils.error('Start All Error', err); }

        VlocityUtils.verbose('Start All Success', res);
    });
...
TestFramework.prototype.result = async function(input, resultWithDetails) { 
    ...
    if (withDetails) {
        endpoint = '/' + this.vlocity.namespace + '/v1/testframework/resultWithDetails';
    } else {
        endpoint = '/' + this.vlocity.namespace + '/v1/testframework/result';
    }
...
TestFramework.prototype.resultChunked = async function(input, resultWithDetails) {
    ...
    VlocityUtils.verbose('Promise Results', promiseResults);
...
TestFramework.prototype.executeGetTestProcedures = async function(jobInfo) {
    ...
    let result = await this.vlocity.jsForceConnection.apex.get('/' + this.vlocity.namespace + '/v1/testframework/alltests', function(err, res) {
        if (err) { return VlocityUtils.error('Get Test Procedures', err); }

        VlocityUtils.verbose('Get Test Procedures', res);
    });
...
TestFramework.prototype.createResponse = function(response, testSuiteUniqueKey) {
    if (response
        ...
        for (var testResult of response.testResults) {
            ...
            let statusField = testResult[this.vlocity.namespacePrefix + 'Status__c'];
            let testName = testResult[this.vlocity.namespacePrefix + 'TestName__c'];
...
TestFramework.prototype.saveToDir = function(path, data) {
    if (path && data) {
        VlocityUtils.verbose('Save To Dir Data', data);
