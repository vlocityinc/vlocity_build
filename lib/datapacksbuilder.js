...
var BULK_RECORDS_LIMIT_MIN = 2000;
        ...
        for (var nextImport of (nextImports || [])) {
            ...
            if (!shouldBreakImportLoop) {
                ...
                if (!jobInfo.singleFile && self.needsPagination(nextImport, jobInfo)) {
                    if (nextImport.VlocityDataPackType === 'CalculationMatrix' && nextImport.recordsCount > BULK_RECORDS_LIMIT_MIN) {
                        await self.vlocity.datapacksjob.extractCalculationMatrixRecords(nextImport, calculationMatrixRecords);
                        ...
                        self.savedBulkRecords[dataPackKey] = JSON.parse(JSON.stringify(calculationMatrixRecords));
                ...
                if (!jobInfo.singleFile && (Object.keys(currentDataPackKeysInImport).length == self.vlocity.datapacksutils.getMaxDeploy(nextImport.VlocityDataPackType) || shouldBreakImportLoop)) {
                    shouldBreakImportLoop = true;
                }
            } else {
...
DataPacksBuilder.prototype.initializeImportStatus = async function(importPath, jobInfo) {
    ...
    for (var dataPackType of importPaths) {
        ...
        VlocityUtils.verbose('Found datapack candidates', dataPackType, allDataPacksOfType.length);
    
        for (var dataPackName of allDataPacksOfType) {
            importFolderPromises.push(this.initializeTypeAtPath(dataPackTypeDir, dataPackName, dataPackType, jobInfo));
        }
    }

    await Promise.all(importFolderPromises);
            ...
            if (field != 'Name') {
                    ...
                    && typeof potentialFileNames[0] === 'string' ) 
                {
                    ...
                    if (allDataPackFileData.length > 0) {
                        self.buildFromFiles(dataPackKey, allDataPackFileData, fullPathToFiles, dataPackType, field);
