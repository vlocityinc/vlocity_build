    ...
    for (var i = 0; i < dataPackKeys.length; i++) {
        ...
        if (jobInfo.currentStatus[dataPackKey] == 'Ready' 
            ...
            try {
                ...
                // Skip When there is nothing relevant to deploy
                if (!jobInfo.singleFile) {   
                    let isSkipDeploy = this.vlocity.datapacksutils.handleDataPackEventSync('isSkipDeploy', dataPackType, { dataPackData: dataPackImportBuilt, vlocity: this.vlocity });
...
DataPacksBuilder.prototype.compile = function(lang, source, options, cb) {
    ...
    switch(lang) {
        case 'scss': {
            // Modern Dart Sass API - using compileStringAsync instead of deprecated render
            var includePaths = options.importer && options.importer.includePaths ? options.importer.includePaths : [];
            
            // Modern importer API using findFileUrl
            var modernImporter = {
                findFileUrl: (importUrl, options) => {
                    // importUrl: the import path
                    // options: compilation options
                    // Returns: URL object or null if not found
                    
                    if (includePaths && Array.isArray(includePaths) && includePaths.length > 0) {
                        // Add .scss extension if not present (backward compatibility)
                        var fileName = importUrl;
                        if (!fileName.endsWith('.scss') && !fileName.endsWith('.sass')) {
                            fileName = fileName + '.scss';
                        }
                        
                        // Try to find file in includePaths synchronously
                        for (var i = 0; i < includePaths.length; i++) {
                            var filePath = path.join(includePaths[i], fileName);
                        ...
                        for (var i = 0; i < includePaths.length; i++) {
                            var filePath = path.join(includePaths[i], importUrl);
                            if (fs.existsSync(filePath)) {
                                return url.pathToFileURL(path.resolve(filePath));
                            }
                        }
                    ...
                    // Return null to let Dart Sass try its default resolution
                    return null;
                }
            };
            
            // Modern compileStringAsync API options
            var compileOptions = {
                importers: [modernImporter],
                loadPaths: includePaths,
                style: 'expanded', // Default style
                silenceDeprecations: ['import'] // Suppress @import deprecation warnings for backward compatibility
            };
            
            // Merge any additional options from the old API
            if (options.outputStyle) {
                compileOptions.style = options.outputStyle;
            }
            if (options.sourceMap) {
                compileOptions.sourceMap = options.sourceMap;
            }
            
            // Use modern compileStringAsync API
            // First argument is the source string, second is options
            sass.compileStringAsync(source, compileOptions)
                .then((result) => {
                    cb(null, result.css);
                })
                .catch((err) => {
                    var error = new Error('SASS compilation failed, see error message for details: ' + err.message);
                    cb(error, null);
                });
        } return;
        default: {       
