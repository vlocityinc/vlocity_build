...
DataPacksBuilder.prototype.getNextImports = function(importPath, jobInfo, currentDataPackKeysInImport, currentBuildLength, validateOnly) {
    ...
    Object.keys(jobInfo.currentStatus).forEach(key => {
        ...
        if ((jobInfo.currentStatus[key] === 'Ready' || jobInfo.currentStatus[key] === 'Added') && !dataPackTypesWithReadyStatus.includes(type)) {
            dataPackTypesWithReadyStatus.push(type);
        }
    });

    var typesToBeDeployedLast = [];
    var dataPacks = this.vlocity.datapacksutils.dataPacksExpandedDefinition.DataPacks;
    Object.keys(dataPacks).forEach(dataPackType => {
        if (dataPacks[dataPackType].DeployLast && dataPackTypesWithReadyStatus.includes(dataPackType)) {
