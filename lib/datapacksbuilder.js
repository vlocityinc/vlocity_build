...
var MAX_IMPORT_PRE_STEP_LENGTH = 8000;
var DEFAULT_MAX_IMPORT_SIZE = 1000000;
...
DataPacksBuilder.prototype.buildImport = function(importPath, jobInfo, onComplete) {
    ...
    do {
        nextImport = self.getNextImport(importPath, Object.keys(jobInfo.currentStatus), jobInfo.singleFile === true, jobInfo, currentDataPackKeysInImport, !jobInfo.singleFile ? stringify(dataPackImport).length : 0, currentImportPreStepLength);

        if (nextImport) {

             // Ignore if does not have an Pre Step Apex Class 
            if (!jobInfo.singleFile && (jobInfo.preStepApex && jobInfo.preStepApex[jobInfo.jobAction] && jobInfo.preStepApex[jobInfo.jobAction][nextImport.VlocityDataPackType])) {
    ...
    } while (nextImport && (jobInfo.singleFile || (stringify(dataPackImport).length < self.maxImportSize && dataPackImport.dataPacks.length < maxImportCount)))
...
DataPacksBuilder.prototype.getNextImport = function(importPath, dataPackKeys, singleFile, jobInfo, currentDataPackKeysInImport, currentBuildLength, currentImportPreStepLength) {
    var self = this;
    ...
    for (var i = 0; i < dataPackKeys.length; i++) {
        ...
        if (jobInfo.currentStatus[dataPackKey] == 'Ready' 
            ...
            try {
                ...
                if (!self.builtDataPacks[dataPackKey]) {
                    self.builtDataPacks[dataPackKey] = self.buildFromFiles(dataPackKey, dataPackDataMetadata, fullPathToFiles, dataPackType, sobjectDataField);
                } 

                var dataPackImportBuilt = self.builtDataPacks[dataPackKey];
                ...
                if (!jobInfo.singleFile && Object.keys(currentDataPackKeysInImport).length != 0) {
                    ...
                    if (self.dataPackSizes[nextImport.VlocityDataPackKey] + currentBuildLength > self.maxImportSize 
                        || currentImportPreStepLength + stringify(jobInfo.allDataSummary[nextImport.VlocityDataPackKey]).length > MAX_IMPORT_PRE_STEP_LENGTH) {
                        continue;
                    }
                }
