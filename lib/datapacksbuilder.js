...
DataPacksBuilder.prototype.compile = function(lang, source, options, cb) {
    ...
    switch(lang) {
        case 'scss': {
            // Modern sass API using compileStringAsync with custom importer
            const sassOptions = {
                style: options.style || 'expanded',
                loadPaths: options.includePaths || [],
                importers: [{
                    findFileUrl(urlString) {
                        // Handle @import statements
                        if (!urlString.startsWith('~')) {
                            // Try to resolve the file from includePaths
                            if (options.includePaths && Array.isArray(options.includePaths)) {
                                for (const includePath of options.includePaths) {
                                    // Try multiple variations of the file path
                                    // Sass allows importing partials with or without underscore and extension
                                    const basename = path.basename(urlString, '.scss');
                                    const variations = [
                                        urlString,                              // exact name as provided
                                        urlString + '.scss',                    // add .scss if missing
                                        '_' + basename + '.scss',               // partial with underscore
                                        basename + '.scss',                     // without underscore
                                        path.join(path.dirname(urlString), '_' + path.basename(urlString, '.scss') + '.scss'),  // partial in subdirectory
                                        path.join(path.dirname(urlString), path.basename(urlString, '.scss') + '.scss')         // in subdirectory
                                    ];
                                    
                                    for (const variant of variations) {
                                        const filePath = path.join(includePath, variant);
            ...
            sass.compileStringAsync(source, sassOptions)
                .then(result => {
                    cb(null, result.css);
                })
                .catch(error => {
                    const err = new Error('SASS compilation failed: ' + (error.message || error.formatted || error));
                    cb(err, null);
                });
        } return;
        default: {       
