...
const UTF8_EXTENSIONS = ['css', 'json', 'yaml', 'scss', 'html', 'js', 'xml']

const DEFAULT_MAX_DEPLOY_COUNT = 20
const DEFAULT_MAX_IMPORT_SIZE = 2000000

const DataPacksBuilder = module.exports = function (vlocity) {
...
DataPacksBuilder.prototype.buildImport = async function (importPath, jobInfo, typesInAllImports) {
  var self = this
...
DataPacksBuilder.prototype.needsPagination = function (dataPackData, jobInfo) {
  if (!this.needsPaginationForKey[dataPackData.VlocityDataPackKey]) {
    ...
    this.needsPaginationForKey[dataPackData.VlocityDataPackKey] = !jobInfo.disablePagination && this.countRecords(dataPackData) > paginationLimit
  }
    ...
    if (Array.isArray(currentSObject[keyField])) {
      ...
      if (recordsCounts > 0) {
        if (currentSObject[keyField].length > 0 && currentSObject[keyField].length < paginationLimit) {
          ...
          for (let i = 0; i < currentSObject[keyField].length; i += paginationLimit) {
            var pageChunk = currentSObject[keyField].slice(i, i + paginationLimit)
            arrayOfPaginated.push(pageChunk)
          }

          for (let i = 0; i < arrayOfPaginated.length; i++) {
