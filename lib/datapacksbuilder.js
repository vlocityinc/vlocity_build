...
DataPacksBuilder.prototype.buildFromFiles = function(dataPackKey, dataPackDataArray, fullPathToFiles, dataPackType, currentDataField, jobInfo) {
    ...
    return this.updateInvalidMatchingKeyFields(dataPackDataArray, [], jobInfo); 
};

DataPacksBuilder.prototype.updateInvalidMatchingKeyFields = function(currentData, updatedDataPackDataArray,jobInfo) {
    if (currentData) {
        if (Array.isArray(currentData)) {
            currentData.forEach((childData) => {
                updatedDataPackDataArray.push(this.updateInvalidMatchingKeyFields(childData, updatedDataPackDataArray, jobInfo));
            });
        } else {
            try {
                Object.keys(currentData).forEach((sobjectField) => {
                    if (typeof currentData[sobjectField] === "object" 
                        ...
                        && this.matchingKeyFields[currentData[sobjectField].VlocityRecordSObjectType]) {
                            Object.keys(currentData[sobjectField]).forEach((sobjField) => {
                                if (sobjField != "VlocityDataPackType"
                                    && sobjField != "VlocityMatchingRecordSourceKey"
                                    && sobjField != "VlocityRecordSObjectType"
                                    && sobjField.indexOf('.') == -1 
                                    && this.matchingKeyFields[currentData[sobjectField].VlocityRecordSObjectType].indexOf(sobjField) == -1) {
                                        if(jobInfo.AutoUpdateMatchingKeys){
                                            currentData[sobjectField][this.matchingKeyFields[currentData[sobjectField].VlocityRecordSObjectType][0]] = currentData[sobjectField][sobjField];
                                            delete currentData[sobjectField][sobjField];
                                        } else {
                                            throw currentData.VlocityRecordSourceKey.split('/')[0]
                                        }
                                } 
                            });
                    } 
                ...
                jobInfo.hasError = true;
                jobInfo.errors.push(`Matching keys different >> ${e}`);
                VlocityUtils.error('Matching keys different', e);
            }
        }
    }
