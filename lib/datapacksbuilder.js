...
DataPacksBuilder.prototype.buildImport = async function(importPath, jobInfo) {
    ...
    this.matchingKeyFields = this.matchingKeyFields || await this.vlocity.utilityservice.getDRMatchingKeyFields();
        ...
        if (Array.isArray(currentData)) {
            ...
            Object.keys(currentData).forEach((sobjectField) => {
                if (typeof currentData[sobjectField] === "object"
                    && currentData[sobjectField].VlocityDataPackType === "VlocityMatchingKeyObject"
                    && this.matchingKeyFields[currentData[sobjectField].VlocityRecordSObjectType]) {
                    let keys = this.matchingKeyFields[currentData[sobjectField].VlocityMatchingRecordSourceKey.split('/')[0]];
                    if (keys) {
                        keys.forEach((key) => {
                            let returnResponse = this.processMatchingKeys(key, currentData, sobjectField);
                            if (!returnResponse.error) {
                                currentData = returnResponse.currentData;
                            } else {
                                jobInfo.hasError = true;
...
DataPacksBuilder.prototype.processMatchingKeys = function (key, currentData, sobjectField) {
    //If matching key is not present, it might be simple matching key, having different name, or linked (nested) matching key.
    if (!currentData[sobjectField][key]) {
        let errorFlag = true;
        // If matching key is present in currentData, it means it is linked(nested) matching key,
        // otherwise, 
        // the simple matching key, with different name.
        if (currentData[key]) {
            // Creating linked matching key, for example - "%vlocity_namespace%__CalculationProcedureId__r.Name"
            let linkedMatchingKey = key.replace('__c', '__r') + '.' + this.matchingKeyFields[currentData[key].VlocityRecordSObjectType][0];
            if (!currentData[sobjectField][linkedMatchingKey]) {
                Object.keys(currentData[sobjectField]).forEach((field) => {
                    //Pointing out linked matching key with same parent
                    if (field.indexOf('.') === -1 && field.indexOf(key.replace('__c', '')) !== -1) {
                        currentData[sobjectField][linkedMatchingKey] = currentData[sobjectField][field];
                        delete currentData[sobjectField][field];
                        errorFlag = false;
                    }
                });
            } else {
            ...
            Object.keys(currentData[sobjectField]).forEach((field) => {
                if (field !== "VlocityDataPackType"
                    && field !== "VlocityMatchingRecordSourceKey"
                    && field !== "VlocityRecordSObjectType"
                    && field.indexOf('.') === -1){
                    if(this.matchingKeyFields[currentData[sobjectField].VlocityRecordSObjectType].indexOf(field) === -1) {
                        currentData[sobjectField][key] = currentData[sobjectField][field];
                        delete currentData[sobjectField][field];
                        errorFlag = false;
                    } else {
                        errorFlag = false;
        ...
        return { currentData: currentData, error: errorFlag };
    } else {
        return { currentData: currentData, error: false };
    }
};
