...
DataPacksBuilder.prototype.getDataPackLabel = function(jobInfo, dataPackTypeDir, dataPackName) {
    try {
        var joinedFolder = path.join(dataPackTypeDir, dataPackName);
    
        if (jobInfo.sanitizedLabelsToDirectories[dataPackName]) {
            joinedFolder = dataPackTypeDir + '/' + jobInfo.sanitizedLabelsToDirectories[dataPackName];
        }

        var allFiles = this.vlocity.datapacksutils.getFiles(joinedFolder);
...
DataPacksBuilder.prototype.initializeImportStatus = function(importPath, jobInfo) {
    ...
    importPaths.forEach(function(dataPackType) {
        ...
        allDataPacksOfType.forEach(function(dataPackName) {
            ...
            var dataPackLabel = self.getDataPackLabel(jobInfo, dataPackTypeDir, dataPackName);

            jobInfo.sanitizedLabelsToDirectories[self.vlocity.datapacksexpand.generateFolderOrFilename(dataPackName)] = dataPackLabel;
    ...
    for (var i = 0; i < dataPackKeys.length; i++) {
        ...
        if (jobInfo.currentStatus[dataPackKey] == 'Ready' 
            ...
            try {
                ...
                var dataPackLabel = self.getDataPackLabel(jobInfo, importPath + '/' + dataPackType, self.vlocity.datapacksexpand.generateFolderOrFilename(dataPackName));

                var fullPathToFiles = path.join(importPath, dataPackType, dataPackLabel);
                var parentData;
