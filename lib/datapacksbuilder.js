                ...
                if (typeof currentData[sobjectField] === "object"
                    ...
                    if (matchingObjectKeys) {
                        matchingObjectKeys.forEach((key) => {
                            let returnResponse = this.processMatchingKeys(matchingObjectKeys, key, currentData, sobjectField);
...
DataPacksBuilder.prototype.processMatchingKeys = function (matchingObjectKeys, key, currentData, sobjectField) {
    var errorFlag;
    if (!currentData[sobjectField][key]) {
        errorFlag = true;
        var recordSourceKeyArr = currentData[sobjectField].VlocityMatchingRecordSourceKey.split('/');
        Object.keys(currentData[sobjectField]).forEach((sobjField) => {
            if (sobjField != "VlocityDataPackType"
                ...
                && matchingObjectKeys.indexOf(sobjField) === -1) { 
                    Object.keys(this.allFileDataMap).forEach((filePath)=>{
                        Object.keys(recordSourceKeyArr).forEach((index) => {
                            if(filePath.indexOf(recordSourceKeyArr[index].toLowerCase()) !== -1 && filePath.indexOf('datapack.json') !== -1){
                                let availableDataSearch = JSON.parse(this.allFileDataMap[filePath]);
                                if(availableDataSearch[key]){
                                    currentData[sobjectField][key] = availableDataSearch[key];
