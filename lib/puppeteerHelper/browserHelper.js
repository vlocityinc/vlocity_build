...
const traverseDOM = async (element,packageNamespaceWithoutendspecialCharacter) => {
    ...
    if (tagName === 'iframe') {
        ...
        if (frame) {
            ...
            if (frameBody && await traverseDOM(frameBody,packageNamespaceWithoutendspecialCharacter)) {
    ...
    if (hasShadowRoot) {
        ...
        for (let i = 0; i < shadowChildrenArray.length; i++) {
            ...
            if (await traverseDOM(childHandle,packageNamespaceWithoutendspecialCharacter)) {
...
exports.verifyOmniscriptActivation = async (page,packageNamespace) => {
    ...
    if (!canvasWebComponentHandle) throw new Error(`${packageNamespace[0]}-omni-designer-canvas element not found`);
    ...
    const contentFrame = await iframeHandle.contentFrame();
        ...
        if (bodyHandle) {
            return await traverseDOM(bodyHandle,packageNamespaceWithoutendspecialCharacter);
        } else {
        return true;
        }
};
