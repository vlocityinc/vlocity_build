    ...
    try {
        ...
        if (dataPackData.isChunked) {
            ...
            try {
                ...
                throw { retrieveError: err.message };
            }
        }
...
DataPacks.prototype.getDataPackDataChunked = async function(dataPackDataInfo) {
    ...
    for (var chunkKey of dataPackDataInfo.keysToRetrieve) {
        allChunkInfos.push({ context: this, func: 'getChunk', argument: { chunkKey: chunkKey, dataPackDataInfo: dataPackDataInfo }});
    }
    
    await this.vlocity.utilityservice.parallelLimit(allChunkInfos, 5);
...
DataPacks.prototype.runDataPackProcess = async function(dataPackData, options) {
    ...
    while (true) {
        ...
        try {
            ...
            if (!result) {
                result = { VlocityDataPackId: dataPackId, message: 'Transaction Failed', dataPackError: true };
                throw result;
            }
            ...
            for (var dataPackToAdd of addDataPackArray) {
                await self.addDataPack(result.VlocityDataPackId, dataPackData.processType, dataPackToAdd);
            }
...
DataPacks.prototype.addDataPack = async function(dataPackId, processType, dataPack, retryCount) {
    ...
    try {
        ...
        if (!retryCount || retryCount < 3) {
            ...
            throw { Message: 'Failed to Add Children Records to Export ' + dataPack.VlocityDataPackType + ' ' + (dataPack.VlocityDataPackData.Id ? ('ParentId: ' + dataPack.VlocityDataPackData.Id) : '') + ' ' + (err.message ? err.message : '')};
        }
    }
}
