            ...
            if (err) { 
                if (err.code == 'ECONNRESET') {
                    ...
                    q.push([chunkKey], function (err) {
                        if (err) {
                            VlocityUtils.error('Error retreiving chunk', chunkKey, err);
                        }
                    });

                } else if (retriedKeys.indexOf(chunkKey) == -1) {
                    VlocityUtils.error('Retrying API Error - Get Chunk', dataPackDataInfo.dataPackId, chunkKey, err);
                ...
                try {
                    ...
                    VlocityUtils.error('API Error - Get Chunk', dataPackDataInfo.dataPackId, chunkKey, e);
                }
            }
...
DataPacks.prototype.runDataPackProcess = function(dataPackData, options, onSuccess, onError) {
    ...
    self.vlocity.jsForceConnection.apex.post(self.dataPacksEndpoint(), dataPackData, function(err, result) {
        try {
                ...
                if (err.code == 'ECONNRESET' || err.errorCode == 'ECONNRESET') {
                    ...
                    if (dataPackId && self.retried.indexOf(dataPackId) == -1) {
                        ...
                        self.vlocity.checkLogin(function() {
                            self.runDataPackProcess(dataPackData, options, onSuccess, onError); 
                        }, onError);
            ...
            else if (onSuccess) onSuccess(exception);
        }
    });
};
