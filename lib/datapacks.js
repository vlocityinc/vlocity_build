...
DataPacks.prototype.runDataPackProcess = async function(dataPackData, options) {
    ...
    if (!dataPackId) {
        if (dataPackData.processType == "Import") {
            ...
            dataPackData.processData.VlocityDataPackData.forEach(function(dataPack) {
                ...
                var exportGroupSize = self.vlocity.datapacksutils.getExportGroupSizeForType(dataPack.VlocityDataPackType);

                if (exportGroupSize == 1 && dataPack.VlocityDataPackRelationshipType != "Children") { 
                    isSingleExportGroup = true;
                }
            });
    ...
    while (true) {
        ...
        try {
            ...
            if (isSingleExportGroup && (result.Status != "Ready" && result.Status != "Error")) {
