const DATA_PACKS_REST_RESOURCE = '/v1/VlocityDataPacks/'
const VlocityUtils = require('./vlocityutils')

const DataPacks = module.exports = function (vlocity) {
...
DataPacks.prototype.getDataPackData = async function (dataPackId, retry) {
  ...
  try {
    ...
    if (!retry || err.code === 'ECONNRESET') {
      ...
      return self.getDataPackData(dataPackId, true)
    } else {
  ...
  while (true) {
    try {
      ...
      if (err.code === 'ECONNRESET' || err.errorCode === 'ECONNRESET') {
        ...
        let newerr = { VlocityDataPackId: dataPackId, message: err.message ? err.message.trim() : '', code: err.name, dataPackError: true }

        if (dataPackId && self.retried.indexOf(dataPackId) === -1) {
          self.retried.push(dataPackId)

          VlocityUtils.error('RETRYING FOR ERROR', dataPackId, newerr.code + ':', newerr.message)

          await self.vlocity.checkLogin()
        } else {
          VlocityUtils.error('Error', dataPackId, newerr.code || newerr.errorCode, ':', newerr.message)
          return newerr
        }
      }
    }
...
DataPacks.prototype.export = async function (dataPackType, exportData, options) {
  ...
  return this.runDataPackProcess(dataPackData, options)
}
