...
DataPacksExpand.prototype.getNestedDataPackReferences = function(jobInfo, currentData, parentKeys, referencefield, referenceType) {
    ...
    if (!referencefield.includes('.')) {
        ...
        if (currentData[referencefield]) {
            ...
            if (Array.isArray(currentData[referencefield])) {
                ...
                if(!(jobInfo.maxDepth === 0) || jobInfo.includeSalesforceMetadata){
                    let referenceInfo =  this.vlocity.datapacksutils.handleDataPackEvent('onAutoExport', referenceType, currentData);
                    if (!jobInfo.specificManifestKeys){
                        jobInfo.specificManifestKeys = [];
                    } 
                    referenceInfo.then(result => {
                        if(result && jobInfo.specificManifestKeys.indexOf(result) === -1){
                            jobInfo.specificManifestKeys.push(result);
                        }
                    });
                }
        ...
        if (Array.isArray(currentData[primaryField])) {
            for (var nestedMore of currentData[primaryField]) {
                this.getNestedDataPackReferences(jobInfo, nestedMore, parentKeys, splitFields.join('.'), referenceType);
            } 
        } else {
            this.getNestedDataPackReferences(jobInfo, currentData[primaryField], parentKeys, splitFields.join('.'), referenceType);
        } 
    }
}
...
DataPacksExpand.prototype.expandFile = async function(targetPath, expandFile, jobInfo) {
    try {
        ...
        VlocityUtils.error('Invalid DataPackFile ', expandFile, e.message);
    }
};
