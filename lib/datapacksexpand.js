...
DataPacksExpand.prototype.removeDuplicates = function(listData) {
    ...
    for (var i = 0; i < listData.length; i++) {
        if (listData[i].VlocityRecordSourceKey && !alreadyExisting[listData[i].VlocityRecordSourceKey]) {
            newList.push(listData[i]);
            alreadyExisting[listData[i].VlocityRecordSourceKey] = true;
        }
    }
    ...
    if (currentData) {
        ...
        if (currentData.VlocityDataPackData) {
            ...
            if (Array.isArray(currentData)) {
                ...
                currentData = self.sortList(currentData, dataPackType);
    ...
    if (fileType == "json") {
        if (typeof fileData === "object") {
            ...
            try {
                ...
                VlocityUtils.error('JSON Parsing Failed', filename + "." + fileType, e.message);
                return fileData;
            }
        }
