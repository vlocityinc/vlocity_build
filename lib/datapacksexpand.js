...
const DataPacksExpand = module.exports = function (vlocity) {
...
DataPacksExpand.prototype.getNameWithFields = function (nameFields, dataPackData) {
  ...
  if (processedNames.length === 0 || !processedNames[0]) {
    if (dataPackData.Name && typeof dataPackData.Name === 'string') { return dataPackData.Name }
  } else {
...
DataPacksExpand.prototype.filterNestedField = function (currentData, filterField) {
  try {
    if (filterField.indexOf('.') !== -1) {
      ...
      var primaryField = splitFields[0]

      if (typeof currentData[primaryField] === 'string') {
        var nestedJson = JSON.parse(currentData[primaryField])

        if (!Array.isArray(nestedJson)) {
          delete nestedJson[splitFields[1]]
        }

        currentData[primaryField] = JSON.stringify(nestedJson)
      }
    }
  } catch (e) {
      ...
      if (Array.isArray(currentData)) {
        ...
        if (filterFields) {
          filterFields.forEach(function (field) {
            delete currentData[field]
            self.filterNestedField(currentData, field)
          })
        }
  ...
  if (isPagination) {
    try {
      if (Array.isArray(fileData)) {
        let previousFileData = JSON.parse(fs.readFileSync(fullFilePath, { 'encoding': encoding }))
