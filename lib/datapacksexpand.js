        ...
        if (currentData[referencefield]) {
            ...
            if (Array.isArray(currentData[referencefield])) {
                ...
                if(!(jobInfo.maxDepth === 0) || jobInfo.includeSalesforceMetadata){
                    ...
                    let referenceInfo = await this.vlocity.datapacksutils.handleDataPackEvent('onAutoExport', referenceType, currentData);
                    if(referenceInfo && jobInfo.specificManifestKeys.indexOf(referenceInfo) === -1){
                        jobInfo.specificManifestKeys.push(referenceInfo);
                    }
                }
