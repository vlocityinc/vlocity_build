...
DataPacksExpand.prototype.generateFolderPath = function(dataPackType, parentName) {
    ...
    // dataPackType comes from untrusted DataPack content and is not sanitized; assert the
    // resolved folder stays under targetPath so a crafted VlocityDataPackType can't escape it.
    var folderPath = path.normalize(path.join(this.targetPath, dataPackType, validParentName, path.sep));
    this.assertPathWithin(this.targetPath, folderPath);
    return folderPath;
};
...
DataPacksExpand.generateFolderOrFilename = function(filename, extension) {
    ...
    if (basename.length > MAX_FILE_LENGTH) {
        basename = basename.substr(basename.length - MAX_FILE_LENGTH);
        sanitizedFilename = path.join(path.dirname(sanitizedFilename), basename);
    }
...
// Opt-in via enforceSecurityValidations (default off) so customers can validate the guard in a
// sandbox by enabling it through job YAML / CLI before it becomes the default in a later release.
DataPacksExpand.prototype.assertPathWithin = function(baseDir, targetPath) {
    ...
    if (this.enforceSecurityValidations === true &&
        resolvedTarget !== resolvedBase && !resolvedTarget.startsWith(resolvedBase + path.sep)) {
        // Some callers swallow expansion errors, so log before throwing to leave an audit trail.
        VlocityUtils.error('Path Traversal Blocked', 'Refusing to access path outside target directory', targetPath);
        throw new Error('Path traversal detected: refusing to write outside target directory: ' + targetPath);
    }
    ...
    if (listData.length > 0 && typeof listData[0] === "object" && listData[0].VlocityRecordSObjectType) {
        ...
        if (listData.length < 1000) {
            ...
            listData.forEach(item => {
                item.listSortKey = self.generateListSortKey(item, sortFields);
            });
    
            listData.sort(function(a, b) {
                return (a.listSortKey > b.listSortKey) ? 1 : -1;
            });
        }

        listData.forEach(item => {
            if (item) {
                delete item.listSortKey;
...
DataPacksExpand.prototype.generateListSortKey = function(obj1, fieldsArray) {
    var listSortKey = '|';

    for (var fieldsArrayIndex = 0; fieldsArrayIndex < fieldsArray.length; fieldsArrayIndex++) {
        try {
            ...
            if (obj1Data !== 0 && !obj1Data) {
                ...
                obj1Data = String(obj1Data);
            }

            if (obj1Data.length < 30) {
                obj1Data.padStart(30 - obj1Data.length, '|');
            }

            listSortKey += obj1Data;
        } catch (e) {
            listSortKey += '|';
        }
    }
    ...
    if (currentData) {
        ...
        if (currentData.VlocityDataPackData) {
            ...
            if (Array.isArray(currentData)) {
                ...
                if (self.vlocity.datapacksutils.ensureGlobalKey(dataPackType, currentData.VlocityRecordSObjectType)) {
                    if (currentData['%vlocity_namespace%__GlobalKey__c'] === "") {
                        currentData['%vlocity_namespace%__GlobalKey__c'] = self.vlocity.datapacksutils.guid();
                    } else if (currentData['GlobalKey__c'] === "") {
                        currentData['GlobalKey__c'] = self.vlocity.datapacksutils.guid();
                    }
                }
                ...
                if (currentId) {
                    for (var sobjectField in currentData) {
                        if (typeof currentData[sobjectField] === "object"
                        && currentData[sobjectField].VlocityMatchingRecordSourceKey) {
                            if (`VlocityRecordSourceKey:${currentId}` == currentData[sobjectField].VlocityMatchingRecordSourceKey) {
                                delete currentData[sobjectField];
                                VlocityUtils.error('Deleting Self Reference', sobjectField);
                            }
                        }
                    }
                }
        ...
        if (currentData.VlocityDataPackData) {
            ...
            if (Array.isArray(currentData)) {
                ...
                if (currentData.VlocityDataPackType == 'VlocityLookupMatchingKeyObject' || currentData.VlocityDataPackType == 'VlocityMatchingKeyObject') {
                    ...
                    if (jobInfo.sourceKeysByParent[currentData.VlocityRecordSourceKey] && jobInfo.sourceKeysByParent[currentData.VlocityRecordSourceKey].indexOf(parentKey) == -1) {
    ...
    if (dataPackData.VlocityDataPackData) {
        ...
        if (dataField && dataPackData.VlocityDataPackData[dataField]) {
            ...
            for (var sobjectData of dataPackData.VlocityDataPackData[dataField]) {
                ...
                try {
                    if (!isPagination) {
                        let incomingFolder = self.generateFolderPath(dataPackType, parentName);
                        await fs.ensureDir(incomingFolder);
                        await fs.emptyDir(incomingFolder);
                    }
                } catch (e) {
...
DataPacksExpand.prototype.writeFile = async function(dataPackType, parentName, filename, fileType, fileData, isPagination, encoding) {
    ...
    if (isPagination) {
        try {
            ...
            VlocityUtils.error('Error in Pagination', e.stack);
        }
    }

    if (fileType == "json") {
        if (typeof fileData === "object") {
            fileData = stringify_pretty(fileData, { space: 4 });
        } else {
            try {
                // There is an issue with sometimes strange escape chars in sample JSON         
                fileData = stringify_pretty(JSON.parse(fileData.replace(/&amp;quot;/g, '').replace(/&quot;/g, '"').replace(/”|“/g, '"')), { space: 4 });
            } catch (e) {
    ...
    let fileReturnName = self.generateFolderOrFilename(filename, fileType);
 
    if (fileReturnName == fileData) {
        VlocityUtils.error('Bad File', 'File data is not being written as it contains no real data', fileReturnName);
        return "";
    }

    // There is some error occurring where extra characters are added to a file.
    if (fileType == "json") {
        var tries = 10;
        for (var i = 0; i < tries; i++) {
            try {
                await fs.outputFile(fullFilePath, fileData, { "encoding": encoding });
                JSON.parse(await fs.readFile(fullFilePath));
                break;
            } catch (e) {
                if (i == (tries-1)) {
                    VlocityUtils.error('Error Creating File', path.normalize(fullFilePath));
                } else {
                    VlocityUtils.verbose('Potential Error Creating File', path.normalize(fullFilePath));
                }   
            }
        }
    } else {
...
DataPacksExpand.prototype.expand = async function(targetPath, dataPackData, jobInfo) {
    ...
    this.enforceSecurityValidations = jobInfo.enforceSecurityValidations;
