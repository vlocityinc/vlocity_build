...
DataPacksExpand.prototype.getNameWithFields = function(nameFields, dataPackData) {
    ...
    var processedNames = (nameFields || []).map(function(key) {
        ...
        if (key.indexOf('_') == 0) {
            ...
            let sObjDataRecordType = '';
            if (jobInfo.upgradeDataPackFields && dataPackData[key].VlocityRecordSObjectType === '%vlocity_namespace%__OmniScript__c') {
                sObjDataRecordType = 'OmniScript__c';
            } else {
                sObjDataRecordType = dataPackData[key].VlocityRecordSObjectType;
            }
            return self.getDataPackFolder(null, sObjDataRecordType, dataPackData[key]);
        }
    });    
        ...
        if (currentData.VlocityDataPackData && currentData.VlocityDataPackData[dataField]) {
            ...
            for (var sobjectData of currentData.VlocityDataPackData[dataField]) {
                if (sobjectData) {
                    ...
                    let sObjDataRecordType = '';
                    if (jobInfo.upgradeDataPackFields && sobjectData.VlocityRecordSObjectType === '%vlocity_namespace%__OmniScript__c') {
                        sObjDataRecordType = 'OmniScript__c';
                    } else {
                        sObjDataRecordType = sobjectData.VlocityRecordSObjectType;
                    }
                    var parentName = self.getDataPackFolder(dataPackType, sObjDataRecordType, sobjectData);
    ...
    if (dataPackData.VlocityDataPackData) {
        ...
        if (dataField && dataPackData.VlocityDataPackData[dataField]) {
            ...
            for (var sobjectData of dataPackData.VlocityDataPackData[dataField]) {
                ...
                var parentName = self.getDataPackFolder(dataPackType, sObjDataRecordType, sobjectData);
                var dataPackName = self.getDataPackName(dataPackType, sObjDataRecordType, sobjectData);
