...
DataPacksExpand.prototype.buildHierarchyKey = function(listDataBySourceKey, sortFields, listRec) {
    var self = this;
    ...
    if (!listRec.HierarchyKey) {
        ...
        if (!listRec[parentField]) {
            ...
            if (listDataBySourceKey[parentRecKey]) {

                var parentHierarchyKey = self.buildHierarchyKey(listDataBySourceKey, sortFields, listDataBySourceKey[parentRecKey]);
        ...
        if (currentData.VlocityDataPackData) {
            ...
            if (Array.isArray(currentData)) {
                ...
                if (currentData.Name 
                    ...
                    currentData.Name = self.vlocity.datapacksutils.guid();
                }
        ...
        if ((!jobInfo.manifestOnly || self.vlocity.datapacksutils.isInManifest(dataPackData.VlocityDataPackData, jobInfo.manifest))) {
            ...
            if (dataField) {
                ...
                dataPackData.VlocityDataPackData[dataField].forEach(function(sobjectData) {
                    ...
                    dataPackData.VlocityDataPackParents.forEach(function(parentKey) {
                        if (jobInfo.allParents.indexOf(parentKey) != -1) {
                            ...
                            jobInfo.sourceKeysByParent[parentKey].forEach(function(ultimateParent) {
                                ...
                                if (jobInfo.vlocityKeysToNewNamesMap[ultimateParent]) {
                                    allParentKeys.push(jobInfo.vlocityKeysToNewNamesMap[ultimateParent]);
                                }
                            });
                        } else if (jobInfo.vlocityKeysToNewNamesMap[parentKey]) {
                    ...
                    self.processDataPackData(dataPackType, parentName, null, sobjectData, isPagination, jobInfo);
...
DataPacksExpand.prototype.processDataPackData = function(dataPackType, parentName, filename, dataPackData, isPagination, jobInfo) {
    var self = this;
                    ...
                    if (expansionData) {
                        if (expansionType == "list") {
                            ...
                            expansionData.forEach(function(childInList) {
                                listExpansion.push(self.processDataPackData(dataPackType, parentName, packName, childInList, isPagination, jobInfo));
                            });
    ...
    if (extension === 'scss') {
        ...
        if (found) {
            for (var importName of found) {
                ...
                referencedDataPacksNames[dataPackName] = '';
            }
        }
    }

    if (!this.vlocity.utilityservice.isEmptyObject(referencedDataPacksNames)) {
        if (!jobInfo.referencedDataPacks) {
            jobInfo.referencedDataPacks = {};
        }

        if (!jobInfo.referencedDataPacks[dataPackType]) {
            jobInfo.referencedDataPacks[dataPackType] = {};
        }

        for (var dataPackName in referencedDataPacksNames) {
            jobInfo.referencedDataPacks[dataPackType][dataPackName] = '';
        }
    }
};
...
DataPacksExpand.prototype.expandFile = function(targetPath, expandFile, jobInfo) {
    try {
        ...
        this.expand(targetPath, { dataPacks: expandPacks }, jobInfo);
...
DataPacksExpand.prototype.expandSFDX = function(dataPack, jobInfo) {
    if (dataPack.SFDXData) {
        ...
        fs.outputFileSync(fullFilePath, dataPack.SFDXData, { "encoding": dataPack.SFDXDataType });
    }
}

DataPacksExpand.prototype.expand = function(targetPath, dataPackData, jobInfo) {
    var self = this;
    ...
    if (dataPackData.dataPacks) {
        ...
        dataPackData.dataPacks.forEach(function(dataPack) {
            self.expandSFDX(dataPack, jobInfo);
        });
