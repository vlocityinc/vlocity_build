...
DataPacksExpand.prototype.getNestedDataPackReferences = function(currentData, parentKeys, referencefield, referenceType) {
    ...
    if (!referencefield.includes('.')) {
        ...
        if (Array.isArray(currentData[primaryField])) {
            for (var nestedMore of currentData[primaryField]) {
                this.getNestedDataPackReferences(nestedMore, parentKeys, splitFields.join('.'), referenceType);
            } 
        } else {
            this.getNestedDataPackReferences(currentData[primaryField], parentKeys, splitFields.join('.'), referenceType);
        } 
    }
}
                        ...
                        if (reference.Field.indexOf('.') != -1) {
                            try {
                                ...
                                if (currentData[primaryField]) {
                                    self.getNestedDataPackReferences(JSON.parse(currentData[primaryField]), parentKeys, splitFields.join('.'), reference.Type);
                                }
                            } catch (e) {
                                VlocityUtils.error('Failure in nested parsing', e);
                            }
    ...
    if (dataPack.SFDXData) {
        ...
        if (dataPack.SFDXDataType == 'staticresources') {
            ...
            for (resourceFile of allFiles) {
                await fs.outputFile(path.join(jobInfo.sfdxFolderPath, resourceFile.filePath), resourceFile.fileData, { "encoding": resourceFile.encoding });
            }
        } else {
