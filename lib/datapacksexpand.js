                ...
                if (expansionType && !doNotExpand) {
                    ...
                    if (expansionData) {
                        if (expansionType == "list") {
                            ...
                            self.addDataPackToFullManifest(extension, dataPackType, expansionData);
                        }
                    } 
                }
            });
...
DataPacksExpand.prototype.addDataPackToFullManifest = function(extension, dataPackType, expansionData) {
    var self = this;

    if (extension === 'scss'
    && self.jobInfo.compileOnBuild 
    && self.jobInfo.maxDepth != 0) {
        var regex = new RegExp("(?:@import \")(.*?)(?:;)", "ig");
        var found = expansionData.match(regex);
        var templates = [];

        for (var key in self.jobInfo.allDataSummary) {
            if (self.jobInfo.allDataSummary[key].VlocityDataPackType === dataPackType) {
                templates.push(self.jobInfo.allDataSummary[key].VlocityDataPackDisplayLabel);
            }
        }

        if (found) {
            for(var item of found) {
                var importName = item.split("\"")[1];
                
                if (!templates.includes(importName)) {
                    if (!self.jobInfo.fullManifest[dataPackType]) {
                        self.jobInfo.fullManifest[dataPackType] = {};
                    }

                    self.jobInfo.fullManifest[dataPackType][importName] = {VlocityDataPackType: dataPackType, Id: importName};
                }
            }
        }
    }
