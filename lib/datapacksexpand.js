        ...
        if ((!jobInfo.manifestOnly || self.vlocity.datapacksutils.isInManifest(dataPackData.VlocityDataPackData, jobInfo.manifest))) {
            ...
            if (dataField) {
                ...
                for (var sobjectData of dataPackData.VlocityDataPackData[dataField]) {
                    ...
                    await this.processDataPackData(dataPackType, parentName, null, sobjectData, isPagination);
...
DataPacksExpand.prototype.processDataPackData = async function(dataPackType, parentName, filename, dataPackData, isPagination) {
    var self = this;
                    ...
                    if (expansionData) {
                        if (expansionType == "list") {
                            ...
                            for (var childInList of expansionData) {
                                listExpansion.push(await self.processDataPackData(dataPackType, parentName, packName, childInList, isPagination));
                            }
...
DataPacksExpand.prototype.expand = async function(targetPath, dataPackData, jobInfo) {
    ...
    if (dataPackData.dataPacks) {
        VlocityUtils.report('Start Expand');
        var expandPromises = [];
        ...
        for (var dataPack of dataPackData.dataPacks) {
            this.preprocessDataPack(dataPack, jobInfo);
        }
        ...
        VlocityUtils.report('Expanding All');
        var processPromises = [];
        for (var dataPack of dataPackData.dataPacks) {
