...
Vlocity.getComparisonInfo = function(options) {
    ...
    if (sourceCompFolder == 'local') {
        sourceCompFolder = options.projectPath.replace(new RegExp('/', 'g'), '_');
    }
...
Vlocity.migrate = async function(options) {
    ...
    if (!comparisonInfo.hasComparison && options.autoDeployDependencies) {
        VlocityUtils.report('No Comparison Found - Required for Deploy With Dependencies');
...
Vlocity.compare = async function(options) {
    ...
    if (options.source == 'local') {
        if (!options.workingSetKeys) {
            ...
            if (typeof options.workingSetKeys === 'string') {
                options.workingSetKeys = JSON.parse(options.workingSetKeys);
            }
    
            var allParentsByKey = {};
            options.manifest = options.workingSetKeys;
            for (var dataPack of options.sourceData.dataPacks) {
    ...
    if (options.source == 'local') {
        ...
        for (var dataPack of options.sourceData.dataPacks) {
            if (options.manifest.indexOf(dataPack.VlocityDataPackKey) != -1) {
