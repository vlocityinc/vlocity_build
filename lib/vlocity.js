...
Vlocity.removeDiscardedSObjects = function(dataPackData) {
    if (dataPackData.dataPacks) {
        dataPackData.dataPacks.forEach(dataPack => {
            Vlocity.removeDiscardedSObjects(dataPack);
        });
...
Vlocity.revertChange = async function(options) {
    ...
    if (options.revertByKey) {
        if (typeof options.revertByKey === 'object') {
    ...
    for (var revertRecord of revertByKey) {
        ...
        } else if (revertRecord.specialRevertHandler) {
    ...
    if (options.discardSObjectByKey && !Array.isArray(options.discardSObjectByKey)) {
        options.discardSObjectByKey = [ options.discardAllByKey ];
    ...
    for (var discardSObject of (options.discardSObjectByKey || [])) {
        var diffsObject = allDiffsBySourceKey[discardSObject.VlocityRecordSourceKey];
       
        if (diffsObject.diffType === 'New') {
        ...
        } else if (diffsObject.diffType === 'Deleted') {
            var deletedObject = allDataPacksBySourceKeyTarget[discardSObject.VlocityRecordSourceKey];
            var parentObject = allDataPacksBySourceKeyTarget[diffsObject.VlocityDiffParentKey];

            if (!parentObject[diffsObject.VlocityDiffParentField]) {
                parentObject[diffsObject.VlocityDiffParentField] = [];
            }

            parentObject[diffsObject.VlocityDiffParentField].push(deletedObject);
        }
    }
    ...
    if (options.refreshSource) { 
        ...
        if (options.source != 'local') {
            ...
            if (options.allowChildProcesses 
                ...
                await Vlocity.runDataPacksCommand('RetrieveSalesforce', JSON.parse(JSON.stringify(refreshOptionsSource)));
