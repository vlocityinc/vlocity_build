...
Vlocity.getEnvironmentInfo = function(environment, options) {
    ...
    if (environment == 'local') {
        passedInOptions.useProjectPathForSFDX = true;
    } else {
...
Vlocity.getComparisonInfo = function(options) {
    ...
    var targetCompFolder = options.target.replace('.', '_').replace('@', '_');

    if (sourceCompFolder == 'local') {
        sourceCompFolder = options.projectPath.replace(new RegExp('/|:', 'g'), '_').replace();
    }

    if (targetCompFolder == 'local') {
        targetCompFolder = options.projectPath.replace(new RegExp('/|:', 'g'), '_');
    }
...
Vlocity.migrate = async function(options) {
    ...
    if (options.applyByKey) {
        await Vlocity.revertChange(options);
        await Vlocity.compare(options);
    }
    ...
    try {
        ...
        for (var data of diffsFile.records) {
            ...
            for (var record of (data.SObjects || [])) {
                for (var diff of (record.fieldDiffs || [])) {
                    ...
                    if (allDiffsBySourceKey[diff.diffKey]) {
                        VlocityUtils.error('Duplicate Diff Key?', diff.diffKey);
                    }

                    if (diff.status != "Unchanged" && diff.status != "Title" && !diff.readOnly) {
                        diff.ParentDiffStatus = record.diffType;
                        allDiffsBySourceKey[diff.diffKey] = diff;
                    }
                }
            }
        }

        if (typeof options.applyByKey == 'string') {
            options.applyByKey = JSON.parse(options.applyByKey);
        }

        var revertByKey = [];

        if (options.applyByKey) {
            if (typeof options.applyByKey === 'object' && options.applyByKey[0]) {
                for (var key of Object.keys(allDiffsBySourceKey)) {

                    if (options.applyByKey.indexOf(key) == -1) {
                        revertByKey.push(allDiffsBySourceKey[key]);
                    }
                }
            }

            if (revertByKey.length == 0) {
        ...
        for (var revertRecord of revertByKey) {
            ...
            if (revertSObject) {
                ...
                } else if (revertRecord.field.indexOf('DisplayCodeText') != -1 || revertRecord.field == 'SFDXData') {
        ...
        for (var overwritingSObject of (options.editByKey || [])) {
            ...
            if (overwritingSObject.VlocityRecordEditField) {
                currentSObject[overwritingSObject.VlocityRecordEditField] = overwritingSObject.DisplayCodeText;
...
Vlocity.compare = async function(options) {
    ...
    if (!fs.existsSync(comparisonInfo.sourceFileCurrent) || options.resetDiffs) {
        ...
        options.sourceData = await Vlocity.getDataPacks(Vlocity.getEnvironmentInfo(buildSourceFileOptions.source, buildSourceFileOptions));
