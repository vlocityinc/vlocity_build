...
var omnistudio = require('./omnistudio.js');

VLOCITY_BUILD_VERSION = require('../package.json').version;

VLOCITY_SALESFORCE_JSFORCE_VERSION = '52.0';
...
var Vlocity = module.exports = function(options) {
    ...
    this.credentialsFolder = options.credentialsFolder || this.tempFolder;
    ...
    this.salesforceApiVersion = options.salesforceApiVersion || VLOCITY_SALESFORCE_JSFORCE_VERSION;
    ...
    this.omnistudio = new omnistudio(this);
};
...
Vlocity.runDataPacksCommandInChildProcess = async function(action, options) {
    let promise = await new Promise((resolve) => {
        ...
        try {
            ...
            VlocityUtils.error('Error in Child Process', e);
        }
...
Vlocity.runDataPacksCommand = async function(action, options) {
    try {
        ...
        var loginError = await vlocity_run.checkLogin(options);
        ...
        VlocityUtils.error('Login Error', e);
        throw e;
    }
}
...
Vlocity.getEnvironmentInfo = function(environment, options, sourceOrTarget) {
    ...
    if (environment == 'local') {
        if (sourceOrTarget === 'source') {
            ...
            passedInOptions.sfdxJSONFolderPath = passedInOptions.targetSFDXFolderPath;
        }
    } else {
    ...
    if (fs.existsSync(comparisonInfo.diffsFileCurrent)) {
        ...
        do {
            ...
            for (var dataPackKey of options.manifest) {
                if (currentDataPacks[dataPackKey]) {
                    if (currentDataPacks[dataPackKey].VlocityDataPackType === 'Project') {
                        let tempKeysArr = Object.keys(currentDataPacks).filter(key => {
                            return (existingComparison[key] && existingComparison[key].DiffType != 'Unchanged');
                        });
                        options.manifest = Array.from(new Set((options.manifest).concat(tempKeysArr)));
                        previousManifestLength = options.manifest.length;
                        break;
                    } else if (currentDataPacks[dataPackKey].VlocityDataPackParents && dataPackKey.indexOf('lwc/') !== 0) {
                        ...
                        if (dataPackKey.includes('lwc/') && dataPackKey.includes('.html')) {
                            let strArr = currentDataPacks[dataPackKey].SFDXData.split('<');
                            strArr.forEach(content => {
                                if (content.indexOf('c-') === 0) {
                                    let manifestKey = `lwc/${(content.split('>')[0]).split(' ')[0].split('\n')[0].split('-')[1]}`;
                                    Object.keys(currentDataPacks).forEach(key => {
                                        if (key.includes(manifestKey) && !options.manifest.includes(key)) {
                                            VlocityUtils.log('Adding Dependency', key);
                                            options.manifest.push(key);
                                        }
                                    });
                                }
                            });
...
Vlocity.migrate = async function(options) {
    ...
    if (options.target == 'local') {
        ...
        let allDataPacks = JSON.parse(fs.readFileSync(comparisonInfo.sourceFileCurrent, 'utf8'));
        
        if (allDataPacks.vdxnamespace && [ 'vlocity_cmt', 'vlocity_ins', 'vlocity_ps' ].indexOf(allDataPacks.vdxnamespace) != -1) {
            options.vdxnamespace = allDataPacks.vdxnamespace;
        }
        ...
        let sourceSassFolder;
        if (options.source == 'local') {
            sourceSassFolder = path.join(options.sourceProjectPath, options.sourceExpansionPath, 'VlocityUITemplate');
        } else {
            sourceSassFolder = path.join(options.tempFolder, options.source, 'project', 'datapacks', 'VlocityUITemplate');
        }

        if (fs.existsSync(path.join(sourceSassFolder))) {
            options.additionalSassCompilationIncludePaths = datapacksutils.getDirectories(sourceSassFolder).map(function(dir) {
                return path.join(sourceSassFolder, dir);
            });
        }
    ...
    try {
        ...
        for (var revertRecord of revertByKey) {
            ...
            if (revertRecord.field.includes('.')) {
                ...
                VlocityUtils.verbose('Reverted String', revertedString);
                ...
                VlocityUtils.verbose('revertedObject', revertSObject);
            } else {
                ...
                if (discardSObject.VlocityRecordEditField) {
                    ...
                    if (deletedObject && parentObject) {
                        ...
                        if (!vlocityTemp) {
                            ...
                            await vlocityTemp.checkLogin(options);
                        }
        ...
        for (var dataPack of comparisonFileSource.dataPacks) {
            ...
            var hashCodeGenerated = await datapacksutils.handleStaticDataPackEvent('generateHashCodeData', dataPack.VlocityDataPackType, { dataPack });

            if (!hashCodeGenerated) {
                hashCodeGenerated = dataPack[dataKey];
            }

            dataPack.dataHashCode = datapacksutils.hashCode(stringify(hashCodeGenerated));
        }
        ...
        VlocityUtils.error('Revert Error', e.stack)
...
Vlocity.compare = async function(options) {
    ...
    if (options.manifest) {
        ...
        options.manifest = options.manifest.filter((key) => { return key && typeof key == "string"; });
    }
    ...
    if (options.projectKeys) {
        ...
        options.projectKeys = options.projectKeys.filter((key) => { return key && typeof key == "string"; });
    }
    ...
    if (options.source == 'local' && options.projectKeys) {
        ...
        do {
            var previousManifestKeyTotal = options.projectKeys.length;

            for (var manifestKey of options.projectKeys) {
                if (Array.isArray(allParentsByKey[manifestKey])) {
                    for (var parentKey of allParentsByKey[manifestKey]) {
                        if (options.projectKeys.indexOf(parentKey) == -1) {
                            options.projectKeys.push(parentKey);
                        }
                    }
                }
            }
        } while (previousManifestKeyTotal != options.projectKeys.length)

        var limitedSourceData = [];

        options.projectKeys = options.projectKeys.concat(datapacksexpand.addAdditionalMetadataKeysForSFDX(options.projectKeys));

        var folderMetadataForKeys = [ 'aura', 'lwc' ];

        for (var dataPack of options.sourceData.dataPacks) {   
            var dataPackKeyCheck = dataPack.VlocityDataPackKey;
            var fullKey = dataPackKeyCheck.split('/');
            
            if (fullKey.length > 2 && folderMetadataForKeys.includes(fullKey[0])) {
                dataPackKeyCheck = `${fullKey[0]}/${fullKey[1]}`;
            }

            if (options.projectKeys.includes(dataPackKeyCheck) 
                || options.projectKeys.includes(datapacksutils.withoutMetadataExtension(dataPackKeyCheck))
                || options.projectKeys.includes(dataPackKeyCheck + '-meta.xml')) {
                limitedSourceData.push(dataPack);

                if (!options.projectKeys.includes(dataPack.VlocityDataPackKey)) {
                    options.projectKeys.push(dataPack.VlocityDataPackKey);
                }

                if (!options.projectKeys.includes(dataPackKeyCheck)) {
                    options.projectKeys.push(dataPackKeyCheck);
                }
            }
        }
    ...
    if (!options.selectedOnly) {
        ...
        for (var dataPack of options.sourceData.dataPacks) {  
            let asManifestKey = datapacksutils.getAsManifestInfo(dataPack);
            
            options.manifest.push(asManifestKey.key);
        }
    }
    ...
    if (!fs.existsSync(comparisonInfo.targetFileCurrent) || options.resetDiffs) {
        ...
        if (targetBuildFileOptions.target != 'local') {
            ...
            targetBuildFileOptions.manifest = [];
            for (var dataPack of options.sourceData.dataPacks) {  
                targetBuildFileOptions.manifest.push(dataPack.VlocityDataPackKey);
            }
            targetBuildFileOptions.useProjectPathForSFDX = true;
        }
        ...
        if (Object.keys(options.targetData).length === 0) {
...
Vlocity.prototype.checkLogin = async function(options) {
    return await this.utilityservice.checkLogin(options);
}