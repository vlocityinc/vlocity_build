...
Vlocity.migrate = async function(options) {
    ...
    if (options.target == 'local') {
        ...
        let obj = await Vlocity.runDataPacksCommand('checkOmniScriptAvailability', options);
        if (!obj.data.isOmniScriptAvailable) {
            options.autoUpdateSettings = false;
            options.upgradeDataPackFields = true;
        }
