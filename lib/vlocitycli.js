...
const vlocity = require('./vlocity.js')
const properties = require('properties')
const stringify = require('json-stable-stringify')
const VlocityUtils = require('./vlocityutils.js')
const VLOCITY_BUILD_VERSION = require('../package.json').version

const VLOCITY_COMMANDLINE_OPTIONS = {
  'activate': Boolean,
...
const VLOCITY_COMMANDLINE_OPTIONS_SHORTHAND = {
  'depth': [ '-maxDepth' ],
  'js': [ '-javascript' ],
...
const VLOCITY_COMMANDLINE_OPTIONS_DEFAULTS = {
  tempFolder: './vlocity-temp/',
  source: 'local',
  target: 'local'
}

const VLOCITY_COMMANDLINE_COMMANDS = {
  packExport: {
    name: 'Export',
...
const VlocityCLI = module.exports = function () {
...
VlocityCLI.prototype.formatResponseCLI = function (response) {
  ...
  if (self.responseFormat === 'JSON') {
    ...
    if (response instanceof Error) {
      return { exitCode: 1, print: stringify({ status: 'error', message: response.stack ? response.stack : response.message, result: response }, self.isJsonPretty ? { space: 4 } : {}) }
    } else if (typeof response === 'string') {
      return { exitCode: 1, print: stringify({ message: response, status: 'error', result: null }, self.isJsonPretty ? { space: 4 } : {}) }
    } else {
      return { exitCode: 1, print: stringify(response, self.isJsonPretty ? { space: 4 } : {}) }
    }
...
VlocityCLI.runCLI = async function () {
  ...
  const commands = commandLineOptions.argv.remain

  let vlocityCli = new VlocityCLI()

  try {
    let result = await vlocityCli.runCommands(commands, commandLineOptions)
