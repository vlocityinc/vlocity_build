...
const analytics = new Analytics.default("UA-149957165-1");
...
VlocityCLI.prototype.runCommands = async function(commands, options) {
    ...
    if (commands[0] == 'runTestJob') {
        if(!dataPacksJobData.disableAnalytics){
            self.updateAnalytics('runTestJob');
        }
...
VlocityCLI.prototype.updateAnalytics = function (eventAction) {
    if (!analytics.customParams.uid) {
        analytics.set('uid', machineIdSync.machineIdSync());
    }
    analytics.event("vbt", eventAction, {
        clientID: analytics.customParams.uid
    });
}
