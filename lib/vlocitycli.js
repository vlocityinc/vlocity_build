...
VlocityCLI.prototype.runTestJob = async function(dataPacksJobData) {
    ...
    for (var command of commands) {
        ...
        if (command == 'DeployManifest') {
            ...
            command = "Deploy";
            testJobData.queries = null;
            testJobData.manifest = '[ "staticresources/VBTTestResource.resource", "classes/ApplicationUtilities.cls", "objects/Account/fields/Last_Paid_Amount__c.field" ]';
            testJobData.manifestOnly = true;
            testJobData.includeSalesforceMetadata = true;
            testJobData.sfdxForcePath = path.join(__dirname, '..', 'test', 'salesforce_sfdx');
