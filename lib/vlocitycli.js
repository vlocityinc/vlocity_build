...
VlocityCLI.prototype.runTestJob = async function(dataPacksJobData) {
    ...
    for (var command of commands) {
        ...
        if (command == 'DeployManifest') {
            ...
            testJobData.queries = null;
            testJobData.manifest = { IntegrationProcedure: [ 'DataPackTest_IPCondBlock1', 'DataPackTest_IPTest1' ]};
            testJobData.manifestOnly = true;
            command = 'Deploy';
            result = await self.runJob(testJobData, command);
            if(result.status === 'success'){
                testJobData = JSON.parse(JSON.stringify(dataPacksJobData));
                testJobData.resultWithDetails = true;
                testJobData.testKeys = ["DataPackTest_IPTest1"];
                result = await self.runJob(testJobData, 'runTestProcedure');
                if (result.data.testResults[0].trackingEntry.length != 10) {
                    result.status = 'error';
                }
            }
        }
