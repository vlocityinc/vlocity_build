...
DataPacksJob.prototype.doRunJob = async function(jobInfo, action) {
    ...
    } else if (action === 'checkFoundationOrg'){
        await this.checkFoundationOrg(jobInfo);
    } else if (action === 'checkDuplicateFiles') {
        this.checkDuplicateFiles(jobInfo);
    } else {
...
DataPacksJob.prototype.formatResponse = async function(jobInfo, error) {
    ...
    try {
        ...
        if (jobInfo.jobAction == 'GetAllAvailableExports') {
            ...
            response.records = jobInfo.data;
            response.message = jobInfo.errors.length != 0 ? jobInfo.errors.join('\n') : `Success - ${jobInfo.data.length} Records`;
            response.status = jobInfo.hasError ? 'error' : 'success';
...
DataPacksJob.prototype.downloadPerformanceData = async function (jobInfo) {
    ...
    if (fs.existsSync(jobInfo.projectPath)){
        existingRecords = JSON.parse(await fs.readFileSync(jobInfo.projectPath,'utf-8'));
    ...
    return new Promise(async (resolve, reject) => {
        ...
        var query = `SELECT ${field_name},CreatedBy.Username,CreatedBy.Profile.Name FROM ${this.vlocity.namespacePrefix}VlocityTrackingEntry__c WHERE Name in ('StepActionTime', 'UI Action', 'OS Step Unload') AND ${this.vlocity.namespace}__TrackingService__c in ('IntProc', 'OmniScript') ${timestampFilter} ORDER BY CreatedDate DESC`;
        ...
        for (const obj of jobInfo.queries) {
            if (obj.VlocityDataPackType === jobInfo.type) {
                ...
                if (results.records[0] && results.records[0].Id) {
                    ...
                    open(this.vlocity.jsForceConnection.instanceUrl + this.vlocity.utilityservice.getVisualForcePageUrl(jobInfo.type, results.records[0]), { wait: false });
                } else {
...
DataPacksJob.prototype.checkFoundationOrg = async function(jobInfo) {
    try {
        await this.vlocity.jsForceConnection.query(`SELECT Id FROM ${this.vlocity.namespacePrefix}OmniProcess__c`);
        jobInfo.data = {
            isFoundation: true
        };
    } catch(err) {
...
DataPacksJob.prototype.checkDuplicateFiles = function (jobInfo) {
    try {
        let files = this.vlocity.utilityservice.getFiles(jobInfo.inputDir);
        let errorMessages = [];
        let warningMessages = [];
        let errorFileExtensions = ['cls', 'trigger', 'page'];
        let warningFilesExtensions = ['component', 'flexipage', 'object', 'resource', 'tab', 'remoteSite'];
        let ignoredFilesCount = 0;
        Object.keys(files).forEach(key => {
            if (files[key].length > 1) {
                if (errorFileExtensions.includes(key.split('.')[1])) {
                    errorMessages.push(this.vlocity.utilityservice.getDuplicateFilesErrorMessages(key, files[key], jobInfo.inputDir));
                } else if (warningFilesExtensions.includes(key.split('.')[1])) {
                    warningMessages.push(this.vlocity.utilityservice.getDuplicateFilesErrorMessages(key, files[key], jobInfo.inputDir));
                } else {
                    ignoredFilesCount++;
                }
            }
        ...
        if (errorMessages.length) {
            VlocityUtils.error(`Error: ${errorMessages.length} duplicate files`, errorMessages.sort());
        }
        if (warningMessages.length) {
            VlocityUtils.warn(`Warning: ${warningMessages.length} duplicate files`, warningMessages.sort());
        }
        if (ignoredFilesCount) {
            VlocityUtils.report(`${ignoredFilesCount} duplicate files ignored`);
        }
    } catch (err) {
        VlocityUtils.error('An Error Occured', err);
    }
}