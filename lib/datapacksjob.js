...
DataPacksJob.prototype.convertToCore = async function (jobInfo) {
    ...
    let platformObjectMappings = yaml.safeLoad(fs.readFileSync(path.join(__dirname, 'platformObjectMappings.yaml'), 'utf8'));
    jobInfo.platformObjectMappings = this.vlocity.datapacksutils.updateExpandedDefinitionNamespace(platformObjectMappings);
    dataJSON.dataPacks.forEach(dataPack => {
