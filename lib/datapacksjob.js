...
DataPacksJob.prototype.formatResponse = async function(jobInfo, error) {
    ...
    try {
        ...
        if (jobInfo.jobAction == 'Export' || jobInfo.jobAction == 'Deploy') {
            this.vlocity.datapacksutils.saveCurrentJobInfo(jobInfo, true);
        }
...
DataPacksJob.prototype.exportGroup = async function(inputMap) {
    ...
    if (exportData.length != 0) {
        ...
        if (jobInfo.expansionPath) {
            ...
            this.addReferencedDataPacksToJobInfo(jobInfo);
...
DataPacksJob.prototype.addReferencedDataPacksToJobInfo = function(jobInfo) {

    for (var dataPackType in jobInfo.referencedDataPacks) {
        var allDataSummaryMap = {};

        for (var key in jobInfo.allDataSummary) {
            if (jobInfo.allDataSummary[key].VlocityDataPackType === dataPackType) {
                allDataSummaryMap[dataPackType + jobInfo.allDataSummary[key].VlocityDataPackDisplayLabel] = '';
            }
        }

        for (var dataPackName in jobInfo.referencedDataPacks[dataPackType]) {
            if (!allDataSummaryMap.hasOwnProperty(dataPackType + dataPackName)) {
                if (!jobInfo.fullManifest[dataPackType]) {
                    jobInfo.fullManifest[dataPackType] = {};
                }
                
                jobInfo.fullManifest[dataPackType][dataPackName] = {VlocityDataPackType: dataPackType, Id: dataPackName};
            }
        }
    }
}
