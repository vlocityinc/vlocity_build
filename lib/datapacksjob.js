...
DataPacksJob.prototype.getOrgProjects = async function (jobInfo) {
    try {
        let query = `SELECT Name FROM ${this.vlocity.namespacePrefix}Project__c WHERE ${this.vlocity.namespacePrefix}IsActive__c = true AND ${this.vlocity.namespacePrefix}Status__c IN ('Released') AND RecordType.DeveloperName = 'WorkSet'`;
