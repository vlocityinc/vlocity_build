...
DataPacksJob.prototype.deployJob = async function(jobInfo) {
    ...
    if (jobInfo.gitCheck) {
        ...
        var hashKey = await this.vlocity.utilityservice.getVlocitySetting('VBT_DEPLOY_KEY');
            ...
            for (var line of gitChanges.split('\n')) {
                if (line.length > 0 && line[0] == ':') {
                    ...
                    if (splitFile.length > 2) {
                        ...
                        if (!jobInfo.specificManifestKeys.includes(dataPackKey)) {
    ...
    if (jobInfo.gitCheck) {
        ...
        await this.vlocity.utilityservice.setVlocitySetting('VBT_DEPLOY_KEY', currentHash);
    }
