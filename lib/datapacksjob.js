...
DataPacksJob.prototype.formatResponse = async function(jobInfo, error) {
    ...
    try {
        ...
        var vlocityKeysToUrl = {};
        for (var dataPackType in (jobInfo.fullManifest || {})) {
            for (var dataPackKey in jobInfo.fullManifest[dataPackType]) {
                vlocityKeysToUrl[dataPackType + '/' + jobInfo.fullManifest[dataPackType][dataPackKey].VlocityDataPackDisplayLabel] = this.vlocity.utilityservice.getVisualForcePageUrl(dataPackType, dataPackKey);
        ...
        if (jobInfo.sourceData && jobInfo.sourceData.dataPacks && jobInfo.sourceData.dataPacks.length !== 0){
            for(var key in jobInfo.sourceData.dataPacks){
                ...
                vlocityKeysToUrl[jobInfo.sourceData.dataPacks[key].VlocityDataPackKey] = jobInfo.sourceData.dataPacks[key].VlocityDataPackData[dataField][0].orgUrl;
            }
        }
        ...
        if (jobInfo.jobAction == 'GetAllAvailableExports') {
            ...
            for (var key in jobInfo.currentStatus) {
                ...
                if (!dataPack.orgUrl && dataPack.VlocityDataPackKey && Object.keys(vlocityKeysToUrl).length !== 0) {
                    dataPack.orgUrl = vlocityKeysToUrl[dataPack.VlocityDataPackKey];
                    if(dataPack.orgUrl){
