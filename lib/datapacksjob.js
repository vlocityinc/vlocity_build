...
DataPacksJob.prototype.runJob = async function(action, jobData) {
    ...
    try {
        ...
        sfdx.setEnforceSecurityValidations(jobInfo.enforceSecurityValidations);
...
DataPacksJob.prototype.runQueryForManifest = async function(queryInput) {
    ...
    return await this.vlocity.jsForceConnection.query(query)
    .on("record", (record) => {
    ...
    .on("error", (err) => {
        ...
        VlocityUtils.verbose('Query Error', err);
    })
    .on("end", () => {
        VlocityUtils.report('VlocityDataPackType', queryData.VlocityDataPackType);
        VlocityUtils.report('Query', query);
    ...
    if (jobInfo.queries) {
        ...
        try {
            ...
            for (var queryData of jobInfo.queries) {
                ...
                if (queryData.standardOnly && !this.vlocity.isOmniStudioInstalled) {
                    continue;
                }

                if (queryData.industryOnly && this.vlocity.isOmniStudioInstalled && !this.vlocity.isOmniStudioIndustryInstalled) {
                    continue;
                }
...
DataPacksJob.prototype.deployJob = async function(jobInfo) {
    ...
    if(jobInfo.deployGeneratedLwc === true || jobInfo.useBulkOmniScriptLwcCompile) {
        if(!fs.existsSync(tempSfdxLwcFolder)) {
            ...
            try {
                ...
                const sfdxCommandRunner = this.vlocity.utilityservice.getSfdxCommandRunner(jobInfo);
                
                if(fs.existsSync(mdapiTempFolder)){
                    fs.emptyDirSync(mdapiTempFolder);
                }

                VlocityUtils.log('Running source:convert');
                await sfdxCommandRunner('source:convert', {
                    rootdir: tempSfdxLwcFolder,
                    outputdir: mdapiTempFolder,
                    targetusername: this.vlocity.sfdxUsername,
                    useSfCli: jobInfo.useSfCli,
                    useSfdxCli: jobInfo.useSfdxCli
                });
                VlocityUtils.log('source:convert ended successfully');
                
                VlocityUtils.log('Running mdapi:deploy');
                ...
                VlocityUtils.log('mdapi:deploy ended successfully');

                if (mdapiDeployResult && mdapiDeployResult.details && mdapiDeployResult.details.componentSuccesses) {
                    mdapiDeployResult.details.componentSuccesses
                        .forEach((metadata) => {
    ...
    if (jobInfo.gitCheck && !jobInfo.hasError) {
        var currentHash = childProcess.execSync(`cd ${sfdx.shellEscapeArg(jobInfo.projectPath)} && git rev-parse HEAD`, { encoding: 'utf8' });
