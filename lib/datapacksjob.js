...
DataPacksJob.prototype.getOrgProjects = async function (jobInfo) {
    try {
        let query = `SELECT Name FROM ${this.vlocity.namespacePrefix}Project__c WHERE ${this.vlocity.namespacePrefix}IsActive__c = true AND ${this.vlocity.namespacePrefix}Status__c IN ('Released')`;

        let workSetRecordTypeIds = await this.vlocity.jsForceConnection.query(`SELECT Id FROM RecordType WHERE DeveloperName = 'WorkSet'`);
        if (workSetRecordTypeIds.records.length > 0) {
            query += ` AND RecordTypeId = '${workSetRecordTypeIds.records[0].Id}'`
        }
