...
DataPacksJob.prototype.downloadPerformanceData = async function (jobInfo) {
    return new Promise(async (resolve, reject) => {
        ...
        var query = `SELECT ${field_name},CreatedBy.Username,CreatedBy.Profile.Name FROM ${this.vlocity.namespace}__VlocityTrackingEntry__c WHERE Name = 'StepActionTime' AND ${this.vlocity.namespace}__TrackingService__c in ('IntProc', 'OmniScript') ORDER BY CreatedDate DESC`;
        var records = [];

        VlocityUtils.milestone('Downloading Performance Data');

        var totalEntries = await this.vlocity.jsForceConnection.query(`SELECT count() FROM ${this.vlocity.namespace}__VlocityTrackingEntry__c WHERE Name in ('StepActionTime', 'UI Action') AND ${this.vlocity.namespace}__TrackingService__c in ('IntProc', 'OmniScript')`);
