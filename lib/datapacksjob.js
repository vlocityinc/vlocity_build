...
DataPacksJob.prototype.validateParentKeys = async function (jobInfo) {
    let dataJSON = await this.vlocity.datapacksbuilder.buildImport(jobInfo.projectPath, jobInfo);
    let gaurantedParentKeys = this.vlocity.datapacksutils.dataPacksExpandedDefinition.GuaranteedParentKeys;
    for (var dataPack of dataJSON.dataPacks) {
        if (!dataPack.VlocityDataPackParents) {
            ...
            let parentKeys = dataPack.VlocityDataPackParents || [];
            let tempParentKeys = [];
            parentKeys.forEach(parentKey => {
                parentKey = parentKey.replace('%vlocity_namespace%__','').replace('__c','');
                if (!tempParentKeys.includes(parentKey)
                    && !gaurantedParentKeys.includes(parentKey)
                    && Object.keys(jobInfo.keysToDirectories).includes(parentKey)
                    && dataPack.VlocityDataPackKey !== parentKey) {
                    tempParentKeys.push(parentKey);
                }
            });
            if (tempParentKeys.length > 0) {
                tempParentKeys.sort();
                ...
                await this.vlocity.datapacksexpand.writeFile(dataPack.VlocityDataPackType, dataPack.VlocityDataPackName, dataPackName + "_ParentKeys", "json", tempParentKeys);
            }
        }
    }
}