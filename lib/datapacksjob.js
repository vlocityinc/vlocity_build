...
DataPacksJob.prototype.getAllAvailableExports = async function(jobInfo) {
    ...
    var availableExportsFile = path.join(this.vlocity.tempFolder, 'projectavailables', `${this.vlocity.username.replace('.', '_').replace('@', '_')}-available-${this.vlocity.datapacksutils.hashCode(JSON.stringify(jobInfo.allAllowedTypes || {}))}.json`);
...
DataPacksJob.prototype.updateSettings = async function(jobInfo) {
    ...
    if (jobInfo.separateMatrixVersions) {
        projectPathsToDeploy.push('matrixVersions');
    }
                    ...
                    if (jobInfo.headersOnly) {
                        ...
                        if(jobInfo.versionCompare){
                            ...
                            if (dataField && dataPack.VlocityDataPackData && dataPack.VlocityDataPackData[dataField]) {
                                ...
                                jobInfo.VlocityDataPackKeyToUrlMapping[dataPack.VlocityDataPackKey] = this.vlocity.utilityservice.getVisualForcePageUrl(dataPack.VlocityDataPackType, datapackId);
                            }
                        }
                    }
...
DataPacksJob.prototype.buildManifest = async function(jobInfo) {
    ...
    if (jobInfo.includeSalesforceMetadata) {
        ...
        for (var dataPack of jobInfo.sfdxData) {
            ...
            jobInfo.dataPackDisplayLabels[dataPack.VlocityDataPackKey] = dataPack.VlocityDataPackKey;

            if (!jobInfo.currentStatus[this.vlocity.datapacksutils.withoutMetadataExtension(dataPack.VlocityDataPackKey)]) {
                jobInfo.currentStatus[dataPack.VlocityDataPackKey] = "Added";
            }
        }
    }
...
DataPacksJob.prototype.downloadPerformanceData = async function (jobInfo) {
    return new Promise(async (resolve, reject) => {
        ...
        var query = `SELECT ${field_name} FROM ${this.vlocity.namespace}__VlocityTrackingEntry__c WHERE Name = 'StepActionTime' ORDER BY CreatedDate DESC`;
        var records = [];
        ...
        this.vlocity.jsForceConnection.query(query)
            ...
            .on("end", async function () {
                ...
                var omni_id =records.map(item => {
                    return JSON.parse(item[field_name]).OmniScriptId;
                });

                var unique_omni_id=Array.from([...new Set(omni_id)]);
 
                var unique_omni_id_format="";
                
                for(var j=0;j<unique_omni_id.length;j++){
                    if(j == (unique_omni_id.length-1)){
                        unique_omni_id_format=unique_omni_id_format+`'${unique_omni_id[j]}'`
                    }else{
                        unique_omni_id_format=unique_omni_id_format+ `'${unique_omni_id[j]}',`;
                    }
                }

                var additional_data = await vloc.jsForceConnection.query(`SELECT ${vloc.namespace}__PropertySet__c,Name,${vloc.namespace}__OmniScriptId__c FROM ${vloc.namespace}__Element__c WHERE ${vloc.namespace}__OmniScriptId__c IN (${unique_omni_id_format})`);
                ...
                additional_data.records.map(item => {
                    ...
                    var name=item['Name']
                    var omniscript_id=item[vloc.namespace+'__OmniScriptId__c'];
                    var propertyset=JSON.parse(item[vloc.namespace+'__PropertySet__c']);
                   
                    for(var k=0;k<records.length;k++){
                        var element_name= JSON.parse(records[k][field_name]).ElementName;
                        var element_omniScript_id= JSON.parse(records[k][field_name]).OmniScriptId;
                        var element_type= JSON.parse(records[k][field_name]).ElementType;
                        var element_data=JSON.parse(records[k][field_name])


                        if((element_name == name) && (element_omniScript_id == omniscript_id)){
                            switch (element_type) {
                                case "Calculation Procedure Action":
                                    element_data.configurationName = propertyset.remoteOptions.configurationName;
                                    break;
                                case "DataRaptor":
                                    element_data.bundle = propertyset.bundle;
                                  break;
                                case "Integration Procedure Action":
                                    element_data.integrationProcedureKey = propertyset.integrationProcedureKey;
                                    break;
                                case "Matrix Action":
                                    element_data.matrixName = propertyset.remoteOptions.matrixName;
                                    break;
                                case "Remote Action":
                                    element_data.remoteClass = propertyset.remoteClass;
                                    element_data.remoteMethod = propertyset.remoteMethod;
                                    break;
                                default: 
                              }
                              res.push(JSON.stringify(element_data));
                              VlocityUtils.log("k",k)
                              break;
                        }
                    };
...
DataPacksJob.prototype.getUserNameFromUserId = async function (jobInfo) {
    ...
    try {
        query = `SELECT Id, Name FROM User LIMIT 100`;
        result = await this.vlocity.jsForceConnection.query(query);
...
DataPacksJob.prototype.getOrgProjects = async function (jobInfo) {
    try {
        query = `SELECT Name FROM ${this.vlocity.namespacePrefix}Project__c`;
        result = await this.vlocity.jsForceConnection.query(query);
        jobInfo.data = result.records.map(item => item.Name);
    }
    catch (err) {
