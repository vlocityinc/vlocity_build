...
DataPacksJob.prototype.doRunJob = async function(jobInfo, action) {
    ...
    if (action == 'DiffPacks') {
        ...
        await self.runDeltaCheck(jobInfo);
...
DataPacksJob.prototype.formatResponse = function(jobInfo, error) {
    ...
    try {
        ...
        if (error) {
            ...
            jobInfo.hasError = true;
            jobInfo.errors.push(error.stack || error.message || error);
        }
        ...
        if (jobInfo.jobAction == 'GetAllAvailableExports') {
            ...
            for (var key in jobInfo.currentStatus) {
                ...
                if (jobInfo.jobAction == 'DiffPacks' 
...
DataPacksJob.prototype.runQueryForManifest = async function(queryInput) {
    ...
    try {
        ...
        .on("end", () => {
            ...
            VlocityUtils.report('Records', jobInfo.fullManifest[queryData.VlocityDataPackType] ? Object.keys(jobInfo.fullManifest[queryData.VlocityDataPackType]).length : 0);
        })
...
DataPacksJob.prototype.runDeltaCheck = async function(jobInfo) {
    ...
    if (dataJson && dataJson.dataPacks) {
        await self.vlocity.deltacheck.runDeltaCheck(jobInfo, dataJson.dataPacks);
    }
};
...
DataPacksJob.prototype.exportGroup = async function(inputMap) {
    ...
    if (exportData.length != 0) {
        ...
        if (jobInfo.expansionPath) {
            ...
            this.vlocity.datapackserrorhandling.getSanitizedErrorMessage(jobInfo, dataPackData);
        }
...
DataPacksJob.prototype.buildImportSeries = async function(jobInfo, deploySeries, currentImportInfo) {
    ...
    do {
        deployEntry = await this.vlocity.datapacksbuilder.buildImport(jobInfo.projectPath, jobInfo, currentImportInfo);
...
DataPacksJob.prototype.deployPack = async function(inputMap) {
    ...
    await this.runStepApex(jobInfo.projectPath, apexSettings, preStepDeployData, jobInfo.shouldDebug);
        ...
        for (var dataPack of dataPackData.dataPacks) {
            ...
            if (dataPack.VlocityDataPackStatus == 'Success') {
                ...
                if (dataPack.VlocityDataPackMessage && dataPack.VlocityDataPackMessage.indexOf('unable to obtain') != -1) {
                    ...
                    this.vlocity.datapackserrorhandling.getSanitizedErrorMessage(jobInfo, dataPack);
                }
            }
        }
...
DataPacksJob.prototype.deployJob = async function(jobInfo) {
    ...
    do {
        deploySeries = await this.buildImportSeries(jobInfo, [], {});
...
DataPacksJob.prototype.diffPacks = async function(jobInfo) {
    ...
    sourceData.dataPacks.forEach(function(dataPack) {
