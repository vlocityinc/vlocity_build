...
DataPacksJob.prototype.downloadPerformanceData = async function (jobInfo) {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        var query = `SELECT ${field_name},CreatedBy.Username,CreatedBy.Profile.Name FROM ${this.vlocity.namespacePrefix}VlocityTrackingEntry__c WHERE Name = 'StepActionTime' ${timestampFilter} ORDER BY CreatedDate DESC`;
        var records = [];

        VlocityUtils.milestone('Downloading Performance Data');

        var totalEntries = await this.vlocity.jsForceConnection.query(`SELECT count() FROM ${this.vlocity.namespacePrefix}VlocityTrackingEntry__c WHERE Name = 'StepActionTime' ${timestampFilter}`);
