        ...
        this.vlocity.jsForceConnection.query(query)
            ...
            .on("end", async function () {
                ...
                let keys = ["preTransformBundle", "postTransformBundle", "sendJSONNode", "sendJSONPath", "responseJSONNode", "responseJSONPath", "useQueueableApexRemoting", "remoteTimeout",
                    ...
                    "remoteOptions.matrixName", "remoteOptions", "Type", "Sub Type", "Language", "preIP", "postIP", "remoteClass", "remoteMethod", "returnFullDataJSON"];
                ...
                records.map(item => {
                    ...
                    for (let k in propertyset) {

                        if (keys.includes(k) && (propertyset[k] !== "")) {
                            if (typeof propertyset[k] == 'object') {
                                for (var j in propertyset[k]) {
                                    if (keys.includes(`${k}.${j}`) && (propertyset[k][j] !== "")) {
                                        element_data[j] = propertyset[k][j];
                                    }
                                }
                            } else {
