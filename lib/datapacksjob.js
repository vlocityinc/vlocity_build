...
DataPacksJob.prototype.deployJob = async function(jobInfo) {
    ...
    if (!jobInfo.isRetry) {        
        ...
        if (Object.keys(jobInfo.sourceKeyToMatchingKeysData).length === 0) {
            ...
            tempJobInfo.platformObjectMappings = yaml.safeLoad(fs.readFileSync(path.join(__dirname,'platformObjectMappings.yaml'), 'utf8'));
            if (dataJSON && dataJSON.dataPacks && tempJobInfo.upgradeDataPackFields) {
                dataJSON.dataPacks.forEach(dataPack => {
                    this.vlocity.datapacksutils.updateDataPackWithNewMappings(dataPack, tempJobInfo);
                });
                let ipConfig = this.vlocity.datapacksutils.dataPacksExpandedDefinition.DataPacks.IntegrationProcedure;
                ipConfig['%vlocity_namespace%__OmniProcess__c'] = ipConfig['%vlocity_namespace%__OmniScript__c'];
                delete ipConfig['%vlocity_namespace%__OmniScript__c'];
            }
...
DataPacksJob.prototype.convertToCore = async function (jobInfo) {
    ...
    jobInfo.platformObjectMappings = yaml.safeLoad(fs.readFileSync(path.join(__dirname, 'platformObjectMappings.yaml'), 'utf8'));
    dataJSON.dataPacks.forEach(dataPack => {
    ...
    let outputPath = jobInfo.outputDir || path.join(jobInfo.tempFolder, 'CoreDataPacks');
    this.vlocity.datapacksexpand.expand(outputPath, dataJSON, jobInfo);
}