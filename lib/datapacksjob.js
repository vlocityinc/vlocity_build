...
const DataPacksJob = module.exports = function (vlocity) {
...
const SUPPORTED_JOB_KEY_TO_OPTION_MAP = {
...
DataPacksJob.prototype.runJob = async function (action, jobData) {
  ...
  try {
    ...
    return this.formatResponse(jobInfo, e)
  }
}

DataPacksJob.prototype.mergeInJobDefaults = function (jobInfo) {
  for (let settingsKey in this.defaultJobSettings) {
...
DataPacksJob.prototype.intializeJobInfo = function (jobInfo, action) {
  ...
  if (action === 'Continue' || action === 'Retry') {
    ...
    if (jobInfo.jobAction === 'Deploy') {
      ...
      for (let dataPackKey in jobInfo.currentStatus) {
        if (action === 'Retry') {
  ...
  if (jobInfo.jobAction === 'Continue' || jobInfo.jobAction === 'Retry') {
    throw new Error(jobInfo.jobAction + ' initialization failed. Please use another Command')
  }

  if (jobInfo.manifest || jobInfo.workingSet) {
    this.formatManifest(jobInfo)
  }
...
DataPacksJob.prototype.runJobWithInfo = async function (jobInfo, action) {
  try {
    this.intializeJobInfo(jobInfo, action)
    action = jobInfo.jobAction
    ...
    return this.formatResponse(jobInfo)
  } catch (err) {
    VlocityUtils.error('Uncaught Job Error', err)
    return this.formatResponse(jobInfo, err)
  }
}
...
DataPacksJob.prototype.formatResponse = async function (jobInfo, error) {
  try {
    ...
    for (let dataPackType in (jobInfo.fullManifest || {})) {
      for (let dataPackKey in jobInfo.fullManifest[dataPackType]) {
    ...
    if (jobInfo.jobAction === 'GetAllAvailableExports') {
      var manifestArray = []
      for (let key in allManifestByKey) {
      ...
      for (let key in jobInfo.currentStatus) {
...
DataPacksJob.prototype.exportGroup = async function (inputMap) {
  ...
  if (exportData.length !== 0) {
    ...
    for (var dataPack of dataPackData.dataPacks) {
      ...
      if (dataPack.VlocityDataPackStatus === 'Success') {
        ...
        if (jobInfo.currentStatus[dataPack.VlocityDataPackKey] !== 'Success') {
          ...
          if (jobInfo.currentStatus[dataPack.VlocityDataPackKey] === 'Error') {
            let errorMessage = dataPack.VlocityDataPackType + ' --- ' + dataPack.VlocityDataPackName + ' --- ' + (dataPack.VlocityDataPackMessage ? dataPack.VlocityDataPackMessage.trim() : '')
    ...
    for (let pack of originalKeys) {
      let packKey = pack.VlocityDataPackKey
...
DataPacksJob.prototype.updateSettings = async function (jobInfo) {
  ...
  this.intializeJobInfo(tempJobInfo, 'BuildFile')
  ...
  let result = await this.vlocity.jsForceConnection.query(`Select ${this.vlocity.namespace}__MapId__c from ${this.vlocity.namespace}__DRMapItem__c where ${this.vlocity.namespace}__MapId__c = '${hashCode}'`)
...
DataPacksJob.prototype.activateAll = async function (dataPack, jobInfo, attempts) {
  ...
  for (let dataPack of dataPackData.dataPacks) {
    if (dataPack.ActivationStatus === 'Ready' && dataPack.VlocityDataPackStatus === 'Success') {
      ...
      let message = 'Activation Error >> ' + dataPack.VlocityDataPackKey + ' --- ' + (dataPack.ActivationMessage ? dataPack.ActivationMessage : (activateResult.error !== 'OK' ? activateResult.error : ''))
...
DataPacksJob.prototype.deployPack = async function (inputMap) {
  ...
  try {
    while (this.allThreadNotStalled(jobInfo)) {
      ...
      let preStepDeployData = []

      for (let dataPack of dataJson.dataPacks) {
        let data = jobInfo.allDataSummary[dataPack.VlocityDataPackKey]
      ...
      let thisDeployHasError = result.Status === 'Error'
      let atLeastOneRecordHasError = false
      ...
      if (result.VlocityDataPackId) {
        let dataPackId = result.VlocityDataPackId
        let stepPostDeployResults = []
        ...
        for (let dataPack of dataPackData.dataPacks) {
          ...
          if (dataPack.VlocityDataPackStatus === 'Success') {
            ...
            if (jobInfo.headersOnly) {
              let headersType = this.vlocity.datapacksutils.getHeadersOnly(dataPack.VlocityDataPackType)
              ...
              let sobjTypeList = this.vlocity.datapacksutils.getApexSObjectTypeList(dataPack.VlocityDataPackType)
  ...
  if (jobInfo.deltaCheck) {
    ...
    if (deltaCheckJobInfo.deltaCheckResults) {
      for (var dataPackKey in deltaCheckJobInfo.deltaCheckResults) {
        if (deltaCheckJobInfo.deltaCheckResults[dataPackKey].status !== null) {
          jobInfo.specificManifestKeys.push(dataPackKey)
        }
      }
    }
...
DataPacksJob.prototype.getJobErrors = async function (err, jobInfo) {
  if (err.VlocityDataPackId) {
    await this.vlocity.datapacks.getErrors(err.VlocityDataPackId)
  } else if (err.dataPackId) {
    await this.vlocity.datapacks.getErrorsFromDataPack(err)
  } else {
    return
  }
  ...
  if (jobInfo.delete) {
    await this.vlocity.datapacks.delete(err.VlocityDataPackId ? err.VlocityDataPackId : err.dataPackId, this.getOptionsFromJobInfo(jobInfo))
  }
}
...
DataPacksJob.prototype.diffPacks = async function (jobInfo) {
  ...
  for (let dataPack of targetData) {
  ...
  for (let dataPack of sourceData.dataPacks) {
    if (!jobInfo.specificManifestKeys || jobInfo.specificManifestKeys.includes(dataPack.VlocityDataPackKey)) {
