        ...
        for (key in records) {
            if (typeof records[key] === 'object') {
                if (records[key].hasOwnProperty('VlocityMatchingRecordSourceKey')) {
                    ...
                    if (self.vlocity.salesforceIdMap[salesforceIdMapKey]) {
                        if (!jobInfo.salesforceIdDeleteBatch[self.vlocity.salesforceIdMap[salesforceIdMapKey]]) {
                            ...
                            VlocityUtils.verbose('Running Query', queryString);

                            var result = await self.vlocity.datapacksutils.deleteBulkRecords(queryString, objectName, self.vlocity.salesforceIdMap[salesforceIdMapKey], jobInfo.salesforceIdDeleteBatch);
                ...
                for (var dataPack of dataPackData.dataPacks) {
                    ...
                    if (dataPack.VlocityDataPackStatus == 'Success') {
                        ...
                        if (dataPack.VlocityDataPackMessage && dataPack.VlocityDataPackMessage.indexOf('unable to obtain') != -1) {
                            ...
                            await this.vlocity.datapacksutils.handleDataPackEvent('onDeployError', dataPack.VlocityDataPackType, {dataPack: dataPack, jobInfo: jobInfo});
...
DataPacksJob.prototype.deployJob = async function(jobInfo, runJustDeploy) {
