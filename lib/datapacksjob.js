...
DataPacksJob.prototype.deployBulkRecords = async function (dataPack, BulkRecords, jobInfo, objectName, objectKey) {
    ...
    self.vlocity.insertIndexToSfIdMap = new Map();
    self.vlocity.nameToSfIdMap = new Map();
    self.vlocity.salesforceIdMap = new Map();
        ...
        for (key in records) {
            if (typeof records[key] === 'object') {
                if (records[key].hasOwnProperty('VlocityMatchingRecordSourceKey')) {
                    ...
                    if (!jobInfo.salesforceIdDeleteBatch[self.vlocity.salesforceIdMap[salesforceIdMapKey]]) {
                        var queryString = "SELECT Id FROM " + objectName + " WHERE " + objectKey + " = '" + self.vlocity.salesforceIdMap[salesforceIdMapKey] + "'";
                        await self.vlocity.datapacksutils.deleteBulkRecords(queryString, objectName, self.vlocity.salesforceIdMap[salesforceIdMapKey], jobInfo.salesforceIdDeleteBatch);
                    }
                }
            }
        }
    ...
    await self.vlocity.datapacksutils.createBulkJob(objectName, 'insert', BulkRecords, randomNum);
}
    ...
    try {
        ...
        while (this.allThreadNotStalled(jobInfo)) {
            ...
            if (result.VlocityDataPackId) {
                ...
                for (var dataPack of dataPackData.dataPacks) {
                    var objectName = self.vlocity.datapacksutils.getObjectName(dataPack.VlocityDataPackType);
                    var objectKey = self.vlocity.datapacksutils.getObjectKey(dataPack.VlocityDataPackType);
