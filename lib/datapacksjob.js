...
DataPacksJob.prototype.doRunJob = function(jobInfo, action, onComplete) {
    ...
    if (action == 'Export') {
        ...
        self.runDeltaCheck(jobInfo, onComplete);
...
DataPacksJob.prototype.runDeltaCheck = function(jobInfo, onComplete) {
    var self = this;
    var fullDataPath = jobInfo.projectPath;
    ...
    self.vlocity.datapacksbuilder.buildImport(fullDataPath, jobInfo, {}, function(dataJson) { 
        if (dataJson && dataJson.dataPacks) {
            self.vlocity.deltacheck.runDeltaCheck(jobInfo, dataJson.dataPacks, onComplete);
        } else {
            onComplete(jobInfo); 
        }
    });
