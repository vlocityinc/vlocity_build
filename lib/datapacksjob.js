...
DataPacksJob.prototype.deployPack = async function(inputMap) {
    ...
    try {
        ...
        if ((dataJson.dataPacks.length === 1 || Object.keys(jobInfo.sourceKeyToRecordId).length !== 0) && dataJson.dataPacks[0].VlocityDataPackType === 'Project') {
            await this.vlocity.datapacksutils.handleDataPackEvent('checkProjectItems', 'project', { dataPacks: dataJson.dataPacks, jobInfo: jobInfo });
        } else {
            dataJson.dataPacks = dataJson.dataPacks.filter(dataPack => {
                return dataPack.VlocityDataPackType !== 'Project';
            });
        }
