...
DataPacksJob.prototype.doRunJob = async function(jobInfo, action) {
    ...
    } else if (action === 'updateMappings') {
        await this.updateMappings(jobInfo);
    } else {
...
DataPacksJob.prototype.updateMappings = async function (jobInfo) {
    jobInfo.singleFile = true;
    ...
    jobInfo.singleFile = false;
    this.vlocity.datapacksexpand.expand(path.join(jobInfo.tempFolder,'Foundation'), dataJSON, jobInfo);
}