...
DataPacksJob.prototype.formatResponse = async function(jobInfo, error) {
    ...
    try {
        ...
        if (jobInfo.jobAction == 'GetAllAvailableExports') {
            ...
            response.status = jobInfo.hasError ? 'error' : 'success';
        } else if (jobInfo.jobAction === 'openDesigner') {
            response.records = jobInfo.data;
            response.message = jobInfo.errors;
            response.status = jobInfo.hasError ? 'error' : 'success';
        } else {
...
DataPacksJob.prototype.openDesigner = async function (jobInfo) {
    try {
        await this.buildManifestFromQueries(jobInfo);
        if(jobInfo.fullManifest[jobInfo.datapackType] && Object.keys(jobInfo.fullManifest[jobInfo.datapackType]).length !== 0){
            let datapackId = Object.keys(jobInfo.fullManifest[jobInfo.datapackType])[0],
            datapackExpandDefinition = this.vlocity.datapacksutils.dataPacksExpandedDefinition.DataPacks[jobInfo.datapackType],
            defaultVisualPage = '';
            if(datapackExpandDefinition.DefaultVisualforcePage){
                defaultVisualPage = datapackExpandDefinition.DefaultVisualforcePage[0];
                jobInfo.data = {"orgUrl": '/apex/' + this.vlocity.namespacePrefix +  defaultVisualPage + '?id=' + datapackId};
            } else{ 
                jobInfo.data = {"orgUrl": '/' + datapackId};
            }
        } else {
            jobInfo.data = {};
        }
        ...
        VlocityUtils.verbose('An Error occurred', err);
        jobInfo.hasError = true;
        jobInfo.errors.push(err);
    }
}
