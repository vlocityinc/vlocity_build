...
DataPacksJob.prototype.doRunJob = function(jobInfo, action, onComplete) {
    ...
    if (action == 'Export') {
        ...
        self.runValidationTest(jobInfo, onComplete);
...
DataPacksJob.prototype.runValidationTest = function(jobInfo, onComplete) {
    var self = this;
    ...
    self.vlocity.datapacksbuilder.buildImport(fullDataPath, jobInfo, {}, function(dataJson) { 
        if (dataJson && dataJson.dataPacks) {
            self.vlocity.validationtest.validate(jobInfo, dataJson.dataPacks, onComplete);
        } else {
            onComplete(jobInfo); 
        }
    });
