...
DataPacksJob.prototype.buildFile = async function(jobInfo) {
    ...
    if (jobInfo.buildFile) {
        ...
        if (jobInfo.includeSalesforceMetadata) {
            await this.getSfdxData(jobInfo);
        ...
        if (dataJson) {
            ...
            jobInfo.data = JSON.parse(stringify(dataJson).replace(new RegExp(this.vlocity.namespace, 'g'), '%vlocity_namespace%'));
    ...
    for (var dataPack of dataPackData.dataPacks) {
        ...
        if (dataPack.ActivationStatus == 'Ready' && dataPack.VlocityDataPackStatus == 'Success') {
            ...
            if (dataPackData.dataPacks.length == 1) {
                var activateReusableResult = {};

                if (dataPackData.dataPacks[0].VlocityDataPackData['%vlocity_namespace%__OmniScript__c']
                    && dataPackData.dataPacks[0].VlocityDataPackData['%vlocity_namespace%__OmniScript__c'][0]['%vlocity_namespace%__IsReusable__c']) {
                    
                    activateReusableResult = await this.vlocity.datapacksutils.handleDataPackEvent('activateReusable', 'OmniScript', dataPackData.dataPacks[0].VlocityDataPackData['%vlocity_namespace%__OmniScript__c'][0]);
                }

                if (activateReusableResult.ActivationStatus !== 'Success') {
                    var errorMessage = activateReusableResult.ActivationStatus === 'Error' ?  ' --- ' + activateReusableResult.message : ' --- Not Activated';
...
DataPacksJob.prototype.extractCalculationMatrixRecords = function (datapack, calculationMatrixRecords) {

    if (datapack.VlocityDataPackType === 'CalculationMatrix') {
                            ...
                            for (var calculationMatrixData of calculationMatrixVersion[cmkey])//for each calculation matrix version
                            {
                                Object.keys(calculationMatrixData).forEach(function (cmdata) {
                                    if (Array.isArray(calculationMatrixData[cmdata])) {
                                        for (var obj of calculationMatrixData[cmdata]) {
                                            var tempData = {};
                                            tempData['%vlocity_namespace%__InputData__c'] = obj['%vlocity_namespace%__InputData__c'];
                                            tempData['%vlocity_namespace%__CalculationMatrixVersionId__c'] = obj['%vlocity_namespace%__CalculationMatrixVersionId__c'];
                                            tempData['Name'] = obj['Name'];
                                            if (obj.hasOwnProperty("%vlocity_namespace%__OutputData__c")) {
                                                tempData['%vlocity_namespace%__OutputData__c'] = obj['%vlocity_namespace%__OutputData__c'];
                                            }
                                            else {
...
DataPacksJob.prototype.deployBulkRecords = async function (dataPack, BulkRecords, jobInfo, objectName, objectKey) {
        ...
        Object.keys(dataPack).forEach(function (key) {
            if (Array.isArray(dataPack[key])) {
                for (var dprecord of dataPack[key]) {
                    salesforceIdMap[dprecord.VlocityRecordSourceKey] = dprecord.VlocityRecordSalesforceId;
                }
            }
        });
    }
        ...
        for (key in records) {
            if (typeof records[key] === 'object') {
                if (records[key].hasOwnProperty('VlocityMatchingRecordSourceKey')) {
                    ...
                    records[key] = salesforceIdMap[salesforceIdMapKey];
                    //delete existing records for this Id from the org if not already done
                    if (!jobInfo.salesforceIdDeleteBatch[salesforceIdMap[salesforceIdMapKey]]) {
                        var queryString = "SELECT Id FROM " + objectName + " WHERE " + objectKey + " = '" + salesforceIdMap[salesforceIdMapKey] + "'";
                        await self.vlocity.datapacksutils.deleteBulkRecords(queryString, objectName, salesforceIdMap[salesforceIdMapKey], jobInfo.salesforceIdDeleteBatch);
                    }
                }
            }
        }
    ...
    await self.vlocity.datapacksutils.createBulkJob(objectName, 'insert', BulkRecords);
}
    ...
    try {
        ...
        while (this.allThreadNotStalled(jobInfo)) {
            ...
            if (result.VlocityDataPackId) {
                ...
                var objectName = self.vlocity.namespacePrefix + "CalculationMatrixRow__c";
                var objectKey = self.vlocity.namespacePrefix + "CalculationMatrixVersionId__c";
...
DataPacksJob.prototype.retrieveSalesforce = async function(jobInfo) {
    ...
    VlocityUtils.report('Retrieving Salesforce Apex');
    
    let allMetadata = await this.vlocity.jsForceConnection.metadata.describe('44.0');

    var allowedTypes = [ "ApexClass", "ApexComponent", "ApexPage", "ApexTrigger", "CustomMetadata", "CustomObject", "StaticResource"];

    var allSettingsText = '';

    for (var metaDesc of allMetadata.metadataObjects) {

        if (!allowedTypes.includes(metaDesc.xmlName)) continue;
       
       let metaList = await this.vlocity.jsForceConnection.metadata.list([{type: metaDesc.xmlName, folder: null}], '44.0');
       ...
       if (metaList && !Array.isArray(metaList)) {
        ...
        for (var meta of metaList) {
           // let read = await this.vlocity.jsForceConnection.metadata.read(meta.type,  [ meta.fullName ]);

            //console.log("Full Name: " + read);

            if (meta.namespacePrefix == null) {
                if (allSettingsText) {
                    allSettingsText += ',';
                }
                allSettingsText += `${meta.type}:${meta.fullName}`;
            }

           

            // Get All Unpackaged fields
            // Get All Packaged Picklist filed values
            // Standard picklists
        }
    }

    // Must exist in folder where command is being run no matter what
    if (!fs.existsSync('sfdx-project.json') || jobInfo.deleteSfdxProject) { 
        var sfdxTempFolder = path.relative(".", `${this.vlocity.tempFolder}/DX/salesforce_sfdx`);
        ...
        if (fs.existsSync(sfdxTempFolder)) {
            try {
        ...
        fs.writeFileSync('sfdx-project.json', stringify(salesforceProject), { encoding: 'utf8' });

        fs.writeFileSync(path.join(sfdxTempFolder,'..','sfdx-project.json'), stringify({
            "packageDirectories": [
                {
    ...
    var response = await this.vlocity.utilityservice.sfdx('source:retrieve', { targetusername: this.vlocity.sfdxUsername, metadata: allSettingsText, quiet: false });

    for (var includedFile of response.inboundFiles) {
        jobInfo.currentStatus[`${includedFile.type}/${includedFile.fullName}`] = 'Success';
    }
    //this.getSfdxData(jobInfo, '.');
}

DataPacksJob.prototype.getSfdxData = async function(jobInfo, sfdxPath) {
    ...
    if (!fs.existsSync(path.join(sfdxPath, 'sfdx-project.json'))) {
        VlocityUtils.error('No sfdx-project.json found');
        return;
    }

    VlocityUtils.report('Found SFDX Project', path.join(sfdxPath, 'sfdx-project.json'));

    var sfdxProject = JSON.parse(fs.readFileSync(path.join(sfdxPath, 'sfdx-project.json')));
    var sfdxFolder = sfdxProject.packageDirectories[0].path;
    var directories = this.vlocity.datapacksutils.getDirectories(path.join(sfdxPath, sfdxFolder), true);

    for (let dir of directories) {
        VlocityUtils.report('Dir', dir);
        for (let file of this.vlocity.datapacksutils.getFiles(path.join(sfdxPath, sfdxFolder, dir))) {
            VlocityUtils.report('File', file);
            if (file[0] != '.') {

                var dataPackType = dir.substring(dir.lastIndexOf('/')+1);
                ...
                try {
                    var fileData = {
                        ...
                        SFDXData: fs.readFileSync(path.join(sfdxPath, sfdxFolder, dir, file), { encoding: 'utf8' })
                    };
