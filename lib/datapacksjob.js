    ...
    try {
        ...
        if (jobInfo.jobAction == 'GetAllAvailableExports') {
            ...
            for (var key in jobInfo.currentStatus) {
                ...
                if(Object.keys(allManifestByKey).length !== 0){
                    for(var key in allManifestByKey){
                        if(allManifestByKey[key].VlocityDataPackDisplayLabel === dataPack.VlocityDataPackKey.split('/')[1]){
                            let visualForceUrlTemplate = this.vlocity.datapacksutils.getVisualForcePage(dataPack.VlocityDataPackType);
                            let orgUrl = this.vlocity.utilityservice.createVisualForceOrgUrl(visualForceUrlTemplate,key);
                            dataPack.Id = key;
                            dataPack.orgUrl = orgUrl;
                            break;
                        }
                    }
                } else if (jobInfo.sourceData && jobInfo.sourceData.dataPacks && jobInfo.sourceData.dataPacks.length !== 0){
                    for(var key in jobInfo.sourceData.dataPacks){
                        if(dataPack.VlocityDataPackKey === jobInfo.sourceData.dataPacks[key].VlocityDataPackKey){
                            let dataField = this.vlocity.datapacksutils.getDataField(jobInfo.sourceData.dataPacks[key]);
                            dataPack.Id = jobInfo.sourceData.dataPacks[key].VlocityDataPackData[dataField][0].Id;
                            dataPack.orgUrl = jobInfo.sourceData.dataPacks[key].VlocityDataPackData[dataField][0].orgUrl;
                            break;
                        }
                    }
                } else{
                    dataPack.Id = null;
                    dataPack.orgUrl = null;
                }
                if(dataPack.orgUrl){
            ...
            if (dataPack.VlocityDataPackStatus == 'Success') {
                ...
                if (!jobInfo.maxDepth || jobInfo.maxDepth == -1 || dataPack.VlocityDepthFromPrimary == 0) {
                    ...
                    if (dataField && dataPack.VlocityDataPackData && dataPack.VlocityDataPackData[dataField]) {
                        ...
                        let visualForceUrlTemplate = this.vlocity.datapacksutils.getVisualForcePage(dataPack.VlocityDataPackType);
                        dataPack.VlocityDataPackData[dataField][0].orgUrl =  this.vlocity.utilityservice.createVisualForceOrgUrl(visualForceUrlTemplate,dataPack.VlocityDataPackData[dataField][0].Id);
                    }
