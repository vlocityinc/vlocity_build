    ...
    try {
        ...
        if (jobInfo.jobAction == 'GetAllAvailableExports') {
            ...
            for (var key in jobInfo.currentStatus) {
                ...
                if (!dataPack.orgUrl && jobInfo.VlocityDataPackKeyToUrlMapping[dataPack.VlocityDataPackKey]) {
                    dataPack.orgUrl = jobInfo.VlocityDataPackKeyToUrlMapping[dataPack.VlocityDataPackKey];
                    if(dataPack.orgUrl){
                        dataPack.orgUrl = dataPack.orgUrl.replace(/%vlocity_namespace%__/g,this.vlocity.namespacePrefix);
                    }
                }
            ...
            if (jobInfo.specificManifestKeys.length == 0) {
                ...
                for (var query of jobInfo.queries) {
                    if (allTypesForQuery.has(query.VlocityDataPackType)) {
                        if(query.VlocityDataPackType.includes('Version')){
                            query.VlocityDataPackType = query.VlocityDataPackType.replace('Version','');
                        }
                        validQueries.push(query);
                    }
                }
                if(jobInfo.specificManifestKeys.length === 1 && jobInfo.specificManifestKeys[0].includes('Version')){
                    jobInfo.specificManifestKeys[0] = jobInfo.specificManifestKeys[0].replace('Version','');
                }
        ...
        try {
            ...
            for (var queryData of jobInfo.queries) {
                ...
                if((jobInfo.versionCompare && queryData.versionCompare) || (!jobInfo.versionCompare && !queryData.versionCompare)){
                    allQueries.push( { context: this, func: 'runQueryForManifest', argument: { jobInfo: jobInfo, queryData: queryData } });	                    allQueries.push( { context: this, func: 'runQueryForManifest', argument: { jobInfo: jobInfo, queryData: queryData } });
                }
            }
...
DataPacksJob.prototype.getAllAvailableExports = async function(jobInfo) {
    ...
    var availableExportsData, availableExportsFile;

    if (this.vlocity.username) {
        availableExportsFile = path.join(this.vlocity.tempFolder, `${this.vlocity.username.replace('.', '_').replace('@', '_')}-available.json`);
     
        try {
            availableExportsData = JSON.parse(await fs.readFile(availableExportsFile));
        } catch (e) {
            // Ignored
        }
    }
    ...
    if (jobInfo.resetAvailableCache || !availableExportsData) {
        ...
        if (availableExportsFile) {
            await fs.writeFile(availableExportsFile, JSON.stringify(jobInfo.fullManifest), { encoding: 'utf8' });
        }
    } 
    else {
            ...
            if (dataPack.VlocityDataPackStatus == 'Success') {
                ...
                if (!jobInfo.maxDepth || jobInfo.maxDepth == -1 || dataPack.VlocityDepthFromPrimary == 0) {
                    ...
                    if (dataField && dataPack.VlocityDataPackData && dataPack.VlocityDataPackData[dataField]) {
                        ...
                        dataPack.VlocityDataPackData[dataField][0].orgUrl =  this.vlocity.utilityservice.getVisualForcePageUrl(dataPack.VlocityDataPackType, dataPack.VlocityDataPackData[dataField][0].Id);
                        jobInfo.VlocityDataPackKeyToUrlMapping[dataPack.VlocityDataPackKey] = dataPack.VlocityDataPackData[dataField][0].orgUrl;
                    }
...
DataPacksJob.prototype.buildFile = async function(jobInfo) {
    ...
    if (jobInfo.buildFile) {
        ...
        await this.vlocity.datapacksutils.getAllParentKeys(dataJson.dataPacks, jobInfo);
...
DataPacksJob.prototype.expandFile = async function (jobInfo) {
    if (jobInfo.expandMultiple) {
        for (let i = 0; i < Object.keys(jobInfo.buildFile).length; i++) {
            let buildFile  = path.join(jobInfo.projectPath, jobInfo.buildFile[i]);
            await this.vlocity.datapacksexpand.expandFile(jobInfo.projectPath + '/' + jobInfo.expansionPath, buildFile, jobInfo);
        }
    }
    else {
    ...
    try {
        ...
        for (res in VplAssetRecords) {
            ...
            for (record in vplAttachmentRecords) {
                ...
                jobInfo_t.expansionPath = folderName;
...
DataPacksJob.prototype.downloadPerformanceData = async function (jobInfo) {
    ...
    try {
        query = `SELECT ${field_name} FROM ${this.vlocity.namespace}__VlocityTrackingEntry__c`;
        var result = await this.vlocity.jsForceConnection.query(query);
        var res = result.records.map(item => {
            return  item[field_name];
        });
        if(res.length > 0) {
            var filePtr = await fs.openSync(jobInfo.projectPath, 'w');
            await fs.writeSync(filePtr, JSON.stringify(res));
            await fs.closeSync(filePtr);
            jobInfo.data = res;
            jobInfo.hasError = false;
        }
        ...
        VlocityUtils.verbose('Error occurred during performance data download', err);
        jobInfo.hasError = true;
        jobInfo.errors.push(err);
    }
}
