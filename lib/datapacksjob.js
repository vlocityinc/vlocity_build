...
DataPacksJob.prototype.getUserNameFromUserId = async function (jobInfo) {
    ...
    var records =  [];
    try {
        for(var i = 0;i<jobInfo.userId.length;i++)  {
            query1  = `SELECT Id, Name FROM User WHERE Id = '${jobInfo.userId[i]}'`;
            VlocityUtils.verbose('query',query1);
            var result = await this.vlocity.jsForceConnection.query(query1);
            records.push(result.records[0]);
        }
        jobInfo.data = records;
    }
    catch (err) {
        VlocityUtils.verbose('Error occurred while fetching the username', err);
        jobInfo.hasError = true;
        jobInfo.errors.push(err);
    }
}