...
DataPacksJob.prototype.downloadPerformanceData = async function (jobInfo) {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        var query = `SELECT ${field_name},CreatedBy.Username,CreatedBy.Profile.Name FROM ${this.vlocity.namespacePrefix}VlocityTrackingEntry__c WHERE Name in ('StepActionTime', 'UI Action') AND ${this.vlocity.namespace}__TrackingService__c in ('IntProc', 'OmniScript') ${timestampFilter} ORDER BY CreatedDate DESC`;

        var records = [];

        VlocityUtils.milestone('Downloading Performance Data');

        var totalEntries = await this.vlocity.jsForceConnection.query(`SELECT count() FROM ${this.vlocity.namespacePrefix}VlocityTrackingEntry__c WHERE Name in ('StepActionTime', 'UI Action') AND ${this.vlocity.namespace}__TrackingService__c in ('IntProc', 'OmniScript') ${timestampFilter}`);
            ...
            .on("end", async function () {
                ...
                records.map(item => {
                    try {
                        ...
                        if (element_data.Name == 'UI Action') {
    ...
    try {
        for (const obj of jobInfo.queries) {
            if (obj.VlocityDataPackType === jobInfo.type) {
                ...
                if (jobInfo.omniType && jobInfo.omniSubType) {
