        ...
        this.vlocity.jsForceConnection.query(query)
            ...
            .on("end", async function () {
                ...
                while (unique_omni_id.length > 0) {
                    ...
                    var chunk = unique_omni_id.splice(0, 150);
                    var unique_omni_id_format = "";

                    for (var j = 0; j < chunk.length; j++) {
                        if (j == (chunk.length - 1)) {
                            unique_omni_id_format = unique_omni_id_format + `'${chunk[j]}'`
                        } else {
                            unique_omni_id_format = unique_omni_id_format + `'${chunk[j]}',`;
                        }
                    }

                    var additional_data = await self.vlocity.jsForceConnection.query(`SELECT ${self.vlocity.namespace}__PropertySet__c,Name,${self.vlocity.namespace}__OmniScriptId__c FROM ${self.vlocity.namespace}__Element__c WHERE ${self.vlocity.namespace}__OmniScriptId__c IN (${unique_omni_id_format})`);

                    additional_data.records.map(item => {

                        var name = item['Name']
                        var omniscript_id = item[self.vlocity.namespace + '__OmniScriptId__c'];
                        var propertyset = JSON.parse(item[self.vlocity.namespace + '__PropertySet__c']);

                        records.map(item => {
                            var element_data = JSON.parse(item[field_name])
                            var element_name = element_data.ElementName;
                            var element_omniScript_id = element_data.OmniScriptId;
                            var element_type = element_data.ElementType;
