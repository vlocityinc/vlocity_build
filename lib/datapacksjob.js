...
DataPacksJob.prototype.updateSettings = async function(jobInfo) {
    ...
    let hashCode = this.vlocity.datapacksutils.hashCode(stringify(dataJson)) + projectPathsToDeploy.join('-');
...
DataPacksJob.prototype.getUrlByActionDetails = async function (trackingEntries) {
    let actionsConfig = this.vlocity.datapacksutils.dataPacksExpandedDefinition.IPActionsConfig;
    let tempTrackingEntries = [];
    let isItemString = typeof (trackingEntries[0]) === 'string';
    for (let entry of trackingEntries) {
        entry = isItemString ? JSON.parse(entry) : entry;
        if (entry.TrackingEvent && entry.TrackingEvent === "StepActionTime") {
            if (actionsConfig[entry.ElementType]) {
                let config = actionsConfig[entry.ElementType];
                let query = config.query.replace(/%vlocity_namespace%__/g, this.vlocity.namespacePrefix);
                let queryAddOns = '';
                if (config.nameField === 'integrationProcedureKey') {
                    queryAddOns = `${this.vlocity.namespacePrefix}Type__c = '${entry[config.nameField].split('_')[0]}' AND ${this.vlocity.namespacePrefix}SubType__c = '${entry[config.nameField].split('_')[1]}'`
                } else {
                    queryAddOns = `Name = '${entry[config.nameField]}'`;
                }

                query += query.includes('WHERE') ? ` AND ${queryAddOns}` : ` Where ${queryAddOns}`;

                let results = await this.vlocity.jsForceConnection.query(query);
                if (results.records[0] && results.records[0].Id) {
                    entry.orgUrl = this.vlocity.utilityservice.getVisualForcePageUrl(config.type, results.records[0].Id);
                }
            }
        }
        tempTrackingEntries.push(isItemString ? JSON.stringify(entry) : entry);
    }
    return tempTrackingEntries;
}
