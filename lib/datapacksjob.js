    ...
    for (let entry of trackingEntries) {
        ...
        if (entry.TrackingEvent && entry.TrackingEvent === "StepActionTime") {
            if (actionsConfig[entry.ElementType]) {
                ...
                let nameFieldValue = config.nameField.split('.').reduce(function (currentNameObj, key) {
                    return currentNameObj[key];
                }, entry)

                let query = config.query.replace(/%vlocity_namespace%__/g, this.vlocity.namespacePrefix).replace(/%nameFieldValue%/g, nameFieldValue);
