...
DataPacksJob.prototype.deployJob = async function(jobInfo) {
    ...
    if(jobInfo.deployGeneratedLwc === true) {
        if(!fs.existsSync(tempSfdxLwcFolder)) {
            ...
            if(!commandExistsSync('sfdx')) {
                VlocityUtils.error('SFDX is not installed. Skipping...');
            }else{
                ...
                try {
                    ...
                    const sourceConvertExec = await sfdx.sourceConvert(
                        tempSfdxLwcFolder,
                        mdapiTempFolder
                    );
                    if(sourceConvertExec.stderr || sourceConvertExec.stdout.status === 1){
                        VlocityUtils.error("sfdx force:source:convert failed", sourceConvertExec);
                        jobInfo.errors.push("sfdx force:source:convert failed:\n" + sourceConvertExec.stderr);
                        jobInfo.hasError = true;
                    }else{
                        if(fs.existsSync(mdapiTempFolder)){
                            VlocityUtils.log('Running sfdx force:mdapi:deploy');
                            const mdapiDeployExec = await sfdx.mdapiDeploy(mdapiTempFolder);
                            if(mdapiDeployExec.stderr) {
                                VlocityUtils.error("sfdx force:mdapi:deploy failed", mdapiDeployExec.stderr);
                                jobInfo.errors.push("sfdx force:mdapi:deploy failed:\n" + mdapiDeployExec.stderr);
                                jobInfo.hasError = true;
                            } else {
                                const mdapiDeployResponse = mdapiDeployExec.stdout;
                                if(mdapiDeployResponse?.result?.success){
                                    mdapiDeployResponse?.result?.details?.componentSuccesses
                                        ?.forEach((metadata) => {
                                    ...
                                    VlocityUtils.error("Failed Deploy Response", mdapiDeployResponse);
                                    jobInfo.errors.push( "Failed Deploy Response:\n" + JSON.stringify(mdapiDeployResponse));
                                    jobInfo.hasError = true;
                                }
                            }
                        }
