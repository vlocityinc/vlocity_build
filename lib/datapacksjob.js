...
DataPacksJob.prototype.deployMultipleJob = async function (jobInfo) {
    try {
        if (Array.isArray(jobInfo.projectPaths)
            && Array.isArray(jobInfo.expansionPaths)
            && jobInfo.projectPaths.length === jobInfo.expansionPaths.length) {

            jobInfo.projectPath = path.join(this.vlocity.tempFolder, 'combined');
            ...
            for (const i in jobInfo.projectPaths) {
                let individualExpansionPath = jobInfo.expansionPaths[i] || '';
                await fs.copy(path.join(jobInfo.projectPaths[i], individualExpansionPath), destination);
            }

            await this.deployJob(jobInfo);
        } else {
            throw "Project paths or their respective expansion paths are invalid."
        }
    }
    catch (err) {
        jobInfo.hasError = true;
