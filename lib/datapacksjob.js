...
DataPacksJob.prototype.doRunJob = async function(jobInfo, action) {
    ...
    } else if (action === 'checkOmniScriptAvailability'){
        await this.checkOmniScriptAvailability(jobInfo);
    } else {
...
DataPacksJob.prototype.checkOmniScriptAvailability = async function(jobInfo) {
    try {
        await this.vlocity.jsForceConnection.query(`SELECT Id FROM ${this.vlocity.namespacePrefix}OmniScript__c`);
        jobInfo.data = {
            isOmniScriptAvailable: true
        };
    } catch(err) {
        jobInfo.data = {
            isOmniScriptAvailable: false
        };
    }
}