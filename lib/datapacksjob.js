...
DataPacksJob.prototype.formatResponse = async function(jobInfo, error) {
    ...
    try {
        ...
        } else if (jobInfo.jobAction == 'refreshVlocityProcessListing') {
            response.status = jobInfo.hasError ? 'error' : 'success';
            response.records = jobInfo.data;
        } else {
...
DataPacksJob.prototype.getAllAvailableExports = async function(jobInfo) {
    ...
    VlocityUtils.report('getAllAvailableExports1');
    ...
    VlocityUtils.report('getAllAvailableExports2');
...
DataPacksJob.prototype.diffPacks = async function(jobInfo) {
    ...
    try {
        await this.checkDiffs(jobInfo, sourceData, targetOrgRecordsHash);
...
DataPacksJob.prototype.downloadVplDatapack = async function (jobInfo) {
    ...
    try {
        ...
        let query = "SELECT VPL_Asset__c FROM VlocityProcessAssetAssociation__c WHERE VPL_Listing__r.Name = '"+ vplName +"' AND VPL_Asset__r.Type__c='JSON DataPack' AND VPL_Asset__r.Active__c=true";
        VlocityUtils.verbose('query:',query);
        await this.vlocity.jsForceConnection.query(query, function (err, result) {
            if (err) {
                jobInfo.hasError = true;
                jobInfo.errors.push(err);
            }
            VplAssetRecords = result.records;
            VlocityUtils.verbose("total-2 : " + result.totalSize);
            VlocityUtils.verbose("fetched-2 : " + result.records.length);
            VlocityUtils.verbose(VplAssetRecords);
            jobInfo.data = result;
        });

        for(res in VplAssetRecords)  {
            let query = "SELECT Name, Body FROM Attachment WHERE ParentId = '" + VplAssetRecords[res].VPL_Asset__c + "'";
            VlocityUtils.verbose('query:');
            VlocityUtils.verbose(query);
            await this.vlocity.jsForceConnection.query(query, function (err, result) {
                if (err) {
                    jobInfo.hasError = true;
...
DataPacksJob.prototype.refreshVlocityProcessListing = async function (jobInfo) {
    var VplList;
    try {
    let query = "SELECT VPL_Listing__r.Name  FROM VlocityProcessAssetAssociation__c WHERE VPL_Asset__c IN (SELECT Id FROM Vlocity_Process_Asset__c WHERE Active__c=true AND Type__c='JSON DataPack')";
    await this.vlocity.jsForceConnection.query(query, function (err, result) {
        if (err) {
    ...
    for (item in VplList) {
        ...
        let folderPath = path.join(jobInfo.projectPath, VplList[item]);
