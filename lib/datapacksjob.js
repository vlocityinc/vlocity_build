...
DataPacksJob.prototype.formatManifest = function(jobInfo) {
    ...
    if (jobInfo.workingSet) {
        ...
        var projectWorkingSetFile = path.join(jobInfo.projectPath, 'workingsets', jobInfo.workingSet);
        var tempDirWorkingSetFile = path.join(this.tempFolder, 'workingsets', jobInfo.workingSet);

        var workSetData;
        
        if (this.vlocity.datapacksutils.fileExists(projectWorkingSetFile)) {
            workSetData = yaml.safeLoad(fs.readFileSync(tempDirWorkingSetFile), 'utf8');
        ...
        jobInfo.manifest = workSetData.manifest;
    }
...
DataPacksJob.prototype.runDeltaCheck = async function(jobInfo) {
    ...
    if (dataJson && dataJson.dataPacks) {
        jobInfo.deltaInfo = await self.vlocity.deltacheck.runDeltaCheck(jobInfo, dataJson.dataPacks);
    }
};
