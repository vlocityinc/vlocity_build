...
DataPacksJob.prototype.doRunJob = async function(jobInfo, action) {
    ...
    } else if (action == 'ImportAllFromFolder') {
        await this.importAllFromFolder(jobInfo);
    ...
    } else if (action == 'InstallDPsfromStaticResource') {
        await this.installDPsfromStaticResource(jobInfo);        
...
DataPacksJob.prototype.formatResponse = async function(jobInfo, error) {
    ...
    try {
        ...
        } else if (jobInfo.jobAction == 'RefreshVlocityBase' || jobInfo.jobAction == 'InstallVlocityInitial' || jobInfo.jobAction == 'InstallDPsfromStaticResource') {
...
DataPacksJob.prototype.importAllFromFolder = async function(jobInfo) {
    ...
    for (var i = 0; i < allFiles.length; i++) {
        ...
        try {
            ...
            VlocityUtils.error("Error Importing", e);
        }
    }
};
...
DataPacksJob.prototype.installDPsfromStaticResource = async function(jobInfo) {
    if(!jobInfo.StaticResourceQuery){
        VlocityUtils.error('No Query found for InstallDPsfromStaticResource', 'Please add "StaticResourceQuery: <SOQL>" to your Job File ');
    } else {
        await this.installFromStaticResourceQuery(jobInfo, jobInfo.StaticResourceQuery);
    }
}
...
DataPacksJob.prototype.deployJob = async function(jobInfo) {
    ...
    if (!jobInfo.tempFolder) {
        jobInfo.tempFolder = "vlocity-temp";
    }
    ...
    if(jobInfo.deployGeneratedLwc === true) {
        ...
        if(!commandExistsSync('sfdx')) VlocityUtils.error('SFDX is not installed. Generated LWC components won\'t be deployed');
        if(!fs.existsSync(tempSfdxLwcFolder)) VlocityUtils.error('Can\'t find compiled LWC components path. Skipping...');
        ...
        try {
            ...
            VlocityUtils.log('Running sfdx force:source:convert');
            await sfdx.sourceConvert(
                tempSfdxLwcFolder,
                mdapiTempFolder
            );

            if(fs.existsSync(mdapiTempFolder)){
                VlocityUtils.log('Running sfdx force:mdapi:deploy');
                const result = await sfdx.mdapiDeploy(mdapiTempFolder);
                
                if(result.stderr) {
                    VlocityUtils.error("sfdx force:mdapi:deploy failed");
                    VlocityUtils.log(result.stderr);
                    jobInfo.errors.push("sfdx force:mdapi:deploy failed:\n" + result.stderr);
                } else {
                    const stdout = JSON.parse(result.stdout);
                    jobInfo.hasError = !stdout?.result?.success;
                    if(stdout?.result?.success){
                        const successes = this.vlocity.datapacksutils.formatMdapiSuccessDeployResponse(stdout);
                        jobInfo.currentStatus = Object.assign(
                            jobInfo.currentStatus,
                            successes
                        );
                    }else{
                        if(stdout?.result?.details?.componentFailures) {
                            const componentFailuresString = JSON.stringify(stdout?.result?.details?.componentFailures, null, 2);
                            VlocityUtils.error("Component Failures", componentFailuresString);
                            jobInfo.errors.push( "Component Failures:\n" + componentFailuresString);
                        }

                        if(stdout?.result?.numberTestErrors){
                            const runTestResultString = JSON.stringify(stdout?.result?.details?.runTestResult, null, 2);
                            VlocityUtils.error("Test Failures", runTestResultString);
                            jobInfo.errors.push( "Test Failures:\n" + runTestResultString);
                        }
                    }
                }
            }
