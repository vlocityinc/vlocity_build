...
DataPacksJob.prototype.intializeJobInfo = function(jobInfo, action) {
    ...
    jobInfo.diffFields = jobInfo.diffFields || {};
...
DataPacksJob.prototype.doRunJob = function(jobInfo, action, onComplete) {
    ...
    } else if (action == 'ValidationTest') {
        self.validationTest(jobInfo, onComplete);
    } else if (action == 'BuildManifest') {
        self.buildManifest(jobInfo, onComplete);
    } else {
        jobInfo.hasError = true;
    ...
    try {
        ...
        if (jobInfo.jobAction == 'GetAllAvailableExports') {
            ...
            Object.keys(jobInfo.currentStatus || {}).forEach(function(key) {
                ...
                if (jobInfo.diffFields[dataPack.VlocityDataPackKey]) {
                    dataPack.diffFields = jobInfo.diffFields[dataPack.VlocityDataPackKey];
                }
...
DataPacksJob.prototype.validationTest = function(jobInfo, onComplete) {
    var self = this;
    ...
    self.vlocity.checkLogin(function() {
        ...
        self.vlocity.datapacksbuilder.buildImport(fullDataPath, jobInfo, {}, function(dataJson) { 
            if (dataJson && dataJson.dataPacks) {
                ...
                self.vlocity.validationtest.validate(jobInfo, dataJson.dataPacks);
            } else {
                onComplete(jobInfo); 
            }
        });
...
DataPacksJob.prototype.refreshVlocityBase = function(jobInfo, onComplete) {
    ...
    self.vlocity.jsForceConnection.query("Select Name, Body from StaticResource where Name LIKE 'DP_%'", function(err, result) {
                    ...
                    self.vlocity.datapacks.import(JSON.parse(res), self.getOptionsFromJobInfo(jobInfo), 
                        ...
                        function(err) {
                            ...
                            self.vlocity.datapacks.getDataPackData(err.VlocityDataPackId, function(dataPackData) {
                                ...
                                VlocityUtils.error('Resource Deploy Failed', record.Name);
                                dataPackData.dataPacks.forEach(function(dataPack) {
                                    
                                    VlocityUtils.error('Deploy ' + dataPack.VlocityDataPackStatus, dataPack.VlocityDataPackType, '-', dataPack.VlocityDataPackKey, '-', dataPack.VlocityDataPackName, dataPack.VlocityDataPackMessage);
                    ...
                    self.runStepApex(jobInfo.projectPath, apexSettings, preStepDeployData, jobInfo.shouldDebug, function() {
                        ...
                        self.vlocity.datapacks.import(dataJson, self.getOptionsFromJobInfo(jobInfo), function(result) {
                            ...
                            if (result.VlocityDataPackId) {
                                ...
                                dataJson.dataPacks.forEach(function(dataPack) {
                                    jobInfo.currentStatus[dataPack.VlocityDataPackKey] = 'Error';
                                    jobInfo.errors.push(dataPack.VlocityDataPackKey + ' ' + (result.Message ? result.Message : '>> Unknown Error - Use --verbose to see more information'));

                                    VlocityUtils.error('Deploy Error', 'Deploy Failed', dataPack.VlocityDataPackKey + ' ' + (result.Message ? result.Message : '>> Unknown Error - Use --verbose to see more information'));
                                });
    ...
    self.exportJob(jobInfo, function(jobInfo) {    
        ...
        self.vlocity.datapacksbuilder.buildImport(jobInfo.projectPath, jobInfo, {}, function(currentFileData) { 
            ...
            self.vlocity.datapacksbuilder.buildImport(jobInfo.projectPath, jobInfo, {},function(checkDiffsFile) { 
                ...
                Object.keys(allTargetDapacks).forEach(function(dataPackKey) {
                    if (jobInfo.diffStatus[dataPackKey] != 'Unchanged') {
                        self.vlocity.datapacksutils.getFieldDiffs(sourceDataByKey[dataPackKey], allTargetDapacks[dataPackKey], jobInfo);
                    }
                });
...
DataPacksJob.prototype.buildManifest = function(jobInfo, onComplete) {
    var self = this;
