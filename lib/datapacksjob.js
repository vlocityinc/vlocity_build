...
DataPacksJob.prototype.runJobWithInfo = async function(jobInfo, action) {
    ...
    try {
        ...
        VlocityUtils.error('Uncaught Job Error', err, err.stack);
...
DataPacksJob.prototype.deployDataPackMetadataConfigurations = async function(jobInfo) {
    ...
    var dataPackMetadataOrg = {
        VlocityDataPackConfiguration: dataPackConfigurations,
        DRMatchingKey: drMatchingKeys
    };

    var metadata = {};

    for (var customMetadataType in dataPackMetadataLatest) {
        var customMetadataRecordsLatest = dataPackMetadataLatest[customMetadataType];
        var customMetadataRecords = dataPackMetadataOrg[customMetadataType];

        for (var metadataTypeRecordName in customMetadataRecordsLatest) {
            var metadataTypeRecordNameOriginal = metadataTypeRecordName;

            if (metadataTypeRecordName.includes('.json')) {
                metadataTypeRecordName = metadataTypeRecordName.substring(0, metadataTypeRecordName.indexOf('.json'));
            }

            if (!customMetadataRecords.hasOwnProperty(metadataTypeRecordName)) {
                if (!metadata[customMetadataType]) {
                    metadata[customMetadataType] = [];
                }

                metadata[customMetadataType].push(JSON.parse(customMetadataRecordsLatest[metadataTypeRecordNameOriginal]));
            }
        }
    }

    if (!this.vlocity.utilityservice.isEmptyObject(metadata)) {
        for (var customMetadataType in metadata) {
            await this.vlocity.utilityservice.createCustomMetadataRecord(metadata[customMetadataType]);
        }
    }
};
...
DataPacksJob.prototype.checkDiffs = async function(jobInfo, currentLocalFileData, targetOrgRecordsHash) {
    ...
    await Promise.all([
        fs.writeFile(path.join(this.vlocity.tempFolder, 'diffs/localFolderFiles.json'), stringify(currentFiles, { space: 4 })),
        fs.writeFile(path.join(this.vlocity.tempFolder, 'diffs/targetOrgFiles.json'), stringify(exportedFiles, { space: 4 }))
    ]);
};
