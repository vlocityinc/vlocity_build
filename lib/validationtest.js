...
ValidationTest.prototype.initializeTestProduct2 = function(runTestData) {
    if (runTestData.dataPack) {
        if (!this.jobInfo.defaultPriceList) {
            this.failTest('Default PriceList Missing ' + runTestData.VlocityDataPackKey);
            VlocityUtils.error('Default PriceList Missing', 'VlocityRecordSourceKey', runTestData.dataPack.VlocityRecordSourceKey);
            return;
        }
        ...
        if (!globalKeys || !globalKeys[0]) {
            this.failTest('Global Key Missing ' + runTestData.VlocityDataPackKey);
            VlocityUtils.error('Global Key Missing', 'VlocityRecordSourceKey', runTestData.dataPack.VlocityRecordSourceKey);
            return;
        }
...
ValidationTest.prototype.stopTestCheck = function(obj) {
    return this.jobInfo.hasError || this.testJobInfo.tests[obj.testUniqueKey].actionFailed;
};
...
ValidationTest.prototype.saveReportLocally = function() {
    ...
    if (testReport.records && testReport.records.length > 0) {
        fs.outputFileSync(path.join('vlocity-temp', 'validationTest', this.jobInfo.logName), yaml.dump(testReport.records), 'utf8');
    }
...
ValidationTest.prototype.createResponse = function(failuresOnly) {
    ...
    if (this.jobInfo.hasError) {
        ...
        response.errors = this.jobInfo.errors;
    }
