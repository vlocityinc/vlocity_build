...
const ValidationTest = module.exports = function (vlocity) {
...
ValidationTest.prototype.createResponse = function (failuresOnly) {
  ...
  for (let testUniqueKey in this.testJobInfo.tests) {
    for (let dataPackKey in this.testJobInfo.tests[testUniqueKey].testReport) {
      ...
      let dataPackKeyData = this.testJobInfo.tests[testUniqueKey].testReport[dataPackKey]

      for (let testName in dataPackKeyData) {
        if (failuresOnly) {
  ...
  if (failuresOnly) {
    for (let dataPackKey in reportMap) {
...
ValidationTest.prototype.query = async function (action) {
  ...
  return this.vlocity.queryservice.query(queryString)
}
...
ValidationTest.prototype.addProduct = async function (action) {
  let result
  const self = this
  const dataJson = self.testJobInfo.tests[action.testUniqueKey].dataJson
  const cartId = dataJson.createCart.Id
  const productGlobalKey = action.dataPackUniqueKey
  const pricebookId = dataJson.createCart.Pricebook2Id

  if (!self.testJobInfo.PricebookEntries[pricebookId]) {
    self.testJobInfo.PricebookEntries[pricebookId] = {}
  }

  let pricebookEntryId = self.testJobInfo.PricebookEntries[pricebookId][productGlobalKey]

  if (!pricebookEntryId) {
    const queryString = self.vlocity.queryservice.buildSOQL('Id', 'PricebookEntry', "Product2Id IN (SELECT Id FROM Product2 WHERE %vlocity_namespace%__GlobalKey__c = '" + productGlobalKey + "') AND Pricebook2Id = '" + pricebookId + "' LIMIT 1")
    result = await self.vlocity.queryservice.query(queryString)
  ...
  const body = { items: [{ itemId: pricebookEntryId }] }
  ...
  return new Promise(function (resolve, reject) {
    self.vlocity.jsForceConnection.sobject(name).retrieve(id, function (err, result) {
      if (err) {
        ...
        VlocityUtils.error('Can Not Retrieve SObject', name, id, err.message)
      }
