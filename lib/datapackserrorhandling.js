...
var DataPacksErrorHandling = module.exports = function(vlocity) {
    ...
    this.autoFixErrorsDefinition = yaml.safeLoad(fs.readFileSync(path.join(__dirname, "datapacksautofixerrors.yaml"), 'utf8'));
};
        ...
        if (dataPack
            ...
            for (var key in jobInfo.errorHandling) {
                if (!jobInfo.errorHandling[key].processed) {
                    ...
                    var matchingErrorKey = self.getMatchingError(this.errorHandlingDefinition, errorMessage, errHandlingObj.dataPack);
                    var newErrorMessage;
...
DataPacksErrorHandling.prototype.getMatchingError = function(definition, errorMessage, dataPackWithError) {
...
// ---------- AUTO FIX ERRORS ----------
DataPacksErrorHandling.prototype.autoFixDeployErrors = async function(jobInfo, dataPack) {
    if (jobInfo.autoFixDeployErrors) {
        var matchingErrorKey = this.getMatchingError(this.autoFixErrorsDefinition, dataPack.VlocityDataPackMessage, dataPack); 
        
        if (matchingErrorKey && jobInfo.autoFixDeployErrors[matchingErrorKey]) {
            VlocityUtils.report('Auto Fix Error On Deploy');
            await this['handle' + matchingErrorKey](jobInfo, dataPack);
        }
    }
};

DataPacksErrorHandling.prototype.handleAttributeCategoryDisplaySequence = async function(jobInfo, dataPackWithError) {
    ...
    this.vlocity.datapacksbuilder.allFileDataMap[this.vlocity.datapacksbuilder.recordSourceKeyToFilePath[dataPack.VlocityRecordSourceKey]] = null;
    
    await this.vlocity.datapacksexpand.expand(jobInfo.projectPath + '/' + jobInfo.expansionPath, {dataPacks: [dataPackWithError]}, jobInfo);
    jobInfo.resetFileData = true;
    await this.vlocity.datapacksjob.deployJob(jobInfo, true);

    if (jobInfo.currentStatus[dataPackWithError.VlocityDataPackKey] === 'Success') {
        dataPackWithError.VlocityDataPackStatus = "Success";
    }
};
