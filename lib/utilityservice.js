...
UtilityService.prototype.getNamespace = async function() {
    ...
    if (!this.vlocity.namespace) {
        ...
        try {
            ...
            {
                let indexOfDR = res.name.indexOf('_OmniScript__c');

                if (res.name == 'OmniScript__c') {
...
UtilityService.prototype.getFiles = function (dir, fileMap = {}) {
    fs.readdirSync(dir).forEach(file => {
        if (file !== 'node_modules' &&
            ...
            if (fs.statSync(filePath).isDirectory()) {
                fileMap = this.getFiles(filePath, fileMap);
            } else {
                let fileTypeAndName = dir.substr(dir.lastIndexOf('/') + 1) + '/' + file;
                let fileFolderPath = dir.substr(0, dir.lastIndexOf('/'));
                if (fileMap[fileTypeAndName]) {
                    fileMap[fileTypeAndName].push(fileFolderPath);
                } else {
                    fileMap[fileTypeAndName] = [fileFolderPath];
                }
            }
        }
    });
...
UtilityService.prototype.getDuplicateFilesErrorMessages = function (fileKey, filePathArr, inputDir) {
    if (inputDir.endsWith('/')) {
        inputDir = inputDir.substr(0, inputDir.lastIndexOf('/'));
    }
    let message = `${fileKey} exists in ${filePathArr.length} repositories - `;
    filePathArr.forEach(filePath => {
        message += filePath.replace(inputDir + '/', '') + ' and ';
    });
    return message.substr(0, message.lastIndexOf(' and '));
}