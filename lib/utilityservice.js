...
const UtilityService = module.exports = function (vlocity) {
...
UtilityService.prototype.getAllDRMatchingKeys = async function () {
  var matchingKeysMap = {}
  var objectAPIName = this.checkNamespacePrefix('%vlocity_namespace%__ObjectAPIName__c')
  var matchingKeyField = this.checkNamespacePrefix('%vlocity_namespace%__MatchingKeyFields__c')
...
UtilityService.prototype.createSObject = async function (sObjectType, sObject) {
  return this.vlocity.jsForceConnection.sobject(sObjectType).create(sObject)
}
  ...
  if (!this.vlocity.namespace) {
    ...
    try {
      ...
      if (this.vlocity.passedInNamespace) {
        ...
        throw new Error('Namespace Query ' + err)
      }
    }
    ...
    if (this.vlocity.namespace === null) {
      throw new Error('No namespace found - Either set the vlocity.namespace property on the CLI or ensure you have the DRDataPackService class deployed')
    }
...
UtilityService.prototype.getFieldsDefinitionsMap = function (sObjectDescribe) {
  ...
  for (var i = 0; i < sObjectDescribe.fields.length; i++) {
    fieldsDefinitionMap[sObjectDescribe.fields[i].name] = sObjectDescribe.fields[i]
  }
