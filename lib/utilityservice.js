...
UtilityService.prototype.checkOmniStudioPackage = async function() {
    try {
        this.vlocity.jsForceConnection.version = '51.0';
        let result = await this.vlocity.jsForceConnection.query(`Select count() from OmniInteractionConfig where DeveloperName = 'TheFirstInstalledOmniPackage'`);
...
UtilityService.prototype.getGitDiffsFromOrgToLocal = async function(jobInfo) {
    try {
        ...
        if (hashKey) {
            ...
            var gitChanges = childProcess.execSync(`cd ${jobInfo.projectPath} && git diff --stat ${hashKey} --raw --no-renames`, { encoding: 'utf8',  maxBuffer: 1024*1024*20  });
