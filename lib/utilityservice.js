...
UtilityService.prototype.createCustomMetadataRecord = async function(metadata) {
    ...
    if (metadata) {
        metadata = JSON.parse(JSON.stringify(metadata).replace(new RegExp('%vlocity_namespace_replace%', 'g'), this.vlocity.namespace));
...
UtilityService.prototype.getVlocityDataPackConfigurations = async function() {
    var fields = ['Id', 'DeveloperName', 'NamespacePrefix'];
    return this.queryCustomMetadata(fields, '%vlocity_namespace%__VlocityDataPackConfiguration__mdt');
};

UtilityService.prototype.getDRMatchingKeys = async function() {
    var fields = ['Id', 'DeveloperName', 'NamespacePrefix'];
    return this.queryCustomMetadata(fields, '%vlocity_namespace%__DRMatchingKey__mdt');
};

UtilityService.prototype.queryCustomMetadata = async function(fields, metadataType, whereClause) {
    var resultMap = {};

    var soqlQuery = await this.vlocity.queryservice.buildSOQL(fields, metadataType, whereClause);
    var queryResult = await this.vlocity.queryservice.query(soqlQuery);
    ...
    if (queryResult && queryResult.records) {
        ...
        for (var i = 0; i < records.length; i++) {
            var developerName = records[i]['DeveloperName'];

            if (resultMap[developerName]) {
                if (records[i]['NamespacePrefix'] !== null) {
                    continue;
                }
            }

            resultMap[developerName] = records[i];
        }
    }
...
UtilityService.prototype.getDRMatchingKeyFields = async function() {
    ...
    var queryResult = await this.vlocity.queryservice.query('SELECT Id,DeveloperName,NamespacePrefix,%vlocity_namespace%__ObjectAPIName__c,%vlocity_namespace%__MatchingKeyFields__c FROM %vlocity_namespace%__DRMatchingKey__mdt');
...
UtilityService.prototype.retrieveCustomObject = async function(sObjectAPIName) {
    ...
    sObjectAPIName = this.vlocity.utilityservice.setNamespaceToOrg(sObjectAPIName);
    
    if (!(sObjectAPIName instanceof Array)) {
        sObjectAPIName = [sObjectAPIName];
    }

    var result = await this.vlocity.jsForceConnection.metadata.read('CustomObject', sObjectAPIName, function(err, metadata) {
        if (err) { 
            VlocityUtils.error('Error Retrieve Custom Object', err);
        }
    });
...
UtilityService.prototype.updateCustomObject = async function(metadata) {
    ...
    var result = await this.vlocity.jsForceConnection.metadata.upsert('CustomObject', metadata, function(err, res) {
        if (err) { 
            VlocityUtils.error('Error Update Custom Object', err);
        }
    });
