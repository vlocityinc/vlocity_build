...
// Escape a value for safe inclusion inside a SOQL single-quoted string literal.
// Backslash must be escaped first so it can't escape the escaping of the quote.
UtilityService.prototype.escapeSOQLString = function(value) {
    if (value == null) {
        return value;
    }
...
UtilityService.prototype.parallelLimit = async function(inputList, limit = 50) {
    ...
    if (status.errors.length > 0) {
        VlocityUtils.error(status.errors);
    }
};
...
UtilityService.prototype.checkLogin = async function() {
    ...
    try {
        if (this.vlocity.authFilePath) { 
            var res = await sfdx.run("org login",{authfilepath:this.vlocity.authFilePath});
        } else if (this.vlocity.sessionId || this.vlocity.accessToken) {
...
UtilityService.prototype.sfdx = async function(command, options) {
    ...
    try {
        return  sfdx.run(command,options);
    } catch (e) {
...
UtilityService.prototype.describeSObject = async function(sObjectName) {
    var self = this;
    try {
        const result = await self.vlocity.jsForceConnection
            .sobject(self.setNamespaceToOrg(sObjectName))
            .describe();

        return self.setNamespaceToDefault(result);
    } catch (err) {
    ...
    try {
        ...
        if (hashKey) {
            ...
            if (jobInfo.expansionPath) {
                var gitChanges = childProcess.execSync(`cd ${sfdx.shellEscapeArg(jobInfo.projectPath)} && git diff --stat ${sfdx.shellEscapeArg(hashKey)} --raw --no-renames -- ${sfdx.shellEscapeArg(jobInfo.expansionPath)}`, { encoding: 'utf8',  maxBuffer: 1024*1024*20  });
            } else {
                var gitChanges = childProcess.execSync(`cd ${sfdx.shellEscapeArg(jobInfo.projectPath)}/../ && git diff --stat ${sfdx.shellEscapeArg(hashKey)} --raw --no-renames -- ${sfdx.shellEscapeArg(jobInfo.projectPath)}`, { encoding: 'utf8',  maxBuffer: 1024*1024*20  });
            }
            ...
            var gitNewFiles = childProcess.execSync(`cd ${sfdx.shellEscapeArg(jobInfo.projectPath)} && git ls-files --others --exclude-standard`, { encoding: 'utf8' });
...
UtilityService.prototype.getSfdxCommandRunner = function(jobInfo) {
    if (!commandExistsSync('sf')) {
        VlocityUtils.error('Please install sf cli as this tool cannot proceed further without sf commands');
        throw new Error("Cannot  proceed further Sf command doesnot exist , please install  sf cli")
    }
    VlocityUtils.log('Using sf cli');
    return sfdx.run;
}
...
UtilityService.prototype.retrieveMetadata = async function(type, component) {
    if (type && component) {
        ...
        try {
            ...
            return this.vlocity.utilityservice.setNamespaceToDefault(result[0]);
        } catch (e) {
...
UtilityService.prototype.extractSessionStorageData = async function (page, componentKey, jobInfo, componentType = 'Component') {
    try {
        VlocityUtils.report('Extracting session storage data for bulk deployment', componentKey);
        ...
        if (!bulkData || (typeof bulkData === 'object' && Object.keys(bulkData).length === 0)) {
            VlocityUtils.warn('No bulkAllResources data found in session storage', componentKey);
            return;
        }
        ...
        const processFile = async (relativePath, content) => {
            try {
                VlocityUtils.verbose(`Processing bulk item: ${relativePath}`, componentKey);
                ...
                if (relativePath === 'package.xml' || relativePath === 'bulkCompiledCount') {
                    ...
                    VlocityUtils.verbose(`Extracted file: ${relativePath}`, componentKey);
                }
            } catch (error) {
                VlocityUtils.warn(`Failed to process bulk item ${relativePath}`, componentKey, error);
        ...
        results.forEach((result, index) => {
            if (result.status === 'fulfilled') {
                if (result.value && result.value.success === false) {
                    failureCount++;
                    VlocityUtils.error(`Failed to process file: ${result.value.key}`, componentKey, result.value.error);
                } else {
                    successCount++;
                }
            } else {
                failureCount++;
                VlocityUtils.error(`Failed to process file at index ${index}`, componentKey, result.reason);
            }
        });

        VlocityUtils.report(`File processing completed: ${successCount} successful, ${failureCount} failed`, componentKey);

        VlocityUtils.success('Session storage data extracted successfully', componentKey);
        jobInfo.deployGeneratedLwc = true;
    } catch (error) {
        VlocityUtils.error('Failed to extract session storage data', componentKey, error);
    }
}
