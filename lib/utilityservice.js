...
UtilityService.prototype.sfdx = async function(command, options) {
    ...
    var argumnentVars = [ `force:${command}`, '--json',  ];

    for (var key in options) {
        argumnentVars.push(`--${key}`);
        argumnentVars.push(options[key]);
    }
    
    return await new Promise((resolve, reject) => {

        var execFile = `${path.join(__dirname, "../node_modules/sfdx-cli/bin/run")}`;

        if (!fs.existsSync(execFile)) {
            execFile = `${path.join(__dirname, "../../node_modules/sfdx-cli/bin/run")}`;
        }

        childProcess.execFile(execFile, argumnentVars, { stdio: 'inherit' }, function(error, stdout, stderr) {
            var standardResponse;

            try {
                VlocityUtils.verbose('Standard Out', stdout);
                standardResponse = JSON.parse(stdout);
            } catch (e) {
                VlocityUtils.verbose('Parsing stdout failed');
            }

            if (stderr) { 
                try {
                    VlocityUtils.verbose('Standard Err', stderr);
                    return reject(JSON.parse(stderr).result);
                } catch (e) {
                    VlocityUtils.verbose('Parsing stderr failed');
                }
            }
                
            if (standardResponse) {
                if (standardResponse.exitCode == 1) {
                    reject(standardResponse);
                } else {
                    resolve(standardResponse.result);
                }
            } else {
                reject(null);
            }
