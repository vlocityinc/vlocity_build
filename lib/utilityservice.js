...
UtilityService.prototype.parallelLimit = async function(inputList, limit = 20) {
    ...
    let promisesForLimit = await this.parallelLimitRunner(inputList, limit);

    await Promise.all(promisesForLimit);
};

UtilityService.prototype.parallelLimitRunner = async function(inputList, limit = 20) {
    let inFlight = new Set();

    return await inputList.map(async (inputMap) => {
