...
UtilityService.prototype.sfdxLogin = async function() {
    ...
    try {
        var orgInfo =  this.sfdx('org:display', { targetusername: this.vlocity.sfdxUsername, verbose: true });
        VlocityUtils.report("SFDX login sucessful for user ", orgInfo.username,orgInfo.instanceUrl ,orgInfo.apiVersion);
...
UtilityService.prototype.sfdx = async function(command, options) {
    ...
    if(options.sessionId && options.instanceUrl){
        ...
        var ConfigSetCommand = require("@salesforce/plugin-config/config/set");
    ...
    try {
        if (command == 'org:display') {
            sfdxCommand = sfdx.run(command,options)
        }    
    /*   } else if (command == 'source:retrieve') {
            var SourceRetrieveCommand = require("@salesforce/plugin-deploy-retrieve/lib/commands/project/retrieve/start");
            sfdxCommand = new SourceRetrieveCommand.SourceRetrieveCommand();
        } else if (command == 'source:deploy') {
            var SourceDeployCommand = require("@salesforce/plugin-deploy-retrieve/lib/commands/project/deploy/start");
            ...
            var SourceDeleteCommand = require("@salesforce/plugin-deploy-retrieve/lib/commands/project/delete/source");
            sfdxCommand = new SourceDeleteCommand.SourceDeleteCommand();
            sfdxCommand.ux = await require("@salesforce/command").UX.create();
        } else if (command == 'mdapi:listmetadata') {
            var MdapiListmetadataCommand = require("@salesforce/plugin-org/lib/commands/org/list/metadata");
            sfdxCommand = new MdapiListmetadataCommand.MdapiListmetadataCommand();
        } else if (command == 'source:convert') {
            var SourceConvertCommand = require("@salesforce/plugin-deploy-retrieve/lib/commands/project/convert/source");
            sfdxCommand = new SourceConvertCommand.SourceConvertCommand();
            sfdxCommand.ux = await require("@salesforce/command").UX.create();
        } else if (command == 'mdapi:deploy') {
            var MdapiDeployCommand = require("@salesforce/plugin-deploy-retrieve/lib/commands/project/deploy/start");
...
UtilityService.prototype.getSfdxCommandRunner = function(jobInfo) {
    if(!commandExistsSync('sf')){
        VlocityUtils.log(' Cannot  proceed further Sf command doesnot exist , please install sf');
        throw new Error("Cannot  proceed further Sf command doesnot exist , please install sf")
    }
        VlocityUtils.log('Using sfdx cli');
        return sfdx.run;
}
