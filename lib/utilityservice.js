...
UtilityService.prototype.checkLogin = async function(options) {
    ...
    try {
        if (this.vlocity.sessionId || this.vlocity.accessToken) {
            ...
            await this.checkRequiredVersion(options);
            var identity = await this.vlocity.jsForceConnection.identity();
            if (identity) {
                this.vlocity.organizationId = identity.organization_id;
            }
...
UtilityService.prototype.checkRequiredVersion = async function(options) {
    let requiredVersion = await this.getVlocitySetting('VBTRequiredVersion',options);
...
UtilityService.prototype.getGitDiffsFromOrgToLocal = async function(jobInfo) {
    try {
        ...
        var hashKey = await this.getVlocitySetting(vbtDeployKey, jobInfo);
...
UtilityService.prototype.setVlocitySetting = async function(settingName, value, jobInfo) {
    var customSettingAPIName = `${this.vlocity.namespacePrefix}GeneralSettings__c`;
    var valueColumn = `${this.vlocity.namespacePrefix}Value__c`;
    if (jobInfo.generalSettingsCustomSetting) {
        customSettingAPIName = jobInfo.generalSettingsCustomSetting
        valueColumn = 'Value__c';
    }
    try {    
        let result = await this.vlocity.jsForceConnection.query(`Select Id, ${valueColumn} from ${customSettingAPIName} where Name = '${settingName}'`);

        var settingsRecord = {};
        settingsRecord.Name = settingName;
        settingsRecord[`${valueColumn}`] = value;
        ...
        if (settingsRecord.Id) {
            VlocityUtils.verbose('Set Setting Update', await this.vlocity.jsForceConnection.sobject(`${customSettingAPIName}`).update([ settingsRecord ], {}));
        } else {
            VlocityUtils.verbose('Set Setting Insert', await this.vlocity.jsForceConnection.sobject(`${customSettingAPIName}`).insert([ settingsRecord ], {}));
        }
    } catch (e) {
...
UtilityService.prototype.getVlocitySetting = async function(settingName, jobInfo) {
    var customSettingAPIName = `${this.vlocity.namespacePrefix}GeneralSettings__c`;
    var valueColumn = `${this.vlocity.namespacePrefix}Value__c`;
    if (jobInfo && jobInfo.generalSettingsCustomSetting) {
        customSettingAPIName = jobInfo.generalSettingsCustomSetting
        valueColumn = 'Value__c';
    }
    try {    
        let result = await this.vlocity.jsForceConnection.query(`Select ${valueColumn} from ${customSettingAPIName} where Name = '${settingName}'`);

        if (result && result.records.length != 0) {
            return result.records[0][`${valueColumn}`];
        }
    } catch (e) {
