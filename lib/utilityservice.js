...
UtilityService.prototype.getNamespace = async function() {
    ...
    if (!this.vlocity.namespace) {
        ...
        try {
            ...
            {
                let indexOfOS = res.name.indexOf('OmniScript__c')
                if (indexOfOS != -1) {
                    this.vlocity.namespace = res.name.substring(0, indexOfOS).replace('__', '');
                }
            }
            ...
            if (sfdxFolder && fs.existsSync(path.join(sfdxProject.sfdxPath, sfdxFolder, 'main', 'default', fileOrFolderName))) {
                ...
                try {
                    if (fs.lstatSync(dirPath).isDirectory()) {
                        VlocityUtils.verbose('Committing', path.join(dirPath, '*'));
                        await simpleGit(jobInfo.localRepoPath).add(path.join(dirPath, '*'));
...
UtilityService.prototype.runGitPull = async function (jobInfo) {
    ...
    try {
        ...
        let mergeResult = await gitP(jobInfo.localRepoPath).merge([jobInfo.branchName]);
