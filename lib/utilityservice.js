...
UtilityService.prototype.updateSObject = async function(sObjectType, sObject) {
    if (sObject && !this.isEmptyObject(sObject)) {
        ...
        var result = await this.vlocity.jsForceConnection.sobject(sObjectType).update(sObject, function(err, result){
            if (err) { 
                VlocityUtils.error('Update Failed', err); 
                return;
            }
...
UtilityService.prototype.retrieveMetadata = async function(type, component) {
    if (type && component) {
        ...
        var result = await this.vlocity.jsForceConnection.metadata.read(type, component, function(err, metadata) {
            if (err) { 
                VlocityUtils.error('Retrieve Failed', err);
            }
        });
    ...
    if (queryResult && queryResult.records.length > 0) {
        ...
        for (var i = 0; i < queryResult.records.length; i++) {
            ...
            for (var field in queryResult.records[i]) {
                ...
                matchingKeyValue += queryResult.records[i][field];
            }
            ...
            if (uniqueKeys.hasOwnProperty(matchingKeyValue)) {
                ...
                VlocityUtils.report('Duplicate Found', 'Record: ' + uniqueKeys[matchingKeyValue]['Id']);
                jobInfo.report.push('Duplicate Found >> ' + 'Record: ' + uniqueKeys[matchingKeyValue]['Id']);
            } else {
