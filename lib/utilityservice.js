...
UtilityService.prototype.getGitDiffsFromOrgToLocal = async function(jobInfo) {
    try {
        ...
        if (hashKey) {
            ...
            VlocityUtils.log('sfdxProjectFolder', sfdxProjectFolder);
            for (var potentialFile of allPotentialFiles) {
                ...
                if (sfdxProjectFolder) {
                    if (potentialFile.indexOf('default/') != -1) {
                        VlocityUtils.log('FILES', potentialFile);
                        var potetnialSfdxFile = potentialFile.substring(potentialFile.indexOf('default/') + 8);
                        var potentialSfdxFileSplit = potetnialSfdxFile.split('/');
                        ...
                        VlocityUtils.log('Here2', path.join(jobInfo.projectPath, sfdxProjectFolder, 'main', 'default', dataPackKey));

                        if (fs.existsSync(path.join(jobInfo.projectPath, sfdxProjectFolder, 'main', 'default', dataPackKey)) && !changedDataPacks.includes(dataPackKey)) {
...
UtilityService.prototype.runGitCommit = async function (jobInfo) {
    ...
    if (Array.isArray(jobInfo.manifest)) {
        for (var fileOrFolderName in jobInfo.manifest) {

            var sanitizedFileorFoldername = unidecode(jobInfo.manifest[fileOrFolderName])
