...
// This is a terrible hack to try to get SFDX Output when it errors
UtilityService.stderroriginal = process.stderr.write;
UtilityService.sfdxResponse;
UtilityService.handlestderr = function(...args) {
    try {
        UtilityService.sfdxResponse = JSON.parse(`${args[0]}`);
    } catch (e) {
        
    }

    UtilityService.stderroriginal(args);
};

process.stderr.write = UtilityService.handlestderr;
...
UtilityService.prototype.sfdxLogin = async function() {
    ...
    try {
        ...
        throw this.loginFailedMessage('Salesforce DX Org Info Not Found', e.message);
    }
}
...
UtilityService.prototype.runGitCheckoutBranch = async function (jobInfo) {
    ...
    try {
        if(jobInfo.createNewBranch) {
            await simpleGit(jobInfo.localRepoPath).checkoutBranch(jobInfo.currentBranch, 'master');
        }
        else {
            await simpleGit(jobInfo.localRepoPath).checkout([jobInfo.currentBranch]);
        }
    }
    catch (err) {
        jobInfo.hasError = true;
...
UtilityService.prototype.runGitCurrentBranch = async function (jobInfo) {
    ...
    await simpleGit(jobInfo.localRepoPath).raw(['rev-parse', '--abbrev-ref', 'HEAD'], (err, result) => {
...
UtilityService.prototype.runGitCommit = async function (jobInfo) {
    ...
    try {
        await simpleGit(jobInfo.localRepoPath).commit(jobInfo.commitMessage);
    }
    catch (err) {
        jobInfo.hasError = true;
        jobInfo.errors.push(err);
...
UtilityService.prototype.runGitPush = async function (jobInfo) {
    ...
    try {
        await simpleGit(jobInfo.localRepoPath).push('origin', jobInfo.targetBranch, { '-f': null },
            () => VlocityUtils.verbose('changes have been pushed to the branch'));
    }
    catch (err) {
        jobInfo.hasError = true;
        jobInfo.errors.push(err);
...
UtilityService.prototype.runGitPull = async function (jobInfo) {
    ...
    if (!jobInfo.currentBranch) {
        jobInfo.currentBranch = 'master';
    }

    try {
        await simpleGit(jobInfo.localRepoPath).pull('origin', jobInfo.currentBranch)
    }
    catch (err) {
        jobInfo.hasError = true;
        jobInfo.errors.push(err);
