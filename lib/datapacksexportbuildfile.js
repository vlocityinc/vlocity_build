...
const DataPacksExportBuildFile = module.exports = function (vlocity) {
...
DataPacksExportBuildFile.prototype.setFilename = function (jobInfo) {
  ...
  } else if (this.vlocity.tempFolder) {
...
DataPacksExportBuildFile.prototype.loadExportBuildFile = function (jobInfo) {
  ...
  if (!this.currentExportFileData && this.filename) {
    try {
...
DataPacksExportBuildFile.prototype.saveFile = function () {
  ...
  if (self.currentExportFileData && this.filename) {
    Object.keys(self.currentExportFileData).forEach(function (dataPackId) {
      savedFormat.push(self.currentExportFileData[dataPackId])
    })

    var saved = stringify(savedFormat, { space: 4 })

    VlocityUtils.verbose('Saving', self.filename, saved.length)

    fs.outputFileSync(self.filename, saved, 'utf8')
  }
}
