...
if (global.VlocityUtils) {
  module.exports = global.VlocityUtils
} else {
  const VlocityUtils = {
  ...
  VlocityUtils.output = function (loggingMethod, color, messageArray) {
    ...
    if (VlocityUtils.verboseLogging) {
      VlocityUtils.fullLog.push(newMessage.replace(color, '').replace(/\x1b\[0m/g, '')) // eslint-disable-line no-control-regex
    }
  }

  module.exports = global.VlocityUtils = VlocityUtils
}
