var AttributeAssignmentRule = module.exports = function(vlocity) {
    this.vlocity = vlocity;
};

AttributeAssignmentRule.prototype.childrenExport = async function(inputMap) {
	try {
		...
		let queryString = `Select Id from ${this.vlocity.namespacePrefix}AttributeAssignmentRule__c WHERE `;
		let whereClause = '';
		...
		for (var dataPack of childrenDataPacks) {
			...
			whereClause += `${this.vlocity.namespacePrefix}AttributeCategoryId__c = '${dataPack.VlocityDataPackData.Id}'`;
		}
