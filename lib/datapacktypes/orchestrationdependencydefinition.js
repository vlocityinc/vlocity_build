var OrchestrationDependencyDefinition = module.exports = function(vlocity) {
    this.vlocity = vlocity;
};

OrchestrationDependencyDefinition.prototype.childrenExport = async function(inputMap) {
	try {
		...
		let queryString = `Select Id from ${this.vlocity.namespacePrefix}OrchestrationDependencyDefinition__c WHERE `;
		let whereClause = '';
		...
		for (var dataPack of childrenDataPacks) {
			...
			whereClause += `${this.vlocity.namespacePrefix}OrchestrationItemDefinitionId__c = '${dataPack.VlocityDataPackData.Id}'`;
		}
