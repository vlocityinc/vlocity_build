...
AttributeCategory.prototype.onDeployError = async function(inputMap) {
    ...
    if (dataPack.VlocityDataPackMessage.includes("%vlocity_namespace%__DisplaySequence__c")
        && jobInfo.autoFixDeployErrors.AttributeCategoryDisplaySequence) {
        await this.handleAttributeCategoryDisplaySequence(jobInfo, dataPack);
    }
};

AttributeCategory.prototype.handleAttributeCategoryDisplaySequence = async function(jobInfo, dataPackWithError) {
