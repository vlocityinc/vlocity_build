...
project.prototype.checkProjectItems = async function (dataObj) {
    let projectItems = dataObj.dataPacks[0].VlocityDataPackData['%vlocity_namespace%__Project__c'][0]['%vlocity_namespace%__ProjectItem__c'];
    for (let projectItem of projectItems) {
        if (projectItem['%vlocity_namespace%__ItemId__c'].length > 18 || projectItem['%vlocity_namespace%__ItemId__c'].includes('%vlocity_namespace%')) {
