var OrchestrationItemDefinition = module.exports = function(vlocity) {
    this.vlocity = vlocity;
};

OrchestrationItemDefinition.prototype.childrenExport = async function(inputMap) {
	try {
		...
		let queryString = `Select Id from ${this.vlocity.namespacePrefix}OrchestrationItemDefinition__c WHERE `;
		let whereClause = '';
		...
		for (var dataPack of childrenDataPacks) {
			...
			whereClause += `${this.vlocity.namespacePrefix}OrchestrationPlanDefinitionId__c = '${dataPack.VlocityDataPackData.Id}'`;
		}
