...
VlocityCard.prototype.flexCardDeployWithPuppeteer = async function(jobInfo){
        ...
        try { 
            ...
            if (idsArray.length < 1){ 
             ...
             const  compilerName = 'flexcard-compiler',
                    ...
                    jobInfo.lwcCompilerVersion = '236.0.0-beta.1207.0'; 
           
            const flexCardsCompiler = await VlocityUtils.loadCompiler(compilerName, jobInfo, packageVersion, namespace);
            if(flexCardsCompiler){
                for (let i = 0; i < idsArray.length; i++) { 
                    flexCardID = idsArray[i];
