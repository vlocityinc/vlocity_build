var VlocityAttachment = module.exports = function(vlocity) {
    this.vlocity = vlocity;
};

VlocityAttachment.prototype.childrenExport = async function(inputMap) {
	try {
		...
		let queryString = `Select Id from ${this.vlocity.namespacePrefix}VlocityAttachment__c WHERE `;
		let whereClause = '';
		...
		for (var dataPack of childrenDataPacks) {
			...
			whereClause += `${this.vlocity.namespacePrefix}ObjectId__c = '${dataPack.VlocityDataPackData.Id}'`;
		}
