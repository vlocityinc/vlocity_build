...
OmniScript.prototype.afterActivationSuccess = async function(inputMap) {
    ...
    var defaultNS = '%vlocity_namespace__%';
                        ...
                        if (Array.isArray(OmniScriptVersion[omniKey])) {
                            for (var OmniScriptData of OmniScriptVersion[omniKey]) {
                                ...
                                Object.keys(OmniScriptData).forEach(function (omniData) {
                                    if (omniData === this.eleFieldMap(`${defaultNS}ParentElementId__c`)) {
                                        tempData[this.eleFieldMap(`${defaultNS}ParentElementId__c`)] = OmniScriptData[this.eleFieldMap(`${defaultNS}ParentElementId__c`)]['VlocityMatchingRecordSourceKey'];
                                        //Add entry to parent-child relation map
                                        self.vlocity.relationMap[OmniScriptData['VlocityRecordSourceKey']] = tempData[this.eleFieldMap(`${defaultNS}ParentElementId__c`)];
                                    }
                                    else if ((omniData.endsWith('__c') || omniData === 'Name' || omniData === 'VlocityRecordSourceKey') && omniData !== this.eleFieldMap(`${defaultNS}ReusableOmniScript__c`)) {
