...
OmniScript.prototype.onDeployFinish = async function (jobInfo) {
    if (jobInfo.resusableOSToCompile) {
        ...
        for (let i = 0; i < idsArray.length; i++) {
            ...
            if (omniScriptId) {
                // Check if LWC already exists when bulk mode is enabled (used for caching)
                if (jobInfo.useBulkOmniScriptLwcCompile) {
                    const lwcExists = await this.checkLwcExists(omniScriptKey);
                    if (lwcExists) {
                        VlocityUtils.report('Parent OmniScript LWC components already exist, skipping compilation', omniScriptKey);
                        continue;
                    }
                }
                await this.compileOmniScriptLwc(jobInfo, omniScriptId, omniScriptKey);
    ...
    // Handle bulk LWC deployment after all OmniScripts (including parents) are compiled
    if (jobInfo.useBulkOmniScriptLwcCompile && jobInfo.bulkLwcDeployFolders?.length > 0) {
        VlocityUtils.report('Starting bulk LWC deployment after parent OmniScript compilation');
        await this.deployBulkLwcComponents(jobInfo);
    } else {
        VlocityUtils.verbose('Bulk LWC deployment skipped - conditions not met');
    }
}

    OmniScript.prototype.deployBulkLwcComponents = async function (jobInfo) {
        const sfdxProjectPath = process.cwd();
        const forceAppDefaultPath = path.join(sfdxProjectPath, 'force-app', 'main', 'default');
        
        // Helper function to clean up LWC folder
        const cleanupLwcFolder = async () => {
            try {
                const lwcFolderToRemove = path.join(forceAppDefaultPath, 'lwc');
                if (await fs.pathExists(lwcFolderToRemove)) {
                    await fs.remove(lwcFolderToRemove);
                    VlocityUtils.verbose('Successfully removed LWC components from force-app folder');
                }
            } catch (cleanupError) {
                VlocityUtils.warn('Failed to clean up LWC components from force-app folder:', cleanupError);
            }
        };

        try {
            VlocityUtils.report('Starting bulk LWC deployment', `Files already written to SFDX project`);
            VlocityUtils.report('Using SFDX project at:', sfdxProjectPath);
            
            // Ensure manifest folder exists
            const manifestFolder = path.join(sfdxProjectPath, 'manifest');
            await fs.ensureDir(manifestFolder);
            
            VlocityUtils.report('Deploying LWC components using source:deploy from SFDX project');
            
            // List contents for debugging
            const defaultContents = await fs.readdir(forceAppDefaultPath, { recursive: true });
            VlocityUtils.verbose('force-app/main/default folder contents:', JSON.stringify(defaultContents, null, 2));
            
            // Deploy using source:deploy
            const deployResult = await this.vlocity.utilityservice.sfdx('source:deploy', {
                sourcepath: path.join(sfdxProjectPath, 'force-app'),
                targetusername: this.vlocity.username
            });
            
            if (deployResult && deployResult.success) {
                VlocityUtils.success('Bulk LWC deployment completed successfully');
                
                // Log deployment details
                if (deployResult.details?.componentSuccesses) {
                    VlocityUtils.report('Successfully deployed components:', deployResult.details.componentSuccesses.length);
                    deployResult.details.componentSuccesses.forEach(component => {
                        VlocityUtils.verbose(`Deployed: ${component.fullName} (${component.componentType})`);
                    });
                }
                
                if (deployResult.details?.componentFailures) {
                    VlocityUtils.warn('Failed components:', deployResult.details.componentFailures.length);
                    deployResult.details.componentFailures.forEach(component => {
                        VlocityUtils.error(`Failed: ${component.fullName} - ${component.problem}`);
                    });
                }
            } else {
                VlocityUtils.error('Bulk LWC deployment failed', deployResult);
            }
            
            // Clean up LWC folder regardless of success/failure
            await cleanupLwcFolder();
            
        } catch (error) {
            VlocityUtils.error('Bulk LWC deployment failed', error);
            await cleanupLwcFolder();
            throw error;
        } finally {
            // Clear the folders array
            jobInfo.bulkLwcDeployFolders = [];
        }
    }
...
OmniScript.prototype.checkLwcExists = async function (omniScriptKey) {
    try {
        const sfdxProjectPath = process.cwd();
        const lwcFolderPath = path.join(sfdxProjectPath, 'force-app', 'main', 'default', 'lwc');
        
        // Check if LWC folder exists
        if (!await fs.pathExists(lwcFolderPath)) {
            return false;
        }
        
        // Extract component name from OmniScript key
        // e.g., "OmniScript/Child_Child_English" -> "childChildEnglish"
        const parts = omniScriptKey.split('/');
        if (parts.length < 2) {
            return false;
        }
        
        const omniScriptName = parts[1];
        const componentName = omniScriptName.replace(/([A-Z])/g, (match, letter, index) => {
            return index === 0 ? letter.toLowerCase() : letter;
        });
        
        const componentFolder = path.join(lwcFolderPath, componentName);
        ...
        // Check if required LWC files exist
        const requiredFiles = [
            `${componentName}.js`,
            `${componentName}.html`,
        ...
        VlocityUtils.warn('Error checking LWC existence', omniScriptKey, error);
        return false;
    }
}
...
OmniScript.prototype.compileOSLWC = async function (jobInfo, omniScriptId, omniScriptKey) {
        ...
        const promiseJob = new Promise(async (res,rej)=>{
            try {
                ...
                if (jobInfo.useBulkOmniScriptLwcCompile) {
                    const lwcExists = await this.checkLwcExists(omniScriptKey);
                    if (lwcExists) {
                        VlocityUtils.report('LWC components already exist, skipping compilation', omniScriptKey + ' (' + omniScriptId + ')');
                        res();
                        return;
                    }
                }
    ...
    lastPromise = lastPromise.then(()=>_compileOSLWC(jobInfo, omniScriptId, omniScriptKey))
    await lastPromise;
...
OmniScript.prototype.extractSessionStorageData = async function (page, omniScriptKey, jobInfo) {
    try {
        ...
        const bulkData = await page.evaluate(() => {
            ...
            if (bulkAllResources) {
                try {
                    ...
                    console.log('🔍 DEBUG: Failed to parse bulkAllResources as JSON');
                    return null;
                }
            }
            return null;
        ...
        // Get SFDX project path and create force-app/main/default structure
        const sfdxProjectPath = process.cwd();
        const forceAppDefaultPath = path.join(sfdxProjectPath, 'force-app', 'main', 'default');
        await fs.ensureDir(forceAppDefaultPath);
        
        VlocityUtils.verbose('Writing files directly to SFDX project:', forceAppDefaultPath);

        // Process each file in bulkAllResources
        const processFile = async (relativePath, content) => {
            try {
                ...
                if (relativePath === 'package.xml') {
                    ...
                    const fullPath = path.join(forceAppDefaultPath, relativePath);
        ...
        for (const [key, value] of Object.entries(bulkData)) {
            await processFile(key, value);
        }

        // Mark that we have extracted files for bulk deployment
        if (!jobInfo.bulkLwcDeployFolders) {
            jobInfo.bulkLwcDeployFolders = [];
        }
        jobInfo.bulkLwcDeployFolders.push(forceAppDefaultPath);
