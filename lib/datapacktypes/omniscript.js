...
OmniScript.prototype.compileOSLWC = async function (jobInfo, omniScriptId, omniScriptKey) {
    const _compileOSLWC = async (jobInfo, omniScriptId, omniScriptKey) => {
            ...
            try {
                ...
                if (!puppeteerOptions.executablePath && !jobInfo.puppeteerInstalled) {
                    ...
                    try {
                        ...
                        browser = await puppeteer.launch({...puppeteerOptions, executablePath: "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"});
                    } catch (error) {
        ...
        return new Promise(async(res,rej)=>{
            await promiseJob
            await promiseJob //re-activating to make sure it gets activated
            res()
        });
    }
