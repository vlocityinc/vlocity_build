...
OmniScript.prototype.compileOmniScriptLwc = async function (jobInfo, omniScriptId, omniScriptKey) {
    ...
    try {
        ...
        if (!lwcCompilerNamespace) {
            switch (namespace) {
                case 'vlocity_ins': lwcCompiler = '@vlocity-ins/omniscript-lwc-compiler'; break;
                case 'vlocity_cme': lwcCompiler = '@vlocity-cme/omniscript-lwc-compiler'; break;
                case 'vlocity_ps': lwcCompiler = '@vlocity-ps/omniscript-lwc-compiler'; break;
                case 'omnistudio': lwcCompiler = '@omnistudio/omniscript-lwc-compiler'; break;
                default:
                    VlocityUtils.warn('Using default @omnistudio/omniscript-lwc-compiler');
                    lwcCompiler = '@omnistudio/omniscript-lwc-compiler';
                    break;
            }
        } else {
            lwcCompiler = `@${lwcCompilerNamespace}/omniscript-lwc-compiler`;
        }

        // If the compiler version was not provided, use the package version
        if (!lwcCompilerVersion) {
            if (packageVersion === 'DeveloperOrg') {
                // select specific version, have global variable for this.
            } else {
        ...
        let omniscriptCompiler;

        try {
            omniscriptCompiler = await VlocityUtils.requirePlugin(jobInfo, lwcCompiler, lwcCompilerVersion);
        } catch (err) {
        ...
        VlocityUtils.verbose(`Compiling OmniScript using ${lwcCompiler}`);
