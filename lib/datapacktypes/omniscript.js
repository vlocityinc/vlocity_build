        ...
        if (dataPack.VlocityDataPackData[this.osObjMap(defaultNS)][0][this.osFieldMap('%vlocity_namespace%__IsReusable__c')]) {
            ...
            let reusableOmnis = await this.vlocity.jsForceConnection.query(
                                    this.vlocity.omnistudio.updateQuery(`SELECT Id, 
                                                                        ...
                                                                        WHERE ${nsPrefix}SearchKey__c = '${this.vlocity.utilityservice.escapeSOQLString(searchKey)}' AND ${nsPrefix}OmniScriptId__r.${nsPrefix}IsActive__c = true`
                                    )
                                );
    ...
    if (jobInfo.resusableOSToCompile) {
        ...
        for(let key of osKeysArray) {
            ...
            if(omniScript[`${nsPrefix}OmniScriptId__r`]) {
                ...
                query = this.vlocity.omnistudio.updateQuery(`SELECT Id FROM ${nsPrefix}OmniScript__c WHERE ${nsPrefix}Type__c = '${this.vlocity.utilityservice.escapeSOQLString(type)}' AND ${nsPrefix}SubType__c = '${this.vlocity.utilityservice.escapeSOQLString(subType)}' AND ${nsPrefix}Language__c = '${this.vlocity.utilityservice.escapeSOQLString(lang)}' AND ${nsPrefix}IsActive__c = true`);
            } else {
                query = this.vlocity.omnistudio.updateQuery(`SELECT Id FROM ${nsPrefix}OmniScript__c WHERE ${nsPrefix}Type__c = '${this.vlocity.utilityservice.escapeSOQLString(omniScript.OmniProcess.Type)}' AND ${nsPrefix}SubType__c = '${this.vlocity.utilityservice.escapeSOQLString(omniScript.OmniProcess.SubType)}' AND ${nsPrefix}Language__c = '${this.vlocity.utilityservice.escapeSOQLString(omniScript.OmniProcess.Language)}' AND ${nsPrefix}IsActive__c = true`);
            }
                            ...
                            if (message) {
                                ...
                                if (currentStatus === 'DONE') {
                                    ...
                                    if (jobInfo.useBulkOmniScriptLwcCompile) {
                                        await this.vlocity.utilityservice.extractSessionStorageData(page, omniScriptKey, jobInfo, 'OmniScript');
                                    }
...
OmniScript.prototype.onActivateError = async function (dataPackData) {
    ...
    if (currentOmniScriptId && omniScriptDataPack[this.osFieldMap(`${defaultNS}IsReusable__c`)]) {
        ...
        let reusableOmnis = await this.vlocity.jsForceConnection.query(this.vlocity.omnistudio.updateQuery(`SELECT Id, ${this.vlocity.namespacePrefix}OmniScriptId__c FROM ${this.vlocity.namespacePrefix}Element__c WHERE ${this.vlocity.namespacePrefix}SearchKey__c = '${this.vlocity.utilityservice.escapeSOQLString(searchKey)}' AND ${this.vlocity.namespacePrefix}OmniScriptId__r.${this.vlocity.namespacePrefix}IsActive__c = true`));
