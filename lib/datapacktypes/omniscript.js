...
OmniScript.prototype.createTitleObjects = function(input) {
    ...
    var titleObjects = [];
    let omniScriptDataField = '%vlocity_namespace%__OmniScript__c',
        element_c_DataField = '%vlocity_namespace%__Element__c';
    if(!dataPackData.VlocityDataPackData['%vlocity_namespace%__OmniScript__c']){
        omniScriptDataField = 'OmniScript__c';
    }
    if(!dataPackData.VlocityDataPackData[omniScriptDataField][0]['%vlocity_namespace%__Element__c']){
        element_c_DataField = 'Element__c';
    }
    if (dataPackData.VlocityDataPackData[omniScriptDataField][0][element_c_DataField]) {
        dataPackData.VlocityDataPackData[omniScriptDataField][0][element_c_DataField] = this.vlocity.datapacksexpand.sortList(dataPackData.VlocityDataPackData[omniScriptDataField][0][element_c_DataField], omniScriptDataField);
    }
    
    var elements = dataPackData.VlocityDataPackData[omniScriptDataField][0][element_c_DataField];

    var sourceKey = dataPackData.VlocityDataPackData[omniScriptDataField][0].VlocityRecordSourceKey;

    if(dataPackData.VlocityDataPackData[omniScriptDataField][0]['%vlocity_namespace%__Type__c']){
        var omniscriptName = `${dataPackData.VlocityDataPackData[omniScriptDataField][0]['%vlocity_namespace%__Type__c']} ${dataPackData.VlocityDataPackData[omniScriptDataField][0]['%vlocity_namespace%__SubType__c']} ${dataPackData.VlocityDataPackData[omniScriptDataField][0]['%vlocity_namespace%__Language__c']}`;
    } else{
        var omniscriptName = `${dataPackData.VlocityDataPackData[omniScriptDataField][0]['Type__c']} ${dataPackData.VlocityDataPackData[omniScriptDataField][0]['SubType__c']} ${dataPackData.VlocityDataPackData[omniScriptDataField][0]['Language__c']}`;
    }
    ...
    if (elements) {
        ...
        dataPackData.VlocityDataPackData[omniScriptDataField][0][element_c_DataField] = this.vlocity.datapacksexpand.sortList(elements, omniScriptDataField);

        elements = dataPackData.VlocityDataPackData[omniScriptDataField][0][element_c_DataField]; 
        ...
        for (var element of elements) {
            ...
            try {
                var parentElementDataField = '%vlocity_namespace%__ParentElementId__c';
                if(!element['%vlocity_namespace%__ParentElementId__c'] && element['ParentElementId__c']){
                    parentElementDataField = 'ParentElementId__c';
                }
                if (element[parentElementDataField] != null) {
                    var parent = elementByKey[element[parentElementDataField].VlocityMatchingRecordSourceKey];

                    if (!parent.childElements) {
                        parent.childElements = [];
                    }

                    elementIndentation = indentations[element[parentElementDataField].VlocityMatchingRecordSourceKey] + 2;
...
OmniScript.prototype.onActivateError = async function(dataPackData) {
    ...
    if (currentOmniScriptId && omniScriptDataPack['%vlocity_namespace%__IsReusable__c']) {
        ...
        if (!this.vlocity.sfdxUsername && !this.vlocity.oauthConnection) {
            ...
            if(!omniScriptDataPack['%vlocity_namespace%__Type__c']){
                defaultNamespace = '';
            }
                    ...
                    Object.keys(OmniScriptVersion).forEach(function (omniKey) {
                        if (Array.isArray(OmniScriptVersion[omniKey])) {
                            for (var OmniScriptData of OmniScriptVersion[omniKey]) {
                                var tempData = {},
                                    parentElementDataField = '%vlocity_namespace%__ParentElementId__c';
                                if(!OmniScriptData['%vlocity_namespace%__ParentElementId__c']){
                                    parentElementDataField = 'ParentElementId__c';
                                }
                                Object.keys(OmniScriptData).forEach(function (omniData) {
                                    if (omniData === parentElementDataField) {
                                        tempData[parentElementDataField] = OmniScriptData[parentElementDataField]['VlocityMatchingRecordSourceKey'];
                                        //Add entry to parent-child relation map
                                        self.vlocity.relationMap[OmniScriptData['VlocityRecordSourceKey']] = tempData[parentElementDataField];
                                    }
                                    else if ((omniData.endsWith('__c') || omniData === 'Name' || omniData === 'VlocityRecordSourceKey') && omniData !== '%vlocity_namespace%__ReusableOmniScript__c' && omniData !== 'ReusableOmniScript__c') {
...
OmniScript.incrementElements = function(elements, orderToIncrementAfter) {
    elements.forEach(element => {
        ...
        if (element[orderCDataField + 'Order__c'] >= orderToIncrementAfter) {
            element[orderCDataField + 'Order__c'] = element[orderCDataField + 'Order__c'] + 1;
        }
    })
}
...
OmniScript.staticHandleRevert = async function(input) {
    ...
    var defaultNamespace = '%vlocity_namespace%__';
    if(!sourceDataPack.VlocityDataPackData['%vlocity_namespace%__OmniScript__c']){
        defaultNamespace = '';
    }
    var sourceElements = '',
        targetElements = '';
    ...
    if(targetDataPack.VlocityDataPackData[defaultNamespace + 'OmniScript__c']){
        targetElements = targetDataPack.VlocityDataPackData[defaultNamespace + 'OmniScript__c'][0][defaultNamespace + 'Element__c'];
    } else{
        targetElements = targetDataPack.VlocityDataPackData['OmniScript__c'][0][defaultNamespace + 'Element__c'];
    }
...
OmniScript.prototype.hashSObjectData = async function(input) {
    ...
    var propertySetDataField = '%vlocity_namespace%__PropertySet__c';
    if(!sobject['%vlocity_namespace%__PropertySet__c']){
        propertySetDataField = 'PropertySet__c';
    }
    if (sobject && sobject['%vlocity_namespace%__PropertySet__c']) {
        sobject[propertySetDataField] = sobject[propertySetDataField].replace(/id=01.+?&/g, 'id=<SalesforceId>').replace(/oid=00.+?\\/g, 'oid=<SalesforceId>\\').replace(/oid=00.+?"/g, 'oid=<SalesforceId>"').replace(/file=015.+?&/g, 'file=<SalesforceId>&');
    }
};
