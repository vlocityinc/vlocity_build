...
OmniScript.prototype.createTitleObjects = function(input) {
    ...
    var titleObjects = [];
    let dataField = '%vlocity_namespace%__OmniScript__c';
    if(!dataPackData.VlocityDataPackData['%vlocity_namespace%__OmniScript__c']){
        dataField = 'OmniScript__c';
    }
    if (dataPackData.VlocityDataPackData[dataField][0]['%vlocity_namespace%__Element__c']) {
        dataPackData.VlocityDataPackData[dataField][0]['%vlocity_namespace%__Element__c'] = this.vlocity.datapacksexpand.sortList(dataPackData.VlocityDataPackData[dataField][0]['%vlocity_namespace%__Element__c'], '%vlocity_namespace%__OmniScript__c');
    }
    
    var elements = dataPackData.VlocityDataPackData[dataField][0]['%vlocity_namespace%__Element__c'];

    var sourceKey = dataPackData.VlocityDataPackData[dataField][0].VlocityRecordSourceKey;

    var omniscriptName = `${dataPackData.VlocityDataPackData[dataField][0]['%vlocity_namespace%__Type__c']} ${dataPackData.VlocityDataPackData[dataField][0]['%vlocity_namespace%__SubType__c']} ${dataPackData.VlocityDataPackData[dataField][0]['%vlocity_namespace%__Language__c']}`;

    var relevantFields = [ 'Name', '%vlocity_namespace%__Type__c', 'VlocityRecordSourceKey' ];

    if (elements) {

        dataPackData.VlocityDataPackData[dataField][0]['%vlocity_namespace%__Element__c'] = this.vlocity.datapacksexpand.sortList(elements, '%vlocity_namespace%__OmniScript__c');

        elements = dataPackData.VlocityDataPackData[dataField][0]['%vlocity_namespace%__Element__c']; 
...
OmniScript.staticHandleRevert = async function(input) {
    ...
    if(sourceDataPack.VlocityDataPackData['%vlocity_namespace%__OmniScript__c']){
        sourceElements = sourceDataPack.VlocityDataPackData['%vlocity_namespace%__OmniScript__c'][0]['%vlocity_namespace%__Element__c'];
    } else{
        sourceElements = sourceDataPack.VlocityDataPackData['OmniScript__c'][0]['%vlocity_namespace%__Element__c'];
    }
    
    if(targetDataPack.VlocityDataPackData['%vlocity_namespace%__OmniScript__c']){
        targetElements = targetDataPack.VlocityDataPackData['%vlocity_namespace%__OmniScript__c'][0]['%vlocity_namespace%__Element__c'];
    } else{
        targetElements = targetDataPack.VlocityDataPackData['OmniScript__c'][0]['%vlocity_namespace%__Element__c'];
    }
...
OmniScript.prototype.hashSObjectData = async function(input) {
    ...
    if (sobject && sobject['%vlocity_namespace%__PropertySet__c']) {
        sobject['%vlocity_namespace%__PropertySet__c'] = sobject['%vlocity_namespace%__PropertySet__c'].replace(/id=01.+?&/g, 'id=<SalesforceId>').replace(/oid=00.+?\\/g, 'oid=<SalesforceId>\\').replace(/oid=00.+?"/g, 'oid=<SalesforceId>"').replace(/file=015.+?&/g, 'file=<SalesforceId>&');
    }
};
