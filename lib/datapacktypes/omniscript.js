...
OmniScript.prototype.afterActivationSuccess = async function(inputMap) {
    ...
    if (dataPack.VlocityDataPackData['%vlocity_namespace%__OmniScript__c'][0]['%vlocity_namespace%__IsLwcEnabled__c']) {
        try {
            ...
            VlocityUtils.log('Going to OmniScript Designer', omniScriptId);
            ...
            while (tries < 10) {
                try {
                    ...
                    if (message) { 
                        ...
                        VlocityUtils.verbose('Activating LWC', dataPack.VlocityDataPackKey, currentStatus);

                        if (currentStatus === 'DONE') {
                            VlocityUtils.success('Activated LWC', dataPack.VlocityDataPackKey);
                            break;
                        } else if (/^ERROR/.test(currentStatus)) {
                            VlocityUtils.error('Error Activating LWC', dataPack.VlocityDataPackKey, currentStatus);
                            break;
                        }
                    }
                } catch {
