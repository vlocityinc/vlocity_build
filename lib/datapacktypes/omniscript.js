...
OmniScript.prototype.deactivateOmniScript = async function(omniscriptID) {
    ...
    try {
        ...
        if(!result.success){
            VlocityUtils.error('LWC Activation Error', 'Id: ' + omniscriptID + ' Unable to deactivate OmniScript after unsuccessful LWC compilation. OmniScript is still activated.');
        }
        else {
            VlocityUtils.log('Deactivating OmniScript', omniscriptID, 'OmniScript was deactivated');
        }
    } catch (error) {
        VlocityUtils.error('LWC Activation Error', 'Id: ' + omniscriptID + 'Unable to deactivate OmniScript after unsuccessful LWC compilation. OmniScript is still activated - ' + error);
    }
}
