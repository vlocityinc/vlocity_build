...
OmniScript.prototype.compileOSLWC = async function (jobInfo, omniScriptId, omniScriptKey) {
    lastPromise = lastPromise.then(() => compileOSLWCJob(jobInfo, omniScriptId, omniScriptKey, async (omniscript)=> await this.deactivateOmniScript(omniScriptId,jobInfo), this.vlocity));
    await lastPromise;
};
