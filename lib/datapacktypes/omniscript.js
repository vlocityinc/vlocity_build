...
OmniScript.prototype.afterActivationSuccess = async function(inputMap) {
    ...
    if (dataPack.VlocityDataPackData['%vlocity_namespace%__OmniScript__c'][0]['%vlocity_namespace%__IsLwcEnabled__c']) {
        ...
        if (jobInfo.isRetry && jobInfo.omniScriptLwcActivationSkip[dataPack.VlocityDataPackKey]) { 
            ...
            jobInfo.errors.push('OmniScript LWC Activation Error >> ' + dataPack.VlocityDataPackKey + oldError);
            VlocityUtils.error('OmniScript LWC Activation Error', dataPack.VlocityDataPackKey + oldError);
        ...
        try {
            ...
            if (!puppeteerOptions.executablePath && !jobInfo.puppeteerInstalled) {
                ...
                let tries = 0;
                var errorMessage;
                var maxNumOfTries = jobInfo.defaultMinToWaitForLWCOmniScript*2;
