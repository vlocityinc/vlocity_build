...
// Gates the CWE-78 shell-escaping fix (W-24098246), mirroring the enforceSecurityValidations
// flag used for the CWE-22 path-traversal fix in datapacksexpand.js. Defaults to enforcing;
// set jobInfo/CLI enforceSecurityValidations to false only if single-quoting breaks a
// legitimate workflow and the old double-quoted behavior must be restored.
let enforceSecurityValidations = true;
const setEnforceSecurityValidations = (value) => {
    enforceSecurityValidations = value !== false;
}
