...
const _run = (command) => {
	...
	if (stdout.status == 1){
		const errorDetail = stdout.message || (stdout.result && stdout.result.details && JSON.stringify(stdout.result.details.componentFailures)) || '';
		const error = new Error(`Command responded with error: ${command}${errorDetail ? '\n' + errorDetail : ''}`);
...
const _sourceDeploy = async (targetusername,sourcePath) =>{
	...
	return _run(`sf project deploy start --source-dir "${sourcePath}" --target-org "${targetusername}" --no-track-source`);
}
